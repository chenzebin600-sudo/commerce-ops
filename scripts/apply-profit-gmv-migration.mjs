...
const version = "020_profit_gmv_module.sql";
const confirmation = "PROFIT_GMV_MODULE_V1";
...
async function main() {
  ...
  if (!apply) {
    return {
      ...
      applyCommand: `node scripts/apply-profit-gmv-migration.mjs --apply --confirm-database=${config.database} --confirm-migration=${confirmation}`,
    };
  }
...
main().then((result) => process.stdout.write(`${JSON.stringify(result, null, 2)}\n`)).catch((error) => {
  process.stderr.write(`Profit GMV migration failed: ${String(error?.message || error).split(/\r?\n/)[0].slice(0, 500)}\n`);
  process.exitCode = 1;
});
