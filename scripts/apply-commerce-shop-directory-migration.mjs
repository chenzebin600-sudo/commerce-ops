...
const migrationConfirmation = "COMMERCE_SHOP_DIRECTORY";
...
async function main() {
  ...
  const setConfirmation = process.argv.find((value) => value.startsWith("--confirm-migration="))?.split("=")[1];
  ...
  if (databaseConfirmation !== config.database || setConfirmation !== migrationConfirmation) {
    throw new Error(`Apply requires --confirm-database=${config.database} --confirm-migration=${migrationConfirmation}`);
  }
  ...
  try {
    ...
    for (const migration of migrations) {
      ...
      if (existing) {
        ...
        results.push({ status: "ALREADY_APPLIED", version: migration.version, sha256: migration.sha256 });
        continue;
      }
      ...
      results.push({ status: "APPLIED", version: migration.version, sha256: migration.sha256 });
    }
...
main().then((result) => process.stdout.write(`${JSON.stringify(result, null, 2)}\n`)).catch((error) => {
  process.stderr.write(`Commerce shop directory migration failed: ${String(error?.message || error).split(/\r?\n/)[0].slice(0, 500)}\n`);
  process.exitCode = 1;
});
