...
function dashboardRoute(url, method) {
  ...
  if (method === "POST" && pathname === "/api/fulfillment-dashboard/warehouse-transfers/batch-preview-tasks") return "/api/fulfillment/warehouse-transfers/batch-preview-tasks";
  ...
  if (method === "POST" && pathname === "/api/fulfillment-dashboard/sku-replacements/batch-preview-tasks") return "/api/fulfillment/sku-replacements/batch-preview-tasks";
  ...
  const warehousePreviewTaskMatch = pathname.match(/^\/api\/fulfillment-dashboard\/warehouse-transfers\/batch-preview-tasks\/([a-zA-Z0-9-]{1,80})$/);
  if (method === "GET" && warehousePreviewTaskMatch) return `/api/fulfillment/warehouse-transfers/batch-preview-tasks/${encodeURIComponent(warehousePreviewTaskMatch[1])}`;
  const skuPreviewTaskMatch = pathname.match(/^\/api\/fulfillment-dashboard\/sku-replacements\/batch-preview-tasks\/([a-zA-Z0-9-]{1,80})$/);
  if (method === "GET" && skuPreviewTaskMatch) return `/api/fulfillment/sku-replacements/batch-preview-tasks/${encodeURIComponent(skuPreviewTaskMatch[1])}`;
  return null;
}
...
async function skuReplacementPlanBody(req) {
  ...
  const rawTargetWarehouse = String(payload.targetWarehouse || "");
  const targetWarehouse = rawTargetWarehouse.trim();
  if (!/^[a-zA-Z0-9_-]{4,100}$/.test(orderReference) || !/^\d{1,40}$/.test(itemId) || !replacementSku
    || replacementSku.length > 160 || /[\u0000-\u001f\u007f]/.test(replacementSku)
    || targetWarehouse.length > 160 || /[\u0000-\u001f\u007f]/.test(rawTargetWarehouse)) {
    throw Object.assign(new Error("订单、商品行或替换 SKU 无效"), { code: "INVALID_SKU_REPLACEMENT_PLAN" });
  }
  return JSON.stringify({ orderReference, itemId, replacementSku, ...(targetWarehouse ? { targetWarehouse } : {}) });
}
...
async function skuReplacementBatchPlanBody(req) {
  ...
  const selections = payload.selections.map((raw) => ({
    ...
    rawTargetWarehouse: String(raw?.targetWarehouse || ""),
    targetWarehouse: String(raw?.targetWarehouse || "").trim(),
  }));
  const keys = new Set();
  for (const selection of selections) {
    ...
    if (!/^[a-zA-Z0-9_-]{4,100}$/.test(selection.orderReference) || !/^\d{1,40}$/.test(selection.itemId)
      ...
      || selection.targetWarehouse.length > 160 || /[\u0000-\u001f\u007f]/.test(selection.rawTargetWarehouse)
      || keys.has(key)) {
      throw Object.assign(new Error("批量更换中存在无效或重复的商品行"), { code: "INVALID_SKU_REPLACEMENT_BATCH_PLAN" });
    }
    keys.add(key);
  }
  return JSON.stringify({ selections: selections.map(({ rawTargetWarehouse, targetWarehouse, ...selection }) => ({
    ...selection, ...(targetWarehouse ? { targetWarehouse } : {}),
  })) });
}
...
export function createFulfillmentDashboardProxy({
  ...
  return async function proxyFulfillmentDashboard(req, res, url) {
    ...
    try {
      ...
      else if (req.method === "POST" && upstreamPath === "/api/fulfillment/warehouse-transfers/batch-preview-tasks") requestBody = await warehouseTransferBatchPreviewBody(req);
      ...
      else if (req.method === "POST" && upstreamPath === "/api/fulfillment/sku-replacements/batch-preview-tasks") requestBody = await skuReplacementBatchPreviewBody(req);
