...
async function warehouseTransferBatchPreviewBody(req) {
  ...
  const targetWarehouse = String(payload.targetWarehouse || "").trim();
  if (!orderReferences.length || orderReferences.length > 10 || orderReferences.some((value) => !/^[a-zA-Z0-9_-]{4,100}$/.test(value)) || targetWarehouse.length > 160) {
    throw Object.assign(new Error("请输入 1-10 个有效订单号"), { code: "INVALID_WAREHOUSE_BATCH_PREVIEW" });
  }
...
async function warehouseTransferBatchExecuteBody(req) {
  ...
  const approvalText = String(payload.approvalText || "").trim();
  if (!/^[a-f0-9]{64}$/i.test(batchHash) || !planHashes.length || planHashes.length > 10
  ...
  return async function proxyFulfillmentDashboard(req, res, url) {
    ...
    try {
      ...
      const response = await fetchImpl(new URL(upstreamPath, upstreamBase), {
        ...
        headers,
        body: requestBody,
        signal: AbortSignal.timeout(suggestionScan ? 600000 : longRunningRequest ? 120000 : 8000),
      });
