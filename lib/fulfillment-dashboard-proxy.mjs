  ...
  return async function proxyFulfillmentDashboard(req, res, url) {
    ...
    try {
      const requestBody = req.method === "POST" && upstreamPath === "/api/fulfillment/manual-reviews/recheck"
        ? await recheckBody(req) : req.method === "POST" ? "{}" : undefined;
      const response = await fetchImpl(new URL(upstreamPath, upstreamBase), {
        method: req.method,
        headers,
        body: requestBody,
        signal: AbortSignal.timeout(req.method === "POST" ? 120000 : 8000),
      });
      ...
      if (["BODY_TOO_LARGE", "INVALID_BODY", "INVALID_MANUAL_REVIEW"].includes(error?.code)) {
