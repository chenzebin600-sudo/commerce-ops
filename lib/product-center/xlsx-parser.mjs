...
export function parseProductPackageXlsx({
  pythonExecutable,
  parserScript,
  filename,
  stagingPath = null,
  ...
  return new Promise((resolve, reject) => {
    const args = [parserScript, filename, "--max-rows", String(maxRows)];
    if (stagingPath) args.push("--output", stagingPath);
    ...
    let outputBytes = 0;
    let settled = false;
    const outputLimit = stagingPath ? 2 * 1024 * 1024 : 128 * 1024 * 1024;
