...
export function createProductCenterApi({ service, maxUploadBytes }) {
    ...
    try {
      if (url.pathname === "/api/product-center/imports") {
        ...
        if (req.method === "POST") {
          ...
          if (!result.reused) req.auditContext?.addRelated("product", "product.import.validated", { metadata: { batchId: result.batch.id, rowCount: result.batch.rowCount } });
          return sendJson(res, result.reused ? 200 : 201, { ok: true, reused: result.reused, detail: publicDetail(result.detail) });
        }
        return sendJson(res, 405, { ok: false, error: "Method not allowed" });
      }

      const match = url.pathname.match(/^\/api\/product-center\/imports\/([0-9a-f-]+)(?:\/(rows|issues|apply))?$/i);
      ...
      req.auditContext?.setOperation("product", "product.import.failed");
      req.auditContext?.annotate({
      ...
      return sendJson(res, publicCodes.has(code) ? Number(error?.status || 400) : 500, {
        ok: false,
        code: publicCodes.has(code) ? code : "PRODUCT_IMPORT_FAILED",
        error: publicCodes.has(code) ? String(error?.message || "产品包导入失败。").split(/\r?\n/, 1)[0].slice(0, 180) : "产品包导入失败。",
      });
    }
  };
}
