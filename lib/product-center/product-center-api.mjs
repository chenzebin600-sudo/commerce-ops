...
export function createProductCenterApi({
  ...
  return async function handleProductCenterApi(req, res, url) {
    ...
    try {
      ...
      if (url.pathname === "/api/product-center/product-models" && req.method === "GET") {
        accessPolicy.assert("product.view");
        const result = await catalogService.listModels({
          keyword: url.searchParams.get("keyword"),
          country: url.searchParams.get("country"),
          accessScope: accessPolicy.listScope,
          page: url.searchParams.get("page"),
      ...
      if (url.pathname === "/api/product-center/products/table-fields" && req.method === "GET") {
        accessPolicy.assert("product.view");
        return sendJson(res, 200, { ok: true, ...(await catalogService.tableFields()) });
      }
      if (url.pathname === "/api/product-center/products/table-preferences" && req.method === "PUT") {
        accessPolicy.assert("product.view");
        const body = await readJson(req);
        const preference = await catalogService.saveTableFieldPreference(body.visibleFields, {
