  ...
  return async function handleProductCenterApi(req, res, url) {
    ...
    try {
      ...
      if (url.pathname === "/api/product-center/ai/status" && req.method === "GET") {
        ...
        return sendJson(res, 200, { ok: true, ...(aiContentService?.status() || { configured: false }) });
      }
      ...
      if (aiMatch) {
        ...
        if (action === "contents" && contentId && subAction === "confirm" && req.method === "POST") {
          ...
          if (!product) return sendJson(res, 404, { ok: false, error: "产品不存在。" });
          const content = await aiContentService.confirm(product, contentId, audit);
      ...
      if (productMatch && req.method === "GET") {
        ...
        if (product && aiContentService) product.confirmedAiContent = await aiContentService.latestConfirmed(product.id);
      ...
      const publicCodes = new Set([
        ...
        "PRODUCT_AI_EMPTY_RESPONSE", "PRODUCT_AI_RESPONSE_TOO_LARGE", "PRODUCT_AI_RESPONSE_NOT_JSON", "PRODUCT_AI_RESPONSE_SCHEMA_INVALID", "AI_NOT_CONFIGURED",
        "PRODUCT_LISTING_PLATFORM_INVALID", "PRODUCT_LISTING_NOT_FOUND", "PRODUCT_LISTING_UNAVAILABLE",
