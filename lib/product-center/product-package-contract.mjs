...
const field = (sourceHeader, systemField, type = "text", options = {}) => Object.freeze({
  ...
  systemField,
  type,
  requiredHeader: options.requiredHeader !== false,
  requiredValue: Boolean(options.requiredValue),
});

export const PRODUCT_PACKAGE_FIELDS = Object.freeze([
  field("周期", "source_period"),
  field("SKU", "sku_code", "text", { requiredValue: true }),
  field("商品名称", "product_name", "text", { requiredValue: true }),
  field("主SKU", "main_sku_code"),
  field("国家", "country_raw"),
  field("一级品类", "category_l1", "text", { requiredValue: true }),
  field("二级品类", "category_l2", "text", { requiredValue: true }),
  field("创建日期", "source_created_date", "date"),
  field("新款年月", "new_product_month"),
  field("新款月龄", "new_product_age_months", "number"),
  field("赠品", "gift_raw"),
  field("SKU状态", "source_status", "text", { requiredValue: true }),
  field("款号", "style_code"),
  field("款名", "style_name"),
  ...
  field("销售成本人民币", "cost_cny", "number", { requiredValue: true }),
  field("国家汇率", "exchange_rate", "number", { requiredValue: true }),
  field("销售成本国家币", "cost_local", "number", { requiredValue: true }),
...
export const PRODUCT_PACKAGE_FIELD_BY_HEADER = new Map(
  [...PRODUCT_PACKAGE_FIELDS, ...PRODUCT_PACKAGE_OPTIONAL_FIELDS].map((item) => [item.sourceHeader, item]),
);
...
export function buildFieldMapping(headers) {
  ...
  for (const contract of PRODUCT_PACKAGE_FIELDS) {
    if (!present.has(contract.sourceHeader)) {
      mapping.push({
        sourceIndex: null,
...
export function mapLifecycleStatus(sourceStatus, { missingMainSku = false } = {}) {
  if (missingMainSku) return { status: "DISCONTINUED", reasonCode: "CONFIRMED_DISCONTINUED_WITHOUT_MAIN_SKU" };
  const status = String(sourceStatus || "").trim().toLowerCase();
  if (["正常", "active", "在售", "销售中"].includes(status)) return { status: "ACTIVE", reasonCode: "CENTRAL_STATUS_ACTIVE" };
  if (["待开发", "新品", "new"].includes(status)) return { status: "NEW", reasonCode: "CENTRAL_STATUS_NEW" };
  if (["清仓", "clearance"].includes(status)) return { status: "CLEARANCE", reasonCode: "CENTRAL_STATUS_CLEARANCE" };
  if (["灭款", "停售", "停产", "discontinued"].includes(status)) return { status: "DISCONTINUED", reasonCode: "CENTRAL_STATUS_DISCONTINUED" };
