...
function listingOutputValidator(contentTypes) {
  return createAiOutputValidator({
    schemaId: `product-listing-content@${LISTING_AI_PROMPT_VERSION}`,
    parse: (content) => parseListingAiResponse(content, contentTypes),
    validate: () => true,
  });
}

function productOutputValidator(promptVersion) {
  return createAiOutputValidator({
    schemaId: `product-selling-points@${promptVersion}`,
    parse: parseProductAiResponse,
    validate: () => true,
  });
}
  ...
  async generateListingContent(product, input = {}, audit = {}) {
    ...
    try {
      ...
      const result = await this.gateway.complete({
        ...
        promptId: "product.listing-content",
        ...
        temperature: 0.2,
        outputValidator: listingOutputValidator(contentTypes),
        signal: audit.signal,
      });
      if (!result.success) throw aiGatewayError(result);
      const output = result.validatedOutput ?? parseListingAiResponse(result.content, contentTypes);
      const records = [];
  ...
  async generate(product, input = {}, audit = {}) {
    ...
    try {
      ...
      const result = await this.gateway.complete({
        ...
        promptId: "product.selling-points-scenarios",
        promptVersion: this.promptVersion,
        requestId: audit.requestId,
        model: this.model,
        ...
        responseFormat: { type: "json_object" },
        temperature: 0.2,
        outputValidator: productOutputValidator(this.promptVersion),
        signal: audit.signal,
      });
      if (!result.success) throw aiGatewayError(result);
      return {
        outputContent: result.validatedOutput ?? parseProductAiResponse(result.content),
