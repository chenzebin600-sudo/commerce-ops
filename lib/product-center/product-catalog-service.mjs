...
function sourceValues(product) {
  ...
  for (const [code, value] of Object.entries(product.manualOverrides || {})) values[code] = value;
  return values;
}
...
export class ProductCatalogService {
  ...
  async detail(id) {
    ...
    return { ...product, fieldValues: sourceValues(product), ...fieldConfig };
  }
  ...
  async update(id, values, audit = {}) {
    ...
    if (!Object.keys(normalized).length) throw Object.assign(new Error("没有可保存的修改。"), { code: "PRODUCT_FIELD_INVALID", status: 400 });
    return this.repository.saveOverrides(id, normalized, {
      operatorLabel: audit.operatorLabel || "local_session",
      requestId: audit.requestId || null,
    });
  }
