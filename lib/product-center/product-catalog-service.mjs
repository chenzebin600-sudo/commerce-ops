...
const PRODUCT_TABLE_SUMMARY_FIELDS = Object.freeze([
  Object.freeze({ code: "summary:country_main_sku", label: "国家 / 主 SKU", group: "summary", sourceColumn: null }),
  Object.freeze({ code: "summary:category_sales_spec", label: "类目 / 规格", group: "summary", sourceColumn: null }),
  Object.freeze({ code: "summary:lifecycle_status", label: "状态", group: "summary", sourceColumn: null }),
  Object.freeze({ code: "summary:data_status", label: "数据状态", group: "summary", sourceColumn: null }),
  Object.freeze({ code: "summary:updated_at", label: "更新", group: "summary", sourceColumn: null }),
]);

export const PRODUCT_TABLE_FIELDS = Object.freeze([
  ...PRODUCT_TABLE_SUMMARY_FIELDS,
  ...PRODUCT_PACKAGE_SOURCE_FIELDS.map((item) => Object.freeze({
    code: `source:${item.column}`,
    label: item.label,
...
export const PRODUCT_TABLE_DEFAULT_FIELDS = Object.freeze(PRODUCT_TABLE_SUMMARY_FIELDS.map((item) => item.code));
const PRODUCT_TABLE_FIELD_BY_CODE = new Map(PRODUCT_TABLE_FIELDS.map((item) => [item.code, item]));
...
export class ProductCatalogService {
  ...
  listModels(options) {
    return this.repository.listModels(options);
  }
  ...
  async tableFields() {
    const preference = await this.repository.getPreference("global_product_table");
    const preferred = Array.isArray(preference?.visibleFields)
      ? preference.visibleFields.map(String).filter((code) => PRODUCT_TABLE_FIELD_BY_CODE.has(code))
      : null;
    return {
      fields: PRODUCT_TABLE_FIELDS,
      visibleFields: preferred ?? [...PRODUCT_TABLE_DEFAULT_FIELDS],
  ...
  async detail(id) {
    ...
    return {
      ...
      sourceDatabaseFields: PRODUCT_PACKAGE_SOURCE_FIELDS.map((field) => ({
        code: field.column,
        label: field.label,
        value: product.sourceUnknownFields?.[field.column] ?? null,
      })),
      ...fieldConfig,
    };
  }
  ...
  async saveTableFieldPreference(visibleFields, audit = {}) {
    if (!Array.isArray(visibleFields)) {
      throw Object.assign(new Error("产品主表字段设置无效。"), { code: "PRODUCT_TABLE_FIELD_PREFERENCE_INVALID", status: 400 });
    }
    const normalized = [...new Set(visibleFields.map(String))].filter((code) => PRODUCT_TABLE_FIELD_BY_CODE.has(code));
    return this.repository.savePreference({
      scopeKey: "global_product_table",
      visibleFields: normalized,
      operatorLabel: audit.operatorLabel || "local_session",
      requestId: audit.requestId || null,
    });
