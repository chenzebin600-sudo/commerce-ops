...
export class SalesAssortmentRepository {
  ...
  placeholders(count, offset = 0) {
    return Array.from({ length: count }, (_, index) => this.provider.placeholder(offset + index + 1)).join(", ");
  }

  boolean(value) {
    return this.provider.dialect === DATABASE_DIALECTS.POSTGRESQL ? String(Boolean(value)) : String(Number(Boolean(value)));
  }
  ...
  async latestInventoryRows() {
    ...
    const result = await this.provider.query(`
      ...
      WHERE i.batch_id = ${this.provider.placeholder(1)}
  ...
  async currentOrderRows({ dateFrom = null, dateToExclusive = null } = {}) {
    ...
    if (dateFrom) {
      conditions.push(`h.paid_at >= ${this.provider.placeholder(params.length + 1)}`);
  ...
  async sourceRows(source, { page = 1, pageSize = 50 } = {}) {
    ...
    if (source === "orders") {
      ...
      const count = await this.provider.query(`
        ...
        FROM growth_order_lines l
        JOIN growth_order_headers h ON h.id = l.order_header_id
        WHERE l.is_current = ${this.boolean(true)} AND h.order_status IN (${placeholders})
      `, VALID_ORDER_STATUSES);
      const result = await this.provider.query(`
        ...
        LIMIT ${this.provider.placeholder(VALID_ORDER_STATUSES.length + 1)} OFFSET ${this.provider.placeholder(VALID_ORDER_STATUSES.length + 2)}
    ...
    if (source === "inventory") {
      ...
      const count = await this.provider.query(
        `SELECT COUNT(*) AS total FROM growth_inventory_snapshots WHERE batch_id = ${this.provider.placeholder(1)}`,
        [batchId],
      );
      const result = await this.provider.query(`
        SELECT i.*, r.raw_values_json
    ...
    const result = await this.provider.query(`
      ...
      WHERE source_system = 'company_product_center'
      ORDER BY source_row_key
      LIMIT ${this.provider.placeholder(1)} OFFSET ${this.provider.placeholder(2)}
    `, [limit, offset]);
    return { total: Number(count.rows[0]?.total || 0), rows: result.rows };
  }
}
