import { targetListPrice50FromCost } from "../product-package-sync/product-package-source-contract.mjs";
import { commerceBusinessDate } from "../data-foundation/business-time.mjs";
...
const DIRECT_ORDER_AMOUNT_SOURCE = "订单核算金额（人民币）";
const DERIVED_ORDER_AMOUNT_SOURCE = "订单核算金额（原始货币）×汇率（原始货币）";
...
function targetListPrice50Cny(payload) {
  const direct = targetListPrice50FromCost(payload?.cost_cny);
  if (direct !== null) return direct;

  const local = targetListPrice50FromCost(payload?.cost_local);
  const exchangeRate = number(payload?.exchange_rate);
  if (local === null || exchangeRate <= 0) return 0;
  if (payload?.exchange_direction === "cny_per_local") return local * exchangeRate;
  if (payload?.exchange_direction === "local_per_cny" || payload?.exchange_direction === "equivalent") {
    return local / exchangeRate;
  }
  return 0;
}
...
function dayKey(value) {
  return commerceBusinessDate(value);
}
...
function buildStyleStoreImpacts(stores, sufficient, comparisonDays) {
  return [...stores.values()].map((item) => {
    const changeRate = rateOfChange(item.currentQuantity, item.previousQuantity);
    const status = trendStatus(changeRate, item.currentQuantity, item.previousQuantity, sufficient);
...
function orderAmountEvidence(row, raw) {
  const storedAmount = nullableNumber(row.order_amount);
  const storedSource = text(row.order_amount_source_field);
  if (storedAmount !== null && storedSource === DIRECT_ORDER_AMOUNT_SOURCE) {
    return { amount: storedAmount, currency: "CNY", basis: "direct", sourceField: storedSource };
  }
  if (storedAmount !== null && storedSource === DERIVED_ORDER_AMOUNT_SOURCE) {
    return { amount: storedAmount, currency: "CNY", basis: "derived", sourceField: storedSource };
  }

  const rawCnyAmount = nullableNumber(raw[DIRECT_ORDER_AMOUNT_SOURCE]);
  if (rawCnyAmount !== null) {
    return { amount: rawCnyAmount, currency: "CNY", basis: "direct", sourceField: DIRECT_ORDER_AMOUNT_SOURCE };
  }

  const originalAmount = nullableNumber(raw["订单核算金额（原始货币）"]);
  const exchangeRate = nullableNumber(raw["汇率（原始货币）"]);
  if (originalAmount === null || exchangeRate === null || exchangeRate <= 0) return null;
  return {
    amount: originalAmount * exchangeRate,
    currency: "CNY",
    basis: "derived",
    sourceField: DERIVED_ORDER_AMOUNT_SOURCE,
  };
}

function orderGroups(facts) {
  const groups = new Map();
  let missingOrderKeyLineCount = 0;
  for (const fact of facts) {
    const orderHeaderId = text(fact.orderHeaderId);
    if (!orderHeaderId) {
      missingOrderKeyLineCount += 1;
      continue;
    }
    const group = groups.get(orderHeaderId) || { facts: [], lineIds: new Set(), missingLineKeyCount: 0 };
    group.facts.push(fact);
    const orderLineId = text(fact.orderLineId);
    if (orderLineId) group.lineIds.add(orderLineId);
    else group.missingLineKeyCount += 1;
    groups.set(orderHeaderId, group);
  }
  return { groups, missingOrderKeyLineCount };
}

function actualSalesCoverage({ scopeFacts, selectedFacts }) {
  const amountsByCurrency = new Map();
  const scope = orderGroups(scopeFacts);
  const selected = orderGroups(selectedFacts);
  let confirmedOrderCount = 0;
  let directOrderCount = 0;
  let derivedOrderCount = 0;
  let missingAmountOrderCount = 0;
  let conflictingAmountOrderCount = 0;
  let partialAttributionOrderCount = 0;
  let currencyMissingOrderCount = 0;
  let confirmedLineCount = 0;
  let unconfirmedLineCount = selected.missingOrderKeyLineCount;
  let unavailableLineCount = 0;

  for (const [orderHeaderId, selectedGroup] of selected.groups) {
    const scopeGroup = scope.groups.get(orderHeaderId);
    const attributionComplete = Boolean(scopeGroup)
      && scopeGroup.missingLineKeyCount === 0
      && selectedGroup.missingLineKeyCount === 0
      && scopeGroup.lineIds.size === selectedGroup.lineIds.size;
    if (!attributionComplete) {
      partialAttributionOrderCount += 1;
      unconfirmedLineCount += selectedGroup.facts.length;
      continue;
    }

    const evidence = selectedGroup.facts.map((fact) => fact.orderAmountEvidence).filter(Boolean);
    if (!evidence.length) {
      missingAmountOrderCount += 1;
      unavailableLineCount += selectedGroup.facts.length;
      continue;
    }
    const first = evidence[0];
    const conflict = evidence.some((item) => (
      item.currency !== first.currency || Math.abs(item.amount - first.amount) > 0.02
    ));
    if (conflict) {
      conflictingAmountOrderCount += 1;
      unconfirmedLineCount += selectedGroup.facts.length;
      continue;
    }
    if (!first.currency) {
      currencyMissingOrderCount += 1;
      unconfirmedLineCount += selectedGroup.facts.length;
      continue;
    }

    confirmedOrderCount += 1;
    confirmedLineCount += selectedGroup.facts.length;
    if (evidence.some((item) => item.basis === "direct")) directOrderCount += 1;
    else derivedOrderCount += 1;
    amountsByCurrency.set(first.currency, number(amountsByCurrency.get(first.currency)) + first.amount);
  }

  const currencies = [...amountsByCurrency.keys()].sort();
  const totalOrderCount = selected.groups.size;
  const issueCount = missingAmountOrderCount + conflictingAmountOrderCount
    + partialAttributionOrderCount + currencyMissingOrderCount + selected.missingOrderKeyLineCount;
  const allConfirmed = totalOrderCount > 0 && confirmedOrderCount === totalOrderCount && issueCount === 0;
  const status = currencies.length > 1
    ? "mixed_currency"
    : allConfirmed
      ? "confirmed"
      : confirmedOrderCount > 0
        ? "partial"
        : "unavailable";
  const availability = status === "confirmed"
    ? "available"
    : status === "partial" || status === "mixed_currency"
      ? "partial"
      : "unavailable";
  const currency = currencies.length === 1 ? currencies[0] : null;
  const amount = currency && confirmedOrderCount > 0 ? round(amountsByCurrency.get(currency)) : null;
  return {
    actualSalesAmount: amount,
    actualSalesAmountAvailability: availability,
    actualSalesAmountStatus: status,
    actualSalesAmountCurrency: currency,
    actualSalesAmountsByCurrency: Object.fromEntries(
      [...amountsByCurrency.entries()].map(([key, value]) => [key, round(value)]),
    ),
    actualSalesOrderCoverage: {
      ...
      conflictingAmountOrderCount,
      partialAttributionOrderCount,
      currencyMissingOrderCount,
      missingOrderKeyLineCount: selected.missingOrderKeyLineCount,
    },
    // Deprecated compatibility coverage: these counts describe whether each selected
    // line belongs to an order with publishable order-level evidence; line_amount is
    // intentionally not treated as additive actual sales.
    actualSalesLineCoverage: {
      totalLineCount: selectedFacts.length,
      confirmedLineCount,
      unconfirmedLineCount,
      unavailableLineCount,
      currencyMissingLineCount: currencyMissingOrderCount,
    },
  };
}
...
function publicTotals(value, ownDataDays = value.periodDays) {
  ...
  return {
    ...
    assortmentEstimatedAmount: round(assortmentAmount),
    ...
    ownEstimatedAmount: round(value.ownAmount),
...
function buildDailyTrend({
  ...
  maxOrderDay,
  filteredOrders,
  actualSalesScope = filteredOrders,
  assortmentDailyAmount,
}) {
  const dailySales = new Map(
    dateRange(startOrderDay, maxOrderDay).map((date) => [date, { amount: 0, quantity: 0, facts: [] }]),
  );
  for (const fact of filteredOrders) {
    const day = dailySales.get(fact.paidDay) || { amount: 0, quantity: 0, facts: [] };
    day.amount += fact.amount;
    day.quantity += fact.quantity;
    day.facts.push(fact);
    dailySales.set(fact.paidDay, day);
  }
  const actualSalesScopeByDay = new Map();
  for (const fact of actualSalesScope) {
    const facts = actualSalesScopeByDay.get(fact.paidDay) || [];
    facts.push(fact);
    actualSalesScopeByDay.set(fact.paidDay, facts);
  }
  return [...dailySales.entries()].map(([date, totals]) => ({
    date,
    ownAmount: round(totals.amount),
    ownEstimatedAmount: round(totals.amount),
    ownAmountStatus: "estimated",
    ownQuantity: round(totals.quantity),
    assortmentDailyAmount: round(assortmentDailyAmount),
    ...actualSalesCoverage({
      scopeFacts: actualSalesScopeByDay.get(date) || [],
      selectedFacts: totals.facts,
    }),
  }));
}
...
export class SalesAssortmentService {
  ...
  async sourceRows(input = {}) {
    ...
    if (!["orders", "inventory", "product-package"].includes(source)) {
      const error = new Error("数据源必须是 orders、inventory 或 product-package。");
      error.code = "SALES_ASSORTMENT_INVALID_SOURCE";
      error.status = 400;
      throw error;
    }
    ...
    const rows = result.rows.map((row) => {
      ...
      if (source === "orders") {
        return {
          ...
          lineAmount: nullableNumber(row.line_amount),
          lineAmountStatus: text(row.line_amount_status, "unavailable").toLowerCase(),
          currency: text(row.order_currency).toUpperCase() || null,
        };
      }
      if (source === "inventory") {
        return {
      ...
      return {
        ...
        costLocal: number(payload.cost_local),
        targetPrice50: targetListPrice50FromCost(payload.cost_local),
        targetPrice50Cny: targetListPrice50Cny(payload),
      };
    });
  ...
  async dashboard(input = {}) {
    const hasDateFrom = input.dateFrom !== undefined && input.dateFrom !== null && String(input.dateFrom).trim() !== "";
    const hasDateTo = input.dateTo !== undefined && input.dateTo !== null && String(input.dateTo).trim() !== "";
    if (hasDateFrom || hasDateTo) {
      const dateFrom = validDay(input.dateFrom);
      const dateTo = validDay(input.dateTo);
      if (!dateFrom || !dateTo || dateFrom > dateTo) {
        const error = new Error("开始日期和结束日期必须同时填写，且开始日期不能晚于结束日期。");
        error.code = "SALES_ASSORTMENT_INVALID_DATE_RANGE";
        error.status = 400;
        throw error;
      }
    }
    const transient = Boolean(input.transient);
    ...
    if (!transient && !input.forceRefresh && this.dashboardCache.has(cacheKey)) {
      const cached = this.dashboardCache.get(cacheKey);
      this.dashboardCache.delete(cacheKey);
      this.dashboardCache.set(cacheKey, cached);
      return cached;
    }
    if (!transient && !input.forceRefresh && this.dashboardInflight.has(cacheKey)) {
    ...
    const pending = this.buildDashboard(input, sourceData)
      ...
      .finally(() => {
        if (!transient && this.dashboardInflight.get(cacheKey) === pending) this.dashboardInflight.delete(cacheKey);
      });
    if (!transient) this.dashboardInflight.set(cacheKey, pending);
    return pending;
  }
  ...
  async loadReferenceData(cacheKey, {
    ...
    const pending = (async () => {
      // These snapshots are large in production. Reading them concurrently can
      // make otherwise healthy PostgreSQL queries exceed the statement timeout.
      const packageRows = await this.repository.productPackageRows();
      const inventoryResult = await this.repository.latestInventoryRows();
      const previousInventoryResult = this.repository.previousInventoryRows
        ? await this.repository.previousInventoryRows()
        : { batch: null, rows: [] };
      const result = {
        sourceStatus,
        mappingRows,
  ...
  async trend(input = {}) {
    return (await this.dashboard({ ...input, transient: true, projection: "trend" })).trend;
  }
  ...
  async buildDashboard(input = {}, preloaded = {}) {
    ...
    for (const row of packageRows) {
      const payload = row.normalized_payload_json === null || row.normalized_payload_json === undefined
        ? row
        : parseJson(row.normalized_payload_json);
      ...
      const standardPriceCny = targetListPrice50Cny(payload);
      const product = {
        country,
        sku,
        warehouse,
    ...
    for (const row of orderRows) {
      ...
      const lineAmount = nullableNumber(row.line_amount);
      const fact = {
        orderHeaderId: text(row.order_header_id),
        ...
        lineAmountStatus: text(row.line_amount_status, "unavailable").toLowerCase(),
        currency: text(row.order_currency).toUpperCase() || null,
        orderAmountEvidence: orderAmountEvidence(row, raw),
        paidDay,
      };
    ...
    const actualSalesScope = orderFacts.filter((item) => (
      withinRange(item.paidDay, startOrderDay, maxOrderDay)
      && (!filters.store || item.store === filters.store)
    ));
    const actualSalesSelectedFacts = actualSalesScope.filter((item) => filterMatch(item, filters));
    const actualSales = actualSalesCoverage({
      scopeFacts: actualSalesScope,
      selectedFacts: actualSalesSelectedFacts,
    });
    for (const fact of filteredOrders) {
    ...
    const ownDataDays = new Set(orderFacts
      ...
      .filter(Boolean)).size;
    const orderCount = new Set(filteredOrders.map((item) => item.orderHeaderId).filter(Boolean)).size;
    ...
    const dailyTrend = buildDailyTrend({
      ...
      filteredOrders,
      actualSalesScope,
      assortmentDailyAmount: summaryTotals.assortmentAmount,
    });
    if (input.projection === "trend") return { trend: dailyTrend };
    ...
    for (const fact of filteredHistoricalOrders) {
      ...
      const storeKey = mapKey(fact.store, fact.country, fact.platform);
      const store = item.stores.get(storeKey) || {
        store: fact.store,
        country: fact.country,
        platform: fact.platform,
        manager: fact.manager,
      ...
      if (withinRange(fact.paidDay, currentComparisonStart, maxOrderDay)) {
        item.currentQuantity += fact.quantity;
        store.currentQuantity += fact.quantity;
      }
      if (withinRange(fact.paidDay, previousComparisonStart, previousComparisonEnd)) {
        item.previousQuantity += fact.quantity;
        store.previousQuantity += fact.quantity;
      }
      item.stores.set(storeKey, store);
    ...
    const styleSalesTrend = [...styleTrendMap.values()].map((item) => {
      ...
      return {
        ...
        priority: trendPriority(status, changeRate),
        comparisonDays,
        storeImpacts: buildStyleStoreImpacts(stores, comparisonSufficient, comparisonDays),
      };
    });
    ...
    const buildMovementWindow = (days, entity) => {
      ...
      for (const fact of filteredHistoricalOrders) {
        ...
        if (entity === "style") {
          ...
          if (withinRange(fact.paidDay, currentFrom, maxOrderDay)) store.currentQuantity += value;
          if (withinRange(fact.paidDay, previousFrom, previousTo)) store.previousQuantity += value;
          item.stores.set(storeKey, store);
        }
      ...
      const rows = [...movements.values()].map((item) => {
        ...
        return {
          ...
          priority: trendPriority(status, changeRate),
          ...(entity === "style" ? {
            storeImpacts: buildStyleStoreImpacts(stores, sufficient, days),
          } : {}),
        };
      });
      return {
    ...
    for (const item of storeAnomalies.declines) {
      priorityAlerts.push({
        ...
        title: `${item.store} 近${comparisonDays}日标准化估值下滑`,
        ...
        evidence: [`本期标准化估值 ${round(item.currentAmount)} 元`, `上期标准化估值 ${round(item.previousAmount)} 元`],
    ...
    for (const item of productSalesRanking.filter((entry) => entry.trendStatus === "decline").sort(byBusinessImpact)) {
      priorityAlerts.push({
        ...
        title: `${item.productName} 近${comparisonDays}日标准化估值下滑`,
    ...
    const packagePriced = packageRows.filter((row) => {
      const payload = parseJson(row.normalized_payload_json);
      return targetListPrice50Cny(payload) > 0;
    }).length;
    const summaryMetrics = publicTotals(summaryTotals, ownDataDays);
    const estimatedGapAmount = round(Math.max(0, summaryMetrics.assortmentEstimatedAmount
      - summaryMetrics.ownEstimatedAmount));
    const estimatedAverageOrderValue = orderCount > 0
      ? round(summaryMetrics.ownEstimatedAmount / orderCount)
      : 0;

    return {
      contract: {
        version: "SALES-ASSORTMENT-1.5.0",
        amountBasis: "我方标准化估值=订单商品数量×同国家50%目标利润标价；货盘标准化估值=库存预测日销量×同国家50%目标利润标价×订单有效付款日期天数；实际销售额按订单头的订单核算金额（人民币）并以唯一订单ID去重，每单仅计一次",
        amountSemantics: {
          ownAmount: "deprecated compatibility alias of ownEstimatedAmount; not actual sales or GMV",
          assortmentAmount: "deprecated compatibility alias of assortmentEstimatedAmount; not actual sales or GMV",
          actualSalesAmount: "confirmed order-level CNY accounting amount, counted once per canonical order; partial availability returns only the confirmed subtotal",
        },
        actualSalesAmountSourceField: DIRECT_ORDER_AMOUNT_SOURCE,
        actualSalesAmountFallback: DERIVED_ORDER_AMOUNT_SOURCE,
        actualSalesFilterAttribution: "date/store filters are order-safe; product filters only include orders whose full line set is selected, and never allocate a whole-order amount to a partial SKU/category match",
        listPriceProfitTarget: 50,
        listPriceFormula: "50%目标利润标价=销售成本÷(1-50%)",
        ownShareFormula: "我方标准化估值÷货盘标准化估值",
        dailySalesGapFormula: "(货盘标准化估值-我方标准化估值)÷我方数据天数",
      ...
      summary: {
        ...summaryMetrics,
        ...actualSales,
        gapAmount: estimatedGapAmount,
        estimatedGapAmount,
        gapAmountStatus: "estimated",
        orderCount,
        averageOrderValue: estimatedAverageOrderValue,
        estimatedAverageOrderValue,
      ...
      dailyReport: {
        ...
        title: "销售与货盘经营日报",
        summary: {
          ownAmount: summaryMetrics.ownAmount,
          ownEstimatedAmount: summaryMetrics.ownEstimatedAmount,
          ownAmountStatus: summaryMetrics.ownAmountStatus,
          assortmentAmount: summaryMetrics.assortmentAmount,
          assortmentEstimatedAmount: summaryMetrics.assortmentEstimatedAmount,
          assortmentAmountStatus: summaryMetrics.assortmentAmountStatus,
          ...actualSales,
          gapAmount: estimatedGapAmount,
          estimatedGapAmount,
          gapAmountStatus: "estimated",
          ownShare: summaryMetrics.ownShare,
          ownShareStatus: summaryMetrics.ownShareStatus,
          orderCount,
          averageOrderValue: estimatedAverageOrderValue,
          estimatedAverageOrderValue,
          averageOrderValueStatus: "estimated",
