...
export function buildSalesAssortmentAnalysisSnapshot(dashboard) {
  ...
  const styleFacts = (items) => list(items, 15).map((item) => ({
    country: item.country,
    style: item.style,
    categoryL1: item.categoryL1,
    ...
    impactQuantity: number(item.impactQuantity),
    impactScore: number(item.impactScore),
    trendStatus: item.trendStatus,
    priority: item.priority,
    storeImpacts: list(item.storeImpacts, 5).map((store) => ({
      store: store.store,
      manager: store.manager,
      platform: store.platform,
      currentQuantity: number(store.currentQuantity),
      previousQuantity: number(store.previousQuantity),
      changeRate: store.changeRate === null ? null : number(store.changeRate),
      quantityChange: number(store.quantityChange),
      impactQuantity: number(store.impactQuantity),
    })),
  }));
  const styleDeclines = styleFacts(dashboard.styleAnomalies?.declines);
  const styleGrowth = styleFacts(dashboard.styleAnomalies?.growth);
...
function analysisPrompt(snapshot) {
  return [
    ...
    "你只能基于用户提供的结构化看板事实生成结论，不得补造实际销售额、库存、店铺、SKU 或时间数据。",
    "货盘标准化估值和我方标准化估值使用同一目标利润标价，仅用于横向比较，绝不能称为实际销售额或 GMV；actualSalesAmount 在 availability=available 时可作为完整金额引用，在 availability=partial 且有单币种金额时只能明确标为“部分”并同时说明 actualSalesOrderCoverage，availability=unavailable 时不得引用金额。实际销售额按订单头“订单核算金额（人民币）”去重；类目 / SKU 部分命中不计整单且不分摊。",
    ...
    "storeDeclines 与 storeGrowth 必须分开分析店铺下滑和上涨；styleDeclines 与 styleGrowth 必须分开分析款名销量变化，并使用每个款名的 storeImpacts 指出主要影响店铺。",
    "异常排序必须同时考虑绝对影响金额或销量、环比幅度和当前业务规模；不能把小金额的大百分比变化当成首要结论。",
    "businessOpportunities 必须优先分析货盘标准化估值高、机会估值大、库存可支撑且我方承接低的款名，并展开多个具体商品证据。",
    "inventory 必须比较本次与上次库存快照，识别急降和到仓增加，并结合货盘标准化估值、我方标准化估值、日销和可售天数解释；库存下降只能表述为销量或出库候选信号，不能直接断言销量上涨。",
    "dailyReport 必须覆盖经营概览、店铺昨日与近7日变化、款名昨日与近7日变化、商业机会和库存快照变化，汇总最多十项最紧急事项。近7日变化必须使用 dailyMovementWindows 的 stores7d/styles7d：以报告日为结束日的最近7天，环比紧接着之前的7天。",
...
function moduleAnalysisPrompt(snapshot) {
  ...
  return [
    "你是 Commerce Ops 的资深电商运营分析助手。",
    "只依据输入的结构化看板事实，为页面的各个细分模块分别生成简短结论；不得补造实际销售额、库存、店铺、SKU 或时间数据。",
    "货盘标准化估值与我方标准化估值只用于横向比较，绝不能称为实际销售额或 GMV；预测日销量不得称为公司真实销量。actualSalesAmount 在 availability=partial 时必须明确标为部分并说明订单覆盖，availability=unavailable 时不得引用；该金额按订单头“订单核算金额（人民币）”去重，类目 / SKU 部分命中不计整单且不分摊。",
    ...
    "businessOpportunities 必须从机会估值、货盘标准化估值、库存标价估值、我方承接率四项综合分析，至少解释三个款名；数据足够时不得只输出一条泛化建议。",
    "inventory 必须比较本次与上次库存，区分急降、到仓增加、断货和低库存，并写出货盘或我方标准化估值证据。库存急降只能提示核查销量、订单或调拨，不能直接断言销量增长。",
    "dailyReport 必须先概括经营指标，再分别覆盖店铺和款名的昨日变化、近7日变化、机会及库存快照变化，帮助识别人眼不易发现的持续趋势。近7日变化固定为以报告日为结束日的最近7天环比紧接着之前的7天。",
...
export class SalesAssortmentAiService {
  constructor({
    ...
    if (analysisStore && (typeof analysisStore.load !== "function" || typeof analysisStore.save !== "function")) {
      throw new TypeError("Sales assortment analysis store is invalid");
    }
  ...
  async current() {
    if (this.currentAnalysis !== undefined) {
      return this.currentAnalysis ? { ...this.currentAnalysis, cached: true } : null;
    }
    ...
    if (!this.currentLoad) {
      this.currentLoad = this.analysisStore.load()
        .then((value) => {
          this.currentAnalysis = value || null;
          return this.currentAnalysis;
        })
        .finally(() => { this.currentLoad = null; });
    }
    const value = await this.currentLoad;
  ...
  async analyze(input = {}) {
    ...
    if (this.analysisStore) {
      try {
        await this.analysisStore.save(value);
      } catch (cause) {
        const error = new Error("DeepSeek 分析已生成，但保存失败，请稍后重新分析。");
        error.code = "AI_ANALYSIS_PERSIST_FAILED";
        error.status = 500;
        error.cause = cause;
        throw error;
      }
