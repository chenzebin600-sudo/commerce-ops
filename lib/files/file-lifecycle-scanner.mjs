...
const EXCLUDED_DIRECTORIES = new Set([
  ...
  "commerceops-backups", "chrome", "chrome-data", "chrome-user-data", "user data", "quarantine",
  "sales-assortment-ai",
]);
...
export class FileLifecycleScanner {
  constructor({ fileRepository, managedFileRepository = null, roots, policy, protectedFileIds = new Set(), now = () => new Date() }) {
  ...
  async scan(scopes) {
    ...
    const tracked = await this.fileRepository.listAll();
    const managed = await (this.managedFileRepository?.listManagedFiles?.() || []);
    ...
    const physicalByManagedPath = new Map(
      collected.files.map((file) => [`${file.scope}\0${file.relativePath}`, file]),
    );
    ...
    for (const record of managed) {
      if (["quarantined", "deleted"].includes(record.status)) continue;
      let normalized;
      try {
        normalized = normalizeStoredRelativePath(record.relativePath);
      } catch {
        items.push({
          ...baseItem({ classification: "path_invalid", scope: record.rootKey, name: "managed-file", reasonCode: "DATABASE_PATH_INVALID", physicalStatus: "unknown", errorCode: "PATH_INVALID" }),
          fileId: record.id, managedFileId: record.id, sourceType: record.sourceType, databaseStatus: record.status,
          detectedFileType: record.sourceType, reviewStatus: "registered",
        });
        continue;
      }
      const physical = physicalByManagedPath.get(`${record.rootKey}\0${normalized}`);
      if (!physical) {
        items.push({
          ...baseItem({ classification: "physical_missing", scope: record.rootKey, name: path.basename(normalized), reasonCode: "DATABASE_RECORD_WITHOUT_FILE", physicalStatus: "missing" }),
          fileId: record.id, managedFileId: record.id, sourceType: record.sourceType, databaseStatus: record.status,
          detectedFileType: record.sourceType, reviewStatus: "registered", fileSize: record.fileSize,
        });
        continue;
      }
      matched.add(physical);
      if (physical.pathError || !physical.stat || !physical.absolutePath) {
        items.push({
          ...baseItem({ classification: "path_invalid", scope: record.rootKey, name: path.basename(normalized), reasonCode: "PHYSICAL_PATH_INVALID", physicalStatus: "invalid", errorCode: physical.pathError || "PATH_INVALID" }),
          fileId: record.id, managedFileId: record.id, sourceType: record.sourceType, databaseStatus: record.status,
          detectedFileType: record.sourceType, reviewStatus: "registered",
        });
        continue;
      ...
      const item = {
        ...baseItem({ classification, scope: record.rootKey, name: path.basename(normalized), stat: physical.stat, reasonCode }),
        fileId: record.id, managedFileId: record.id, sourceType: record.sourceType, databaseStatus: record.status,
        detectedFileType: record.sourceType, reviewStatus: "registered",
      };
      if (!["active_or_recent", "size_mismatch"].includes(classification)) {
        try {
          item._hash = await hashFileStream(physical.absolutePath, { deadline: collected.deadline });
          item.shortHash = item._hash.slice(0, 12);
          if (item._hash.toLowerCase() !== String(record.fileHash).toLowerCase()) {
            item.classification = "hash_mismatch";
            item.categories = ["hash_mismatch"];
            item.reasonCode = "FILE_HASH_MISMATCH";
            item.suggestQuarantine = true;
      ...
      if (item.classification === "healthy"
        && isExpired(physical.stat, { sourceType: record.sourceType, scope: record.rootKey, relativePath: normalized }, startedAt, this.policy)) {
        addCategory(item, "expired_candidate");
      }
      items.push(item);
    }
