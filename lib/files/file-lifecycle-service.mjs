...
async function auditScan(audit, action, scan, extra = {}) {
...
export class FileLifecycleService {
  ...
  async startScan(requestedScopes = "all") {
    const scopes = safeScopes(requestedScopes);
    if (this.runningPromise) {
      const running = await this.repository.getRunningScan();
      return { scan: running, reused: true };
    }
    const existing = await this.repository.getRunningScan();
    if (existing) return { scan: existing, reused: true };
    const scan = await this.repository.createScan(scopes, this.now());
    await auditScan(this.audit, "file.lifecycle.scan.started", scan, { result: "running" });
    this.runningPromise = new Promise((resolve) => setImmediate(resolve))
      .then(async () => {
        try {
          const report = await this.scanner.scan(scopes);
          const completed = await this.repository.completeScan(scan.id, report, this.now());
          await auditScan(this.audit, "file.lifecycle.scan.completed", completed, { result: report.truncated ? "truncated" : "complete" });
          return completed;
        } catch (error) {
          const failed = await this.repository.failScan(scan.id, error?.code || "LIFECYCLE_SCAN_FAILED", this.now());
          await auditScan(this.audit, "file.lifecycle.scan.failed", failed, {
            status: "failed",
  ...
  async getReport(id, itemFilters = {}) {
    const scan = await this.repository.getScan(id);
    if (!scan) return null;
    return { scan, ...await this.repository.listItems(id, itemFilters) };
  }
  ...
  async exportReport(scanId) {
    ...
    if (scan.reportFileId) {
      const existing = await this.fileService.getFile(scan.reportFileId);
      if (existing?.status === "available") return existing;
    }
    const requestKey = `lifecycle_report:${scan.id}`;
    const existing = await this.fileService.getByRequestKey(requestKey);
    if (existing) {
      await this.repository.attachReport(scan.id, existing.id, this.now());
      return existing;
    }
    ...
    try {
      ...
      const records = (await this.repository.allItems(scan.id)).map(reportRecord);
      ...
      await this.repository.attachReport(scan.id, file.id, this.now());
      return file;
    } catch (error) {
