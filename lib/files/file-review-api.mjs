  ...
  return async function handleFileReviewApi(req, res, url) {
    ...
    try {
      ...
      if (pathname === "/api/files/lifecycle/quarantine-records" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, ...service.listQuarantineRecords({ page: url.searchParams.get("page"), pageSize: url.searchParams.get("page_size") }) });
      }
      ...
      if (action === "protect") {
        const item = service.protectItem(itemId, context);
        return sendJson(res, 200, { ok: true, item: { id: item.id, reviewStatus: item.reviewStatus, reviewedAt: item.reviewedAt } });
      }
      if (action === "reject") {
        const item = service.rejectItem(itemId, context);
        return sendJson(res, 200, { ok: true, item: { id: item.id, reviewStatus: item.reviewStatus, reviewedAt: item.reviewedAt } });
      }
      if (action === "approve-quarantine") {
        const item = service.approveQuarantine(itemId, context);
        return sendJson(res, 200, { ok: true, item: { id: item.id, reviewStatus: item.reviewStatus, reviewedAt: item.reviewedAt } });
      }
      ...
      service.rejectPermanentDeletion(itemId);
      return true;
    } catch (error) {
