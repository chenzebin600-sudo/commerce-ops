...
export class FileLifecycleRepository {
  constructor({ db }) {
    ...
    this.supportsReview = this.provider.hasColumn("file_lifecycle_items", "review_status");
  }
  ...
  completeScan(id, report, now = new Date()) {
    ...
    return this.transactionManager.run(() => {
      ...
      const updateReview = this.supportsReview
        ? this.db.prepare(`UPDATE file_lifecycle_items SET detected_file_type=?,review_status=?,managed_file_id=? WHERE id=?`)
        : null;
      for (const item of report.items) {
        ...
        updateReview?.run(item.detectedFileType || null, item.reviewStatus || "pending_review", item.managedFileId || null, itemId);
      }
