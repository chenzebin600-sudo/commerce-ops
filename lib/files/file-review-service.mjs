...
function reviewAudit(audit, action, item, { status = "success", errorCode = null, result = null, fileId = null } = {}) {
  audit?.recordSafely({
    module: "file",
    action,
...
export class FileReviewService {
  ...
  async ensureQuarantineRoot() {
    ...
    const stat = await fs.lstat(this.quarantineRoot);
    if (!stat.isDirectory() || stat.isSymbolicLink()) throw Object.assign(new Error("Quarantine root is invalid"), { code: "QUARANTINE_ROOT_INVALID" });
  }

  async classifyScan(scanId) {
    const scan = this.lifecycleRepository.getScan(scanId);
    if (!scan || scan.status !== "completed") throw Object.assign(new Error("Lifecycle scan is not ready"), { code: "LIFECYCLE_SCAN_NOT_READY" });
    const result = await classifyAdvertisingScanItems({ scanItems: this.repository.scanItems(scanId), roots: this.roots });
    for (const evidence of result.evidence) this.repository.saveEvidence(evidence.lifecycleItemId, evidence);
    return {
  ...
  async revalidateItem(item) {
    const root = this.rootMap.get(item.rootKey);
    if (!root || !item.relativePath || !item.fileHash) throw Object.assign(new Error("File classification evidence is incomplete"), { code: "CLASSIFICATION_EVIDENCE_MISSING" });
    ...
    const resolved = await resolveExistingFile(root, normalizeStoredRelativePath(item.relativePath), { allowedExtensions: extensions });
  ...
  async registerItem(itemId, { actor = "operator", reason = "confirmed_formal_advertising_file" } = {}) {
    let item = this.repository.getItem(itemId);
    ...
    if (item.reviewStatus === "pending_review") {
      item = this.repository.setReviewStatus(itemId, "approved_for_registration", { actor: safe, reason, now: this.now() });
      reviewAudit(this.audit, "file.review.approved", item, { result: "registration" });
    }
    try {
      await this.revalidateItem(item);
      const id = stableUuid([item.rootKey, item.relativePath, item.jobId || "", item.fileHash].join("\0"));
      const result = this.repository.registerManagedFile(itemId, {
        id,
      ...
      reviewAudit(this.audit, "file.metadata.registered", item, { result: result.created ? "created" : "idempotent", fileId: result.file.id });
      return result.file;
    } catch (error) {
      reviewAudit(this.audit, "file.metadata.registration_failed", item, { status: "failed", errorCode: error?.code || "FILE_REGISTRATION_FAILED", result: "rejected" });
      throw error;
    }
  }

  protectItem(itemId, { actor = "operator", reason = "manual_protection" } = {}) {
    ...
    const updated = this.repository.setReviewStatus(itemId, "protected", { actor: safeActor(actor), reason, now: this.now() });
    reviewAudit(this.audit, "file.protected", updated, { result: "protected" });
    return updated;
  }

  rejectItem(itemId, { actor = "operator", reason = "manual_rejection" } = {}) {
    const item = this.repository.getItem(itemId);
    ...
    const updated = this.repository.setReviewStatus(itemId, "rejected", { actor: safeActor(actor), reason, now: this.now() });
    reviewAudit(this.audit, "file.review.rejected", updated, { result: "rejected" });
    return updated;
  }

  approveQuarantine(itemId, { actor = "operator", reason = "confirmed_test_cleanup_candidate" } = {}) {
    const item = this.repository.getItem(itemId);
    if (!item) throw Object.assign(new Error("Lifecycle item was not found"), { code: "LIFECYCLE_ITEM_NOT_FOUND" });
    ...
    const updated = this.repository.setReviewStatus(itemId, "approved_for_quarantine", { actor: safeActor(actor), reason, now: this.now() });
    reviewAudit(this.audit, "file.review.approved", updated, { result: "quarantine" });
    return updated;
  }
  ...
  async quarantineItem(itemId, { actor = "operator", reason = "confirmed_test_cleanup_candidate" } = {}) {
    ...
    try {
      ...
      const root = this.rootMap.get(item.rootKey);
      if (!root || !item.relativePath || !item.fileHash) throw Object.assign(new Error("File evidence is incomplete"), { code: "CLASSIFICATION_EVIDENCE_MISSING" });
      const extension = path.extname(item.relativePath).toLowerCase() || ".tmp";
      source = await resolveExistingFile(root, item.relativePath, { allowedExtensions: [extension] });
      ...
      const [sourceRootStat, quarantineRootStat] = await Promise.all([fs.stat(root), fs.stat(this.quarantineRoot)]);
      if (sourceRootStat.dev !== quarantineRootStat.dev) throw Object.assign(new Error("Quarantine requires the same filesystem"), { code: "QUARANTINE_CROSS_DEVICE" });
      const recordId = randomUUID();
      const quarantineRelativePath = normalizeStoredRelativePath(`${item.rootKey}/${item.id}/${recordId}${extension}`);
      destination = await resolveNewFile(this.quarantineRoot, quarantineRelativePath, { allowedExtensions: [extension] });
      await fs.rename(source.path, destination.path);
      const afterHash = await hashFileStream(destination.path);
      if (afterHash !== beforeHash) {
        await fs.rename(destination.path, source.path);
        destination = null;
        throw Object.assign(new Error("Quarantine hash verification failed"), { code: "QUARANTINE_HASH_MISMATCH" });
      }
      let record;
      try {
        record = this.repository.recordQuarantine(itemId, {
          id: recordId,
          rootKey: item.rootKey,
      ...
      reviewAudit(this.audit, "file.quarantine", item, { result: "quarantined" });
      return publicRecord(record);
    } catch (error) {
      reviewAudit(this.audit, "file.quarantine.failed", item, { status: "failed", errorCode: error?.code || "FILE_QUARANTINE_FAILED", result: "rejected" });
      throw error;
    }
  }
  ...
  async restoreItem(itemId, { actor = "operator" } = {}) {
    ...
    try {
      ...
      const record = this.repository.getActiveQuarantineByItem(itemId);
      if (!record) throw Object.assign(new Error("Quarantine record was not found"), { code: "QUARANTINE_RECORD_INVALID" });
      const root = this.rootMap.get(record.rootKey);
      if (!root) throw Object.assign(new Error("Managed root is unavailable"), { code: "MANAGED_ROOT_INVALID" });
      const extension = path.extname(record.originalRelativePath).toLowerCase() || ".tmp";
      source = await resolveExistingFile(this.quarantineRoot, record.quarantineRelativePath, { allowedExtensions: [extension] });
      ...
      destination = await resolveNewFile(root, record.originalRelativePath, { allowedExtensions: [extension] });
      const [sourceRootStat, destinationRootStat] = await Promise.all([fs.stat(this.quarantineRoot), fs.stat(root)]);
      if (sourceRootStat.dev !== destinationRootStat.dev) throw Object.assign(new Error("Restore requires the same filesystem"), { code: "QUARANTINE_CROSS_DEVICE" });
      await fs.rename(source.path, destination.path);
      ...
      let updated;
      try {
        updated = this.repository.recordRestore(record.id, { actor: safe, now: this.now() });
      } catch (error) {
        await fs.rename(destination.path, source.path);
        destination = null;
        throw error;
      }
      reviewAudit(this.audit, "file.restore", item, { result: "restored" });
      return publicRecord(updated);
    } catch (error) {
      reviewAudit(this.audit, "file.restore.failed", item, { status: "failed", errorCode: error?.code || "FILE_RESTORE_FAILED", result: "rejected" });
      throw error;
    }
  }

  listQuarantineRecords(filters) {
    const result = this.repository.listQuarantineRecords(filters);
    return { ...result, records: result.records.map(publicRecord) };
  }

  rejectPermanentDeletion(itemId) {
    const item = this.repository.getItem(itemId);
    reviewAudit(this.audit, "file.delete.rejected", item, { status: "failed", errorCode: "FILE_DELETION_DISABLED", result: "disabled" });
