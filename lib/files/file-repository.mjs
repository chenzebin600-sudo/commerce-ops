...
export function serializeExportFile(row) {
  ...
  return {
    ...
    sourceType: Object.entries(VIRTUAL_SOURCE_TYPES).find(([, marker]) => metadata.generatedBy === marker)?.[0] || row.source_type,
...
export class ExportFileRepository {
  ...
  create(input) {
    ...
    const metadata = generatedBy
      ? { ...(input.metadata || {}), generatedBy }
      : input.metadata;
  ...
  list({ sourceType, scope, taskId, runId, status, createdFrom, createdTo, page = 1, pageSize = 20 } = {}) {
    const clauses = [];
    const params = [];
    if (scope === "mabang") {
      clauses.push("source_type LIKE 'mabang_%' AND metadata_json NOT LIKE ?");
      params.push('%\"generatedBy\":\"product_package_import\"%');
    }
    if (VIRTUAL_SOURCE_TYPES[sourceType]) {
      clauses.push("source_type='mabang_manual_order' AND metadata_json LIKE ?");
      params.push(`%\"generatedBy\":\"${VIRTUAL_SOURCE_TYPES[sourceType]}\"%`);
    } else if (sourceType) {
