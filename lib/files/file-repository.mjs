...
export function serializeExportFile(row) {
  ...
  return {
    ...
    sourceType: metadata.generatedBy === "lifecycle_scanner" ? "system_file_lifecycle_report" : row.source_type,
...
export class ExportFileRepository {
  constructor({ db }) {
    this.provider = resolveSqliteProvider(db);
  ...
  create(input) {
    ...
    const storedSourceType = sourceType === "system_file_lifecycle_report" ? "mabang_manual_order" : sourceType;
    const metadata = sourceType === "system_file_lifecycle_report"
      ? { ...(input.metadata || {}), generatedBy: "lifecycle_scanner" }
      : input.metadata;
  ...
  listAll() {
    return this.db.prepare("SELECT * FROM export_files ORDER BY created_at,id").all().map(serializeExportFile);
  }
}

export function createExportFileRepository(db) {
  return new ExportFileRepository({ db });
}
