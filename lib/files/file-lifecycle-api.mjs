  ...
  return async function handleFileLifecycleApi(req, res, url) {
    ...
    try {
      if (pathname === "/api/files/lifecycle/scan" && req.method === "POST") {
        ...
        const result = await service.startScan(body.scopes ?? "all");
      ...
      if (pathname === "/api/files/lifecycle/reports" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, ...await service.listReports({ page: url.searchParams.get("page"), pageSize: url.searchParams.get("page_size") }) });
      }
      if (pathname === "/api/files/lifecycle/summary" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, scan: await service.summary() });
      }
      ...
      if (report && req.method === "GET") {
        ...
        const result = await service.getReport(scanId, itemFilters(url.searchParams));
