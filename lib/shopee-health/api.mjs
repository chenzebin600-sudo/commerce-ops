  ...
  return async function handleShopeeHealthApi(req, res, url) {
    ...
    try {
      ...
      if (pathname === "/api/shopee-health/dashboard" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, data: await service.dashboard({ days: url.searchParams.get("days") }) });
      }
      if (pathname === "/api/shopee-health/settings") {
        if (req.method === "GET") return sendJson(res, 200, { ok: true, data: await repository.getSettings() });
        if (req.method === "PUT") {
      ...
      if (pathname === "/api/shopee-health/thresholds") {
        if (req.method === "GET") return sendJson(res, 200, { ok: true, data: await repository.listThresholds() });
        if (req.method === "PUT") {
          const body = await readJson(req);
          if (!Array.isArray(body.items)) throw new Error("阈值配置必须是数组。");
          return sendJson(res, 200, { ok: true, data: await repository.saveThresholds(body.items, body.actorName || "当前用户") });
        }
        return sendJson(res, 405, { ok: false, error: "Method not allowed" });
      }
      if (pathname === "/api/shopee-health/collect" && req.method === "POST") {
        const result = await service.startCollection("manual");
      ...
      if (pathname === "/api/shopee-health/runs" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, data: await repository.listRuns(url.searchParams.get("limit")) });
      }
      if (pathname === "/api/shopee-health/issues" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, data: await repository.listIssues({
      ...
      if (pathname === "/api/shopee-health/appeals") {
        if (req.method === "GET") return sendJson(res, 200, { ok: true, data: await repository.listAppeals({ status: url.searchParams.get("status") || "", assigneeUserId: url.searchParams.get("assignee") || "" }) });
        if (req.method === "POST") {
          const body = await readJson(req);
          const appeal = await repository.createAppeal({ ...body, createdBy: body.createdBy || "current-user", assigneeName: body.assigneeName || "我", assigneeUserId: body.assigneeUserId || "current-user" });
      ...
      if (match) {
        ...
        if (req.method === "GET") {
          const appeal = await repository.getAppeal(id);
          if (!appeal) return sendJson(res, 404, { ok: false, error: "申诉工单不存在。" });
          return sendJson(res, 200, { ok: true, data: { ...appeal, events: await repository.listAppealEvents(id) } });
        }
        if (req.method === "PUT") {
          const body = await readJson(req);
          return sendJson(res, 200, { ok: true, data: await repository.updateAppeal(id, body, { userId: body.actorUserId || "current-user", name: body.actorName || "我" }) });
        }
        return sendJson(res, 405, { ok: false, error: "Method not allowed" });
      }
      if (pathname === "/api/shopee-health/notifications" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, data: { items: await repository.listNotifications(url.searchParams.get("limit")), unread: await repository.unreadNotificationCount() } });
      }
      if (pathname === "/api/shopee-health/notifications/read-all" && req.method === "POST") {
        return sendJson(res, 200, { ok: true, data: { updated: await repository.markNotificationsRead() } });
      }
      match = matchId(pathname, "/api/shopee-health/notifications", "read");
      if (match && req.method === "POST") return sendJson(res, 200, { ok: true, data: { updated: await repository.markNotificationsRead(decodeURIComponent(match[1])) } });
