...
export class ShopeeHealthService {
  ...
  async saveSettings(input, actor = "current-user") {
    ...
    const dingtalkConfigId = input.dingtalkNotificationsEnabled && input.dingtalkConfigId
      ...
      : null;
    if (dingtalkConfigId && !await this.robotRepository?.getDingtalkConfig(dingtalkConfigId)) throw new Error("选择的钉钉机器人不存在。");
    const update = {
  ...
  async testKey() {
    const settings = await this.repository.getSettings({ includeSecret: true });
    ...
    try {
      ...
      await this.repository.saveSettings({ tokenVerifiedAt: this.now().toISOString(), tokenShopCount: count, lastKeyError: null }, this.now());
      return { ok: true, recognizedShopCount: count, monitoredShopCount: this.shops.length };
    } catch (error) {
      await this.repository.saveSettings({ lastKeyError: error.message }, this.now());
      throw error;
    }
  }

  async dashboard({ days = 30 } = {}) {
    const [snapshots, issues, appeals, settings, trend, latestRun, notifications, unreadNotifications, thresholds] = await Promise.all([
      this.repository.latestSnapshots(), this.repository.listIssues({ status: "active", limit: 500 }),
      this.repository.listAppeals({ limit: 200 }), this.repository.getSettings(), this.repository.trend(days),
      this.repository.latestRun(), this.repository.listNotifications(20), this.repository.unreadNotificationCount(),
    ...
    return {
      generatedAt: this.now().toISOString(), settings, monitoredShopCount: this.shops.length,
      ...
      issues, appeals, trend, latestRun, notifications, unreadNotifications, thresholds,
    };
  }

  async startCollection(triggerType = "manual", scheduledFor = null) {
    if (this.activePromise) return { started: false, run: await this.repository.activeRun() };
    const existing = await this.repository.activeRun();
    if (existing) return { started: false, run: existing };
    const run = await this.repository.createRun({ triggerType, scheduledFor, shopTotal: this.shops.length }, this.now());
    this.activePromise = this.executeCollection(run.id).catch((error) => {
      return this.repository.updateRun(run.id, { status: "failed", errorMessage: error.message, finishedAt: this.now().toISOString() }, this.now())
        .then(async () => { if ((await this.repository.getSettings())?.siteNotificationsEnabled) await this.repository.createNotification({ notificationType: "collection_failed", severity: "critical", title: "店铺健康采集失败", message: error.message }, this.now()); });
    }).finally(() => { this.activePromise = null; });
    return { started: true, run };
  }

  async dueForSchedule(date = this.now()) {
    const settings = await this.repository.getSettings();
    if (!settings?.enabled || !settings.tokenConfigured) return false;
    const dateKey = localDate(date, settings.timezone);
    return localTime(date, settings.timezone) >= settings.scheduleTime && !await this.repository.hasRunForDate(dateKey);
  }

  async runScheduledIfDue(date = this.now()) {
    if (!await this.dueForSchedule(date)) return { started: false, reason: "not_due" };
    const settings = await this.repository.getSettings();
    return this.startCollection("scheduled", `${localDate(date)}T${settings.scheduleTime}:00+08:00`);
  }

  async executeCollection(runId) {
    const settings = await this.repository.getSettings({ includeSecret: true });
    if (!settings?.encryptedTokenKey) {
      await this.repository.updateRun(runId, { status: "failed", errorMessage: "未配置 Shopee 专属 Key。", finishedAt: this.now().toISOString() }, this.now());
      return;
    }
    await this.repository.updateRun(runId, { status: "running", startedAt: this.now().toISOString() }, this.now());
    const tokenKey = decryptSecret(settings.encryptedTokenKey);
    try {
      await this.client.verifyToken(tokenKey);
    } catch (error) {
      await this.repository.saveSettings({ lastKeyError: error.message }, this.now());
      if (settings.siteNotificationsEnabled) await this.repository.createNotification({ notificationType: "key_invalid", severity: "critical", title: "Shopee 专属 Key 验证失败", message: `${error.message} 请进入店铺健康设置页更新 Key。` }, this.now());
      await this.repository.updateRun(runId, { status: "failed", errorMessage: error.message, finishedAt: this.now().toISOString() }, this.now());
      await this.sendSummary(settings, { critical: 1, warning: 0, failed: this.shops.length }, error.message).catch(() => undefined);
      return;
    }
    const thresholds = new Map((await this.repository.listThresholds()).map((item) => [item.metricId, item]));
    ...
    if (settings.siteNotificationsEnabled) for (const item of results.filter((entry) => !entry?.ok)) await this.repository.createNotification({
    ...
    await this.sendSummary(settings, { critical, warning, failed }, errorMessage).catch((error) => {
      if (settings.siteNotificationsEnabled) return this.repository.createNotification({ notificationType: "dingtalk_failed", severity: "warning", title: "钉钉通知发送失败", message: error.message }, this.now());
    });
  }
  ...
  async collectShop({ shop, runId, tokenKey, settings, thresholds }) {
    ...
    const record = async (issue) => {
      ...
      const saved = await this.repository.upsertIssue({ ...issue, shopId: shop.shopId, shopCode: shop.code, shopName: shop.name, country: shop.country }, this.now());
      collectedIssues.push(saved.issue);
      if (saved.isNew && settings.siteNotificationsEnabled) await this.repository.createNotification({
    ...
    await this.repository.resolveMissingIssues(shop.shopId, seen, this.now());
  ...
  async sendSummary(settings, counts, errorMessage = null) {
    ...
    const robot = await this.robotRepository.getDingtalkConfig(settings.dingtalkConfigId, { includeSecret: true });
