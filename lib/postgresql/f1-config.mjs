...
export function loadPostgresqlF1Config({ rootDir = process.cwd(), env = process.env, envFile } = {}) {
  ...
  for (const [key, value] of Object.entries(env || {})) {
    if (key.startsWith("POSTGRES_") && String(value ?? "").trim()) values[key] = value;
  }
  ...
  const sslCaFileRaw = String(values.POSTGRES_SSL_CA_FILE || "").trim();
  const sslCaFile = sslCaFileRaw ? path.resolve(rootDir, sslCaFileRaw) : null;
  if (ssl === "true" && !sslCaFile) {
    throw new Error("POSTGRES_SSL_CA_FILE is required when POSTGRES_SSL=true");
  }
  if (sslCaFile && !fs.existsSync(sslCaFile)) {
    throw new Error("POSTGRES_SSL_CA_FILE does not exist");
  }
