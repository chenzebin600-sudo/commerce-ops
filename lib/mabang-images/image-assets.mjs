...
export class MabangImageAssetService {
  constructor({ repository, tempRoot, imageRoot, maxBytes = 10 * 1024 * 1024 }) {
    ...
    this.inflightBySha256 = new Map();
    this.verifiedAssetIds = new Set();
  }
  ...
  async store({
    ...
    if (existing?.status === "available" && await this.assetFileAvailable(existing, inspected)) {
      return { asset: existing, duplicate: true };
    }

    const inflight = this.inflightBySha256.get(inspected.sha256);
    if (inflight) {
      const completed = await inflight;
      return { asset: completed.asset, duplicate: true };
    }
    const task = this.storeNew({ buffer, sourceUrl, sourceSystem, uploadedFilename, inspected });
    this.inflightBySha256.set(inspected.sha256, task);
    try {
      return await task;
    } finally {
      if (this.inflightBySha256.get(inspected.sha256) === task) this.inflightBySha256.delete(inspected.sha256);
    }
  }

  async assetFileAvailable(asset, inspected) {
    if (this.verifiedAssetIds.has(asset.id)) return true;
    try {
      const resolved = await resolveExistingFile(this.imageRoot, asset.relativePath, {
        allowedExtensions: Object.values(MIME_EXTENSIONS),
      });
      if (Number(resolved.stat.size) !== inspected.fileSize) {
        throw new ImageValidationError("IMAGE_ASSET_INTEGRITY_FAILED", "已存储图片大小与素材记录不一致。", { status: 409 });
      }
      const sha256 = createHash("sha256").update(await fs.readFile(resolved.path)).digest("hex");
      if (sha256 !== inspected.sha256) {
        throw new ImageValidationError("IMAGE_ASSET_INTEGRITY_FAILED", "已存储图片哈希与素材记录不一致。", { status: 409 });
      }
      this.verifiedAssetIds.add(asset.id);
      return true;
    } catch (error) {
      if (["FILE_NOT_FOUND", "FILE_MISSING"].includes(error?.code)) return false;
      throw error;
    }
  }

  async reuseExisting(asset) {
    if (!asset || asset.status !== "available") return null;
    const available = await this.assetFileAvailable(asset, {
      fileSize: Number(asset.fileSize),
      sha256: asset.sha256,
    });
    return available ? { asset, duplicate: true } : null;
  }

  async storeNew({ buffer, sourceUrl, sourceSystem, uploadedFilename, inspected }) {
    ...
    try {
      ...
      try {
        const asset = await this.repository.createAsset({
          id: randomUUID(),
          sourceSystem,
          sourceUrl: sourceUrl ? sanitizeStoredSourceUrl(sourceUrl) : null,
          storageFileId: randomUUID(),
          originalFilename: uploadedFilename
            ? sanitizeFilename(uploadedFilename, { fallback: `mabang-image${inspected.extension}` })
            : originalFilename(sourceUrl, inspected.extension),
          storageFilename,
