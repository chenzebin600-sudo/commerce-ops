...
export class MabangSkuImageCollectorService {
  ...
  async resume(batchId, actor) {
    ...
    await this.recordAudit({
      action: "mabang_images.resumed", result: "success", actor, metadata: { batchId, mode: batch.mode },
    });
    this.launch(batchId);
  ...
  async run(batchId) {
    ...
    try {
      ...
      await this.recordAudit({
        ...
        metadata: { batchId, accountId: batch?.accountId, mode: batch?.mode, errorCode: code },
      });
      throw error;
    } finally {
