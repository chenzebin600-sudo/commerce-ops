import { inventoryPageHash } from "./extraction.mjs";
...
export class MabangSkuImageCollectorService {
  constructor({ repository, assetService, browserFactory, accountRepository = null, audit = null,
    wait = delay, concurrency = 4, retryAttempts = 4, maxPages = 10000 }) {
  ...
  async start({ accountId, mode, createdBy, sourceBatchId = null }) {
    ...
    const batch = await this.repository.createBatch({ accountId, mode, createdBy: createdBy || "local_session", sourceBatchId });
    await this.recordAudit("mabang_images.collect_started", createdBy, {
  ...
  async pause(batchId, actor) {
    ...
    await this.recordAudit("mabang_images.pause_requested", actor, { batchId });
    return batch;
  }
  ...
  async resume(batchId, actor) {
    ...
    await this.recordAudit("mabang_images.resumed", actor, { batchId, mode: batch.mode });
    this.launch(batchId);
  ...
  async run(batchId) {
    ...
    try {
      if (batch.mode === "retry_failed") {
        ...
        const retryRows = failed.map((row, index) => ({ ...row, sourceKind: "retry", sourcePage: 1, sourceRowNumber: index + 1 }));
        ...
        await this.repository.upsertCheckpoint({ batchId, pageNumber: 1, pageHash: inventoryPageHash(retryRows),
          rowCount: retryRows.length, discoveredCount: retryRows.length, failedCount: 0, status: "completed", completedAt: new Date().toISOString() });
      } else {
        browser = await this.browserFactory({ accountId: batch.accountId, batchId });
        const opened = await browser.open();
        await this.repository.updateBatch(batchId, { interfaceProfile: opened.interfaceProfile, totalPages: opened.totalPages });
        ...
        while (pageNumber <= this.maxPages) {
          ...
          if (batch.status === "pause_requested") {
            ...
            await this.recordAudit("mabang_images.paused", batch.createdBy, { batchId, pageNumber });
          ...
          await this.repository.upsertCheckpoint({ batchId, pageNumber, pageHash: hash, rowCount: rows.length,
            discoveredCount: new Set(rows.map((row) => row.sourceSkuNormalized)).size, failedCount,
      ...
      await this.recordAudit("mabang_images.collect_completed", completed.createdBy, {
      ...
      await this.repository.recomputeBatchCounters(batchId).catch(() => {});
      ...
      await this.recordAudit("mabang_images.collect_failed", batch?.createdBy, { batchId, accountId: batch?.accountId, mode: batch?.mode, errorCode: code });
      throw error;
    } finally {
  ...
  async downloadOne(batch, row, browser) {
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt += 1) {
      ...
      try {
        ...
        await this.recordAudit("mabang_images.linked", batch.createdBy, { batchId: batch.id, assetId: stored.asset.id,
        ...
        const retryable = RETRYABLE_HTTP.has(Number(lastError.httpStatus)) || /TIMEOUT|ECONNRESET|EAI_AGAIN/.test(String(error?.code || ""));
  ...
  async confirmPrimary(linkId, actor) {
    ...
    await this.recordAudit("mabang_images.primary_confirmed", actor, { linkId, assetId: link.assetId, productId: link.productId });
    return link;
  }

  async recordAudit(action, actor, metadata) {
    if (!this.audit) return;
    await this.audit({ action, actor: actor || "local_session", metadata }).catch(() => {});
  }
}
