...
import {
  MABANG_NON_PRODUCT_IMAGE_FILENAMES,
...
function serializeBatch(row) {
  ...
  return {
    ...
    segmentNo: row.segment_no === null || row.segment_no === undefined ? null : Number(row.segment_no),
    startPage: row.start_page === null || row.start_page === undefined ? null : Number(row.start_page),
    endPage: row.end_page === null || row.end_page === undefined ? null : Number(row.end_page),
    mode: row.mode,
    status: row.status,
...
function serializeSyncRun(row) {
  ...
  return {
    ...
    discoveredSkus: Number(row.discovered_skus || 0),
    discoveredImages: Number(row.discovered_images || 0),
    downloadedImages: Number(row.downloaded_images || 0),
    duplicateImages: Number(row.duplicate_images || 0),
    failedImages: Number(row.failed_images || 0),
    matchedSkus: Number(row.matched_skus || 0),
    unmatchedSkus: Number(row.unmatched_skus || 0),
...
function serializeDiscoveryImage(row) {
  if (!row) return null;
  return {
    id: row.id,
    discoveryId: row.discovery_id,
    sourceSku: row.source_sku || null,
    normalizedSku: row.source_sku_normalized || null,
    productName: row.product_name || null,
    warehouseName: row.warehouse_name || null,
...
const BATCH_FIELDS = Object.freeze({
  ...
  lastErrorCode: "last_error_code", lastErrorMessage: "last_error_message", endPage: "end_page",
});

const SYNC_RUN_FIELDS = Object.freeze({
  status: "status", nextPage: "next_page", totalPages: "total_pages", segmentCount: "segment_count",
  lastBatchId: "last_batch_id", lastErrorCode: "last_error_code", lastErrorMessage: "last_error_message",
...
export class MabangImageRepository {
  ...
  async createSyncRun({ accountId, createdBy }) {
    const id = randomUUID();
    const now = iso();
    const p = this.placeholders(this.provider, 8);
    await this.provider.execute(`INSERT INTO ${this.table("mabang_sku_image_sync_runs")} (
      id,account_id,status,next_page,created_by,created_at,updated_at,segment_count
    ) VALUES (${p.join(",")})`, [id, accountId, "pending", 1, createdBy || "local_session", now, now, 0]);
    return this.getSyncRun(id);
  }

  async getSyncRun(id) {
    const row = (await this.provider.query(`SELECT * FROM ${this.table("mabang_sku_image_sync_runs")}
  ...
  async listSyncRuns({ limit = 50 } = {}) {
    ...
    const result = await this.provider.query(`SELECT * FROM ${this.table("mabang_sku_image_sync_runs")}
      ORDER BY created_at DESC LIMIT ${capped}`);
    return result.rows.map(serializeSyncRun);
  }

  async updateSyncRun(id, changes = {}) {
    const entries = Object.entries(changes).filter(([key]) => SYNC_RUN_FIELDS[key]);
    if (!entries.length) return this.getSyncRun(id);
    const values = entries.map(([, value]) => value);
    values.push(iso(), id);
    const setters = entries.map(([key], index) => `${SYNC_RUN_FIELDS[key]}=${this.provider.placeholder(index + 1)}`);
    setters.push(`updated_at=${this.provider.placeholder(entries.length + 1)}`);
    await this.provider.execute(`UPDATE ${this.table("mabang_sku_image_sync_runs")} SET ${setters.join(",")}
      WHERE id=${this.provider.placeholder(entries.length + 2)}`, values);
    return this.getSyncRun(id);
  }

  async createBatch({ accountId, mode, createdBy, sourceBatchId = null,
    syncRunId = null, segmentNo = null, startPage = null }) {
    const id = randomUUID();
    const now = iso();
  ...
  async saveDiscoveries(batchId, rows) {
    ...
    await this.provider.transaction(async (tx) => {
      for (const row of rows) {
        ...
        const discoveryId = exists?.id || randomUUID();
        if (exists) {
          ...
          ) VALUES (${p.join(",")})`, [discoveryId, batchId, row.sourceSku, normalized, row.productName, row.warehouseName,
        ...
        const candidates = (row.imageCandidates?.length
          ? row.imageCandidates
          : row.sourceImageUrl ? [{ url: row.sourceImageUrl, sourceKind: row.sourceKind || "interface" }] : [])
          .filter((candidate) => candidate?.url);
        for (let index = 0; index < candidates.length; index += 1) {
          const candidate = candidates[index];
          const urlHash = createHash("sha256").update(candidate.url).digest("hex");
          const image = (await tx.query(`SELECT id FROM ${this.table("mabang_sku_image_discovery_images")}
            WHERE discovery_id=${tx.placeholder(1)} AND source_url_hash=${tx.placeholder(2)}`,
          [discoveryId, urlHash])).rows[0];
          if (image) {
            const q = this.placeholders(tx, 6);
            await tx.execute(`UPDATE ${this.table("mabang_sku_image_discovery_images")} SET
              image_index=${q[0]},source_url=${q[1]},source_kind=${q[2]},last_checked_at=${q[3]}
              WHERE id=${q[4]} AND discovery_id=${q[5]}`,
            [index, candidate.url, candidate.sourceKind || "interface", now, image.id, discoveryId]);
          } else {
            const q = this.placeholders(tx, 12);
            await tx.execute(`INSERT INTO ${this.table("mabang_sku_image_discovery_images")} (
              id,discovery_id,image_index,source_url,source_url_hash,source_kind,download_status,asset_id,
              download_attempts,discovered_at,last_checked_at,error_code
            ) VALUES (${q.join(",")})`, [randomUUID(), discoveryId, index, candidate.url, urlHash,
              candidate.sourceKind || "interface", "pending", null, 0, now, now, null]);
          }
        }
      }
    });
  ...
  async listDiscoveries(batchId, { status = "", limit = 500, offset = 0 } = {}) {
    ...
    return this.withDiscoveryImages(result.rows.map(serializeDiscovery));
  }
  ...
  async withDiscoveryImages(discoveries) {
    if (!discoveries.length) return discoveries;
    const placeholders = this.placeholders(this.provider, discoveries.length);
    const result = await this.provider.query(`SELECT * FROM ${this.table("mabang_sku_image_discovery_images")}
      WHERE discovery_id IN (${placeholders.join(",")}) ORDER BY discovery_id,image_index`,
    discoveries.map((row) => row.id));
    const grouped = new Map();
    for (const row of result.rows) {
      if (!grouped.has(row.discovery_id)) grouped.set(row.discovery_id, []);
      grouped.get(row.discovery_id).push(serializeDiscoveryImage(row));
    }
    return discoveries.map((row) => ({ ...row, images: grouped.get(row.id) || [] }));
  }

  async pendingImageDownloads(batchId) {
    const result = await this.provider.query(`SELECT i.*,d.source_sku,d.source_sku_normalized,d.product_name,
      d.warehouse_name,d.source_page,d.source_row_number,d.quality_issue_code
      FROM ${this.table("mabang_sku_image_discovery_images")} i
      JOIN ${this.table("mabang_sku_image_discoveries")} d ON d.id=i.discovery_id
      WHERE d.batch_id=${this.provider.placeholder(1)} AND i.download_status IN ('pending','failed')
      ORDER BY d.source_page,d.source_row_number,i.image_index`, [batchId]);
    return result.rows.map(serializeDiscoveryImage);
  }

  async failedImageDownloads(batchId) {
    const result = await this.provider.query(`SELECT i.*,d.source_sku,d.source_sku_normalized,d.product_name,
      d.warehouse_name,d.source_page,d.source_row_number,d.quality_issue_code
      FROM ${this.table("mabang_sku_image_discovery_images")} i
      JOIN ${this.table("mabang_sku_image_discoveries")} d ON d.id=i.discovery_id
      WHERE d.batch_id=${this.provider.placeholder(1)} AND i.download_status='failed'
      ORDER BY d.source_page,d.source_row_number,i.image_index`, [batchId]);
    return result.rows.map(serializeDiscoveryImage);
  }

  async updateDiscoveryImage(id, changes = {}) {
    const map = {
      downloadStatus: "download_status", assetId: "asset_id", downloadAttempts: "download_attempts",
      httpStatus: "http_status", errorCode: "error_code", errorMessage: "error_message",
    };
    const entries = Object.entries(changes).filter(([key, value]) => map[key] && value !== undefined);
    if (!entries.length) return;
    const values = entries.map(([, value]) => value);
    values.push(iso(), id);
    const setters = entries.map(([key], index) => `${map[key]}=${this.provider.placeholder(index + 1)}`);
    setters.push(`last_checked_at=${this.provider.placeholder(entries.length + 1)}`);
    await this.provider.execute(`UPDATE ${this.table("mabang_sku_image_discovery_images")} SET ${setters.join(",")}
      WHERE id=${this.provider.placeholder(entries.length + 2)}`, values);
  }

  async refreshDiscoveryImageSummary(discoveryId) {
    const p = this.provider.placeholder(1);
    const rows = (await this.provider.query(`SELECT download_status,asset_id,download_attempts,http_status,error_code,error_message
      FROM ${this.table("mabang_sku_image_discovery_images")} WHERE discovery_id=${p}
      ORDER BY image_index`, [discoveryId])).rows;
    if (!rows.length) return;
    const status = rows.some((row) => row.download_status === "downloaded") ? "downloaded"
      : rows.some((row) => row.download_status === "duplicate") ? "duplicate"
        : rows.some((row) => row.download_status === "pending") ? "pending"
          : rows.some((row) => row.download_status === "failed") ? "failed"
            : rows.every((row) => row.download_status === "skipped") ? "skipped" : "missing";
    const firstAsset = rows.find((row) => row.asset_id) || null;
    const firstError = rows.find((row) => row.error_code) || null;
    await this.updateDiscovery(discoveryId, {
      downloadStatus: status,
      assetId: firstAsset?.asset_id || null,
      downloadAttempts: Math.max(...rows.map((row) => Number(row.download_attempts || 0))),
      httpStatus: firstError?.http_status || firstAsset?.http_status || null,
      validationStatus: ["downloaded", "duplicate"].includes(status) ? "valid" : status === "failed" ? "invalid" : undefined,
      errorCode: firstError?.error_code || null,
      errorMessage: firstError?.error_message || null,
    });
  }
  ...
  async syncRunSkuKeys(syncRunId) {
    const result = await this.provider.query(`SELECT DISTINCT d.source_sku_normalized
      FROM ${this.table("mabang_sku_image_discoveries")} d
      JOIN ${this.table("mabang_sku_image_batches")} b ON b.id=d.batch_id
      WHERE b.sync_run_id=${this.provider.placeholder(1)}
      ORDER BY d.source_sku_normalized`, [syncRunId]);
    return result.rows.map((row) => normalizeSku(row.source_sku_normalized)).filter(Boolean);
  }

  async listSyncRunBatches(syncRunId) {
    const result = await this.provider.query(`SELECT * FROM ${this.table("mabang_sku_image_batches")}
      WHERE sync_run_id=${this.provider.placeholder(1)} ORDER BY segment_no`, [syncRunId]);
  ...
  async findAvailableAssetsBySourceUrlHashes(sourceUrlHashes) {
    const hashes = [...new Set((sourceUrlHashes || [])
      .map((value) => String(value || "").trim().toLowerCase())
      .filter((value) => /^[a-f0-9]{64}$/.test(value)))];
    const assets = new Map();
    for (let offset = 0; offset < hashes.length; offset += 500) {
      const chunk = hashes.slice(offset, offset + 500);
      const placeholders = this.placeholders(this.provider, chunk.length);
      const result = await this.provider.query(`SELECT i.source_url_hash,a.*
        FROM ${this.table("mabang_sku_image_discovery_images")} i
        JOIN ${this.table("product_media_assets")} a ON a.id=i.asset_id
        WHERE i.source_url_hash IN (${placeholders.join(",")}) AND a.status='available'
        ORDER BY i.last_checked_at DESC`, chunk);
      for (const row of result.rows) {
        if (!assets.has(row.source_url_hash)) assets.set(row.source_url_hash, serializeAsset(row));
      }
    }
    return assets;
  }
  ...
  async productForId(productId) {
    const row = (await this.provider.query(`SELECT id,source_sku,sku_code_normalized,country_raw,source_product_name
      FROM ${this.table("product_skus")} WHERE id=${this.provider.placeholder(1)}
      AND archived_at IS NULL AND deleted_at IS NULL`, [productId])).rows[0];
    if (!row) return null;
    return {
      id: row.id,
      sourceSku: row.source_sku,
      normalizedSku: row.sku_code_normalized,
      countryCode: row.country_raw || "",
  ...
  async productHasAnyImage(productId) {
    const row = (await this.provider.query(`SELECT
      ...
      + (SELECT count(*) FROM ${this.table("product_media_links")} l JOIN ${this.table("product_media_assets")} a ON a.id=l.asset_id
        WHERE l.product_id=${this.provider.placeholder(2)} AND l.mapping_status IN ('suggested','confirmed')
          AND a.status='available' AND ${productImageAssetPredicate("a")}) total`,
  ...
  async linkAssetToMatchingProducts({ assetId, sourceSku, linkedBy }) {
    ...
    if (!asset || isKnownMabangNonProductImage({
    ...
    for (const product of products) {
      ...
      ON CONFLICT(asset_id,product_id) DO NOTHING`, [id, assetId, sourceSku, normalizeSku(sourceSku), product.id,
        product.countryCode, role, "suggested", now, linkedBy, null]);
      const linked = (await this.provider.query(`SELECT l.*,s.source_product_name
        FROM ${this.table("product_media_links")} l
        JOIN ${this.table("product_skus")} s ON s.id=l.product_id
        WHERE l.asset_id=${this.provider.placeholder(1)} AND l.product_id=${this.provider.placeholder(2)}`,
      [assetId, product.id])).rows[0];
      if (linked) links.push(serializeLink(linked));
    }
    return links;
  }

  async linkAssetToProduct({ assetId, productId, linkedBy }) {
    const [asset, product] = await Promise.all([
      this.getAsset(assetId),
      this.productForId(productId),
    ]);
    if (!asset || asset.status !== "available" || !product) return null;
    const now = iso();
    const found = (await this.provider.query(`SELECT id,mapping_status FROM ${this.table("product_media_links")}
      WHERE asset_id=${this.provider.placeholder(1)} AND product_id=${this.provider.placeholder(2)}`,
    [assetId, productId])).rows[0];
    if (found) {
      if (found.mapping_status === "rejected" || found.mapping_status === "invalid") {
        const p = this.placeholders(this.provider, 6);
        await this.provider.execute(`UPDATE ${this.table("product_media_links")} SET
          source_sku=${p[0]},source_sku_normalized=${p[1]},country_code=${p[2]},
          media_role='gallery',mapping_status='suggested',linked_at=${p[3]},linked_by=${p[4]},
          confirmed_at=NULL,confirmed_by=NULL WHERE id=${p[5]}`, [
          product.sourceSku,
          normalizeSku(product.sourceSku),
          product.countryCode,
          now,
          linkedBy || "local_session",
          found.id,
        ]);
      }
      return this.getLink(found.id);
    }
    const p = this.placeholders(this.provider, 11);
    const id = randomUUID();
    await this.provider.execute(`INSERT INTO ${this.table("product_media_links")} (
  ...
  async matchCollectedAssetsToProducts({ linkedBy = "local_session" } = {}) {
    const [assetsResult, discoveryResult] = await Promise.all([
      this.provider.query(`SELECT id,source_url,original_filename FROM ${this.table("product_media_assets")}
        WHERE status='available' AND ${productImageAssetPredicate("product_media_assets")} ORDER BY created_at,id`),
      this.provider.query(`SELECT DISTINCT asset_id,source_sku,source_sku_normalized FROM (
          SELECT d.asset_id,d.source_sku,d.source_sku_normalized
          FROM ${this.table("mabang_sku_image_discoveries")} d
          JOIN ${this.table("product_media_assets")} a ON a.id=d.asset_id
          WHERE d.asset_id IS NOT NULL AND a.status='available' AND ${productImageAssetPredicate("a")}
          UNION
          SELECT i.asset_id,d.source_sku,d.source_sku_normalized
          FROM ${this.table("mabang_sku_image_discovery_images")} i
          JOIN ${this.table("mabang_sku_image_discoveries")} d ON d.id=i.discovery_id
          JOIN ${this.table("product_media_assets")} a ON a.id=i.asset_id
          WHERE i.asset_id IS NOT NULL AND a.status='available' AND ${productImageAssetPredicate("a")}
        ) candidates WHERE source_sku_normalized IS NOT NULL AND source_sku_normalized<>''`),
    ]);

    const candidates = new Map();
    const addCandidate = ({ assetId, sourceSku, normalizedSku, source }) => {
      const normalized = normalizeSku(normalizedSku || sourceSku);
      if (!assetId || !normalized) return;
      const key = `${assetId}\u0000${normalized}`;
      if (!candidates.has(key)) candidates.set(key, {
        assetId, sourceSku: String(sourceSku || normalized), normalizedSku: normalized, source,
      });
    };
    ...
    const authoritativeAssetIds = new Set([...candidates.values()].map((item) => item.assetId));
    let invalidFilenameAssets = 0;
    for (const asset of assetsResult.rows) {
      if (authoritativeAssetIds.has(asset.id)) continue;
      const sourceSku = filenameSkuFromOriginalFilename(asset.original_filename);
      if (!sourceSku) {
        invalidFilenameAssets += 1;
        continue;
      }
      addCandidate({ assetId: asset.id, sourceSku, normalizedSku: sourceSku, source: "original_filename" });
    }

    const candidateRows = [...candidates.values()];
    const skuKeys = [...new Set(candidateRows.map((item) => item.normalizedSku))];
    const productsBySku = new Map();
    for (let offset = 0; offset < skuKeys.length; offset += 500) {
      const chunk = skuKeys.slice(offset, offset + 500);
      const placeholders = this.placeholders(this.provider, chunk.length);
      const products = await this.provider.query(`SELECT id,source_sku,sku_code_normalized,country_raw,source_product_name
        FROM ${this.table("product_skus")} WHERE sku_code_normalized IN (${placeholders.join(",")})
        AND archived_at IS NULL AND deleted_at IS NULL ORDER BY sku_code_normalized,country_raw,id`, chunk);
      for (const product of products.rows) {
        if (!productsBySku.has(product.sku_code_normalized)) productsBySku.set(product.sku_code_normalized, []);
        productsBySku.get(product.sku_code_normalized).push(product);
      }
    }

    const assetIds = [...new Set(candidateRows.map((item) => item.assetId))];
    const existingLinks = new Set();
    for (let offset = 0; offset < assetIds.length; offset += 500) {
      const chunk = assetIds.slice(offset, offset + 500);
      const placeholders = this.placeholders(this.provider, chunk.length);
      const links = await this.provider.query(`SELECT asset_id,product_id FROM ${this.table("product_media_links")}
        WHERE asset_id IN (${placeholders.join(",")})`, chunk);
      for (const link of links.rows) existingLinks.add(`${link.asset_id}\u0000${link.product_id}`);
    }
    ...
    const unmatchedSkuKeys = new Set();
    let linksExisting = 0;
    for (const candidate of candidateRows) {
      const products = productsBySku.get(candidate.normalizedSku) || [];
      if (!products.length) {
        unmatchedSkuKeys.add(candidate.normalizedSku);
        continue;
      }
      matchedAssetIds.add(candidate.assetId);
      matchedSkuKeys.add(candidate.normalizedSku);
      for (const product of products) {
        matchedProductIds.add(product.id);
        const key = `${candidate.assetId}\u0000${product.id}`;
        if (existingLinks.has(key)) {
          linksExisting += 1;
          continue;
        }
        existingLinks.add(key);
        planned.push({ candidate, product });
      }
    }
    ...
    const now = iso();
    await this.provider.transaction(async (tx) => {
      for (const { candidate, product } of planned) {
        const p = this.placeholders(tx, 11);
        const result = await tx.execute(`INSERT INTO ${this.table("product_media_links")} (
          id,asset_id,source_sku,source_sku_normalized,product_id,country_code,media_role,mapping_status,
          linked_at,linked_by,confirmed_at
        ) VALUES (${p.join(",")}) ON CONFLICT(asset_id,product_id) DO NOTHING`, [
          randomUUID(), candidate.assetId, candidate.sourceSku, candidate.normalizedSku, product.id,
          product.country_raw || "", "gallery", "suggested", now, linkedBy, null,
        ]);
        linksCreated += Number(result.rowCount || 0);
      }
    });

    return {
      assetsScanned: assetsResult.rows.length,
      candidatePairs: candidateRows.length,
      authoritativeCandidates: candidateRows.filter((item) => item.source === "discovery").length,
      filenameFallbackCandidates: candidateRows.filter((item) => item.source === "original_filename").length,
      invalidFilenameAssets,
      matchedAssets: matchedAssetIds.size,
      matchedSkus: matchedSkuKeys.size,
      matchedProducts: matchedProductIds.size,
      linksCreated,
      linksExisting,
      unmatchedAssets: assetsResult.rows.length - matchedAssetIds.size,
      unmatchedSkus: unmatchedSkuKeys.size,
      unmatchedSkuSample: [...unmatchedSkuKeys].sort().slice(0, 100),
    };
  }
  ...
  async confirmGallery(linkId, actor) {
    ...
    await this.provider.execute(`UPDATE ${this.table("product_media_links")} SET
      media_role='gallery',mapping_status='confirmed',confirmed_at=${p[0]},confirmed_by=${p[1]}
      WHERE id=${p[2]} AND product_id=${p[3]}`, [now, actor || "local_session", linkId, link.productId]);
  ...
  async rejectLink(linkId) {
    ...
    await this.provider.execute(`UPDATE ${this.table("product_media_links")} SET
      media_role='gallery',mapping_status='rejected',confirmed_at=NULL,confirmed_by=NULL
      WHERE id=${p[0]} AND product_id=${p[1]}`, [linkId, link.productId]);
    return this.getLink(linkId);
  }
  ...
  async recomputeBatchCounters(batchId) {
    ...
    const image = (await this.provider.query(`SELECT count(*) total,
      sum(CASE WHEN i.download_status='downloaded' THEN 1 ELSE 0 END) downloaded,
      sum(CASE WHEN i.download_status='duplicate' THEN 1 ELSE 0 END) duplicates,
      sum(CASE WHEN i.download_status='missing' THEN 1 ELSE 0 END) missing,
      sum(CASE WHEN i.download_status='failed' THEN 1 ELSE 0 END) failed
      FROM ${this.table("mabang_sku_image_discovery_images")} i
      JOIN ${this.table("mabang_sku_image_discoveries")} d ON d.id=i.discovery_id
      WHERE d.batch_id=${p}`, [batchId])).rows[0] || {};
    const links = (await this.provider.query(`SELECT count(DISTINCT l.product_id) linked,
      ...
      FROM ${this.table("product_media_links")} l
      JOIN ${this.table("mabang_sku_image_discoveries")} d
        ON d.source_sku_normalized=l.source_sku_normalized AND d.batch_id=${p}
    ...
    const useImageRows = Number(image.total || 0) > 0;
    const now = iso();
    const values = [Number(discovery.discovered || 0),
      Number(useImageRows ? image.downloaded : discovery.downloaded || 0),
      Number(useImageRows ? image.duplicates : discovery.duplicates || 0),
      Number(useImageRows ? image.missing : discovery.missing || 0),
      Number(useImageRows ? image.failed : discovery.failed || 0),
      Number(discovery.mismatches || 0),
  ...
  async recomputeSyncRunCounters(syncRunId) {
    const p = this.provider.placeholder(1);
    const aggregate = (await this.provider.query(`SELECT
      count(DISTINCT d.source_sku_normalized) discovered_skus,
      count(i.id) discovered_images,
      sum(CASE WHEN i.download_status='downloaded' THEN 1 ELSE 0 END) downloaded_images,
      sum(CASE WHEN i.download_status='duplicate' THEN 1 ELSE 0 END) duplicate_images,
      sum(CASE WHEN i.download_status='failed' THEN 1 ELSE 0 END) failed_images
      FROM ${this.table("mabang_sku_image_batches")} b
      JOIN ${this.table("mabang_sku_image_discoveries")} d ON d.batch_id=b.id
      LEFT JOIN ${this.table("mabang_sku_image_discovery_images")} i ON i.discovery_id=d.id
      WHERE b.sync_run_id=${p}`, [syncRunId])).rows[0] || {};
    const matches = (await this.provider.query(`SELECT
      count(DISTINCT CASE WHEN EXISTS (
        SELECT 1 FROM ${this.table("product_skus")} s
        WHERE s.sku_code_normalized=d.source_sku_normalized AND s.archived_at IS NULL AND s.deleted_at IS NULL
      ) THEN d.source_sku_normalized END) matched_skus,
      count(DISTINCT CASE WHEN NOT EXISTS (
        SELECT 1 FROM ${this.table("product_skus")} s
        WHERE s.sku_code_normalized=d.source_sku_normalized AND s.archived_at IS NULL AND s.deleted_at IS NULL
      ) THEN d.source_sku_normalized END) unmatched_skus
      FROM ${this.table("mabang_sku_image_batches")} b
      JOIN ${this.table("mabang_sku_image_discoveries")} d ON d.batch_id=b.id
      WHERE b.sync_run_id=${p}`, [syncRunId])).rows[0] || {};
    const values = [
      Number(aggregate.discovered_skus || 0), Number(aggregate.discovered_images || 0),
      Number(aggregate.downloaded_images || 0), Number(aggregate.duplicate_images || 0),
      Number(aggregate.failed_images || 0), Number(matches.matched_skus || 0),
      Number(matches.unmatched_skus || 0), iso(), syncRunId,
    ];
    const q = this.placeholders(this.provider, values.length);
    await this.provider.execute(`UPDATE ${this.table("mabang_sku_image_sync_runs")} SET
      discovered_skus=${q[0]},discovered_images=${q[1]},downloaded_images=${q[2]},
      duplicate_images=${q[3]},failed_images=${q[4]},matched_skus=${q[5]},unmatched_skus=${q[6]},
      updated_at=${q[7]} WHERE id=${q[8]}`, values);
    return this.getSyncRun(syncRunId);
  }
}

export const mabangImageSerializers = Object.freeze({
  serializeBatch,
  serializeSyncRun,
  serializeDiscovery,
  serializeDiscoveryImage,
  serializeAsset,
  serializeLink,
});
