...
export function selectInventoryCapture(captures) {
  return (captures || []).map((capture) => {
    const analyzed = analyzeInventoryPayload(capture.payload, { request: capture.request, transport: capture.transport })
      || analyzeInventoryHtmlPayload(capture.payload, { request: capture.request, transport: capture.transport });
...
const DOM_INVENTORY_SCRIPT = String.raw`(async () => {
  ...
  const rowSelectors = ['table tbody tr', '[role="rowgroup"] [role="row"]', '.el-table__body-wrapper tbody tr', '.ant-table-tbody tr', 'ul.list-body > li'];
  ...
  const rows = allRows();
  const listHeader = [...document.querySelectorAll('ul.list-title')].find((root) => visible(root)
    && root.querySelectorAll(':scope > li > ul > li').length > 1);
  const headerCells = [...document.querySelectorAll('table thead th,[role="columnheader"]'),
    ...(listHeader ? [...listHeader.querySelectorAll(':scope > li > ul > li')] : [])];
  ...
  const nameIndexes = headers.map((value, index) => /商品名称|产品名称|中文名称|英文名称|品名|product.*name|item.*name/i.test(value) ? index : -1).filter((index) => index >= 0);
  ...
  const data = rows.map((row, index) => {
    const listCells = row.matches('ul.list-body > li') ? [...(row.querySelector(':scope > ul')?.children || [])] : [];
    const cells = listCells.length ? listCells : [...row.querySelectorAll(':scope > td,:scope > [role="cell"]')];
    const skuCell = skuIndexes.map((cellIndex) => cells[cellIndex]).find(Boolean);
    const explicitSku = skuCell?.querySelector('.shopStock,[data-sku],[data-field*="sku" i]')
      || row.querySelector('.shopStock,[data-sku],[data-field*="sku" i]');
    const sourceSku = explicitSku?.getAttribute('data-sku') || explicitSku?.innerText?.trim() || readCell(cells, skuIndexes) || null;
    ...
    return {
      sourceSku,
      productName: nameIndexes.map((cellIndex) => cells[cellIndex])
        .flatMap((cell) => cell ? [...cell.querySelectorAll('p.ellipsis,[data-field*="name" i]')] : [])
        .map((node) => (node.getAttribute('title') || node.innerText || node.textContent || '').trim())
        .filter(Boolean).join(' / ') || readCell(cells, nameIndexes),
  ...
  const current = document.querySelector('[aria-current="page"],.active.el-pager__number,.ant-pagination-item-active,.page-nav .text-danger');
  const totalMatch = text.match(/共\s*(\d+)\s*(?:条|项)/);
  const next = [...document.querySelectorAll('button,a,li')].find((el) => visible(el)
    && /下一页|next/i.test((el.innerText || el.getAttribute('aria-label') || el.getAttribute('data-original-title') || el.title || '').trim()));
  return {
    rows: data,
...
const PAGE_SIZE_SCRIPT = String.raw`(() => {
  const roots = [...document.querySelectorAll('[class*="pagination" i],[class*="pager" i],[aria-label*="page" i],.page-nav')];
  const values = roots.flatMap((root) => [...root.querySelectorAll('option,[role="option"],.dropdown-menu a')])
...
const PAGE_METADATA_SCRIPT = String.raw`(() => {
  const visible = (el) => { const r = el.getBoundingClientRect(); const s = getComputedStyle(el); return r.width > 0 && r.height > 0 && s.display !== 'none' && s.visibility !== 'hidden'; };
  const root = [...document.querySelectorAll('.page-nav,[class*="pagination" i],[class*="pager" i]')].find(visible);
  const value = (root?.innerText || root?.textContent || document.body?.innerText || '').replace(/\s+/g, ' ');
  const total = Number(value.match(/共\s*(\d+)\s*(?:条|项)/)?.[1]) || null;
  const pageSize = Number(value.match(/每页\s*(\d+)\s*(?:条|项)/)?.[1]) || null;
  const pageFraction = value.match(/(\d+)\s*\/\s*(\d+)\s*页/);
  return { total, pageSize, currentPage: pageFraction ? Number(pageFraction[1]) : null,
    totalPages: pageFraction ? Number(pageFraction[2]) : null };
})()`;

const HTML_INVENTORY_PARSE_FUNCTION = String.raw`((html) => {
  if (typeof html !== 'string' || !html.trim()) return [];
  const doc = new DOMParser().parseFromString(html, 'text/html');
  const candidates = [...doc.querySelectorAll('li')].filter((node) => {
    const cells = [...(node.querySelector(':scope > ul')?.children || [])];
    return cells.length > 1 && Boolean(node.querySelector('.shopStock'));
  });
  return candidates.map((row, index) => {
    const cells = [...(row.querySelector(':scope > ul')?.children || [])];
    const skuNode = row.querySelector('.shopStock');
    const skuCell = skuNode?.closest('li');
    const image = row.querySelector('img');
    const backgroundNode = [...row.querySelectorAll('*')].find((el) => /url\(/i.test(el.getAttribute('style') || ''));
    const productNames = [...(skuCell?.querySelectorAll('p.ellipsis,[data-field*="name" i]') || [])]
      .map((node) => (node.getAttribute('title') || node.textContent || '').trim()).filter(Boolean);
    const warehouseCell = cells.find((cell) => cell.classList.contains('warehouseIds'));
    return {
      sourceSku: (skuNode?.textContent || '').trim(),
      productName: productNames.join(' / ') || null,
      warehouseName: (warehouseCell?.textContent || '').trim() || null,
      imageSrc: image?.getAttribute('src') || null,
      imageDataSrc: image?.getAttribute('data-src') || image?.getAttribute('data-original') || image?.getAttribute('data-lazy-src') || null,
      imageSrcset: image?.getAttribute('srcset') || image?.getAttribute('data-srcset') || null,
      imageBackgroundUrl: backgroundNode?.getAttribute('style') || null,
      rowNumber: index + 1,
    };
  }).filter((row) => row.sourceSku);
})`;
...
export class MabangInventoryBrowserSession {
  constructor({ targetProvider, connectCdp, wait = delay, responseWindowMs = 2500, maxPages = SAFE_MAX_PAGES,
    ...
    this.preferredTargetId = String(preferredTargetId || "").trim() || null;
    this.cdp = null;
    this.capture = null;
    this.captures = [];
  ...
  async refreshFrameContexts() {
    ...
    for (const frame of frames) {
      this.frameUrls.set(frame.id, frame.url || "");
      try {
  ...
  contextForUrl(value, preferredFrameId = null) {
    const direct = preferredFrameId ? this.frameContexts.get(preferredFrameId) : null;
    if (direct) return direct;
    try {
      const origin = new URL(value).origin;
      for (const [frameId, frameUrl] of this.frameUrls) {
        try {
          if (new URL(frameUrl).origin === origin && this.frameContexts.has(frameId)) return this.frameContexts.get(frameId);
        } catch {}
      }
    } catch {}
    return this.frameContexts.get(this.topFrameId) || null;
  }
  ...
  async open() {
    ...
    const mabangPages = (targets || []).filter((item) => item.type === "page" && isMabangUrl(item.url));
    const target = mabangPages.find((item) => this.preferredTargetId && item.id === this.preferredTargetId)
      || mabangPages.find((item) => {
        try { return new URL(item.url).searchParams.get("mod") === "warehouse.inventorydetail"; } catch { return false; }
      }) || mabangPages[0];
    ...
    const pageMetadata = (await this.evaluateAcrossFrames(PAGE_METADATA_SCRIPT)).map((item) => item.value)
      .filter((item) => Number(item?.total) > 0).sort((left, right) => Number(right.total) - Number(left.total))[0];
    ...
    if (this.capture) {
      const profile = { ...this.capture.analyzed.profile, ...Object.fromEntries(Object.entries(this.pageMetadata)
        .filter(([, value]) => Number.isFinite(Number(value)) && Number(value) > 0)) };
      this.capture.analyzed.profile = profile;
      this.interfaceEnabled = profile.hasImages && this.capture.transport !== "initialization_json";
      const preferredPageSize = Math.max(1, Number(this.pageSizeMaximum || profile.pageSize || 100));
      const totalPages = profile.total ? Math.ceil(profile.total / preferredPageSize) : profile.totalPages;
      return {
  ...
  async interfacePage(pageNumber) {
    ...
    const htmlField = JSON.stringify(analyzed.htmlField || null);
    const expression = `(async () => { const request=${input}; const htmlField=${htmlField}; const parseHtml=${HTML_INVENTORY_PARSE_FUNCTION}; const response=await fetch(request.url,{method:request.method,headers:request.headers,body:/^(GET|HEAD)$/i.test(request.method)?undefined:request.body,credentials:'include',redirect:'follow'}); const text=await response.text(); if(!htmlField)return {status:response.status,text}; try { const payload=JSON.parse(text); return {status:response.status,rows:parseHtml(payload?.[htmlField])}; } catch { return {status:response.status,rows:null}; } })()`;
    const response = await evaluate(this.cdp, expression, { contextId: this.contextForUrl(replay.url, this.capture.frameId) });
    if (Number(response?.status) !== 200) throw publicError(`MABANG_INTERFACE_HTTP_${response?.status || 0}`, "马帮库存接口分页请求失败。 ");
    let result;
    if (analyzed.htmlField) {
      const rows = (response.rows || []).map((row, index) => normalizeDiscoveryRow(row,
        { pageNumber, rowNumber: index + 1, sourceKind: "interface_html" })).filter(Boolean);
      result = { rows, profile: { ...profile, currentPage: pageNumber } };
    } else {
      let payload;
      try { payload = JSON.parse(response.text); } catch { throw publicError("MABANG_INTERFACE_JSON_INVALID", "马帮库存接口返回了非 JSON 内容。 "); }
      result = analyzeInventoryPayload(payload, { request: replay, transport: analyzed.transport });
    }
    if (!result || !result.rows?.length) throw publicError("MABANG_INTERFACE_ROWS_MISSING", "库存接口响应中没有识别到 SKU 行。 ");
    const totalPages = profile.total ? Math.ceil(profile.total / preferredPageSize)
      : result.profile.totalPages || (result.profile.total && result.profile.pageSize ? Math.ceil(result.profile.total / result.profile.pageSize) : null);
    return {
      ...
      totalPages,
      hasNext: result.rows.length > 0 && !(totalPages && pageNumber >= totalPages),
      strategy: "interface",
    };
  }
  ...
  async fetchImage(sourceUrl, { maxBytes = 10 * 1024 * 1024, timeoutMs = 30000 } = {}) {
    ...
    const sendWithTimeout = async (method, params) => {
      let timer;
      try {
        return await Promise.race([
          this.cdp.send(method, params),
          new Promise((_, reject) => {
            timer = setTimeout(() => reject(publicError("IMAGE_DOWNLOAD_TIMEOUT", "图片下载网络请求超时。", 504)), timeoutMs);
            timer.unref?.();
          }),
        ]);
      } finally {
    ...
    const loaded = await sendWithTimeout("Network.loadNetworkResource", {
      frameId: this.topFrameId || undefined,
      url: sourceUrl,
      options: { disableCache: false, includeCredentials: true },
    });
    if (!loaded?.error) {
      const resource = (loaded?.result || loaded)?.resource || {};
      if (!resource.success && !resource.stream) {
        const timeout = /TIMED_OUT|TIMEOUT/i.test(String(resource.netErrorName || ""));
        throw publicError(timeout ? "IMAGE_DOWNLOAD_TIMEOUT" : "IMAGE_NETWORK_ERROR", "图片下载网络请求失败。", 502);
      }
      const chunks = [];
      let size = 0;
      let tooLarge = false;
      try {
        while (resource.stream) {
          const message = await sendWithTimeout("IO.read", { handle: resource.stream, size: 64 * 1024 });
          if (message?.error) throw publicError("IMAGE_NETWORK_ERROR", "图片下载网络请求失败。", 502);
          const part = message?.result || message;
          const chunk = part.base64Encoded ? Buffer.from(part.data || "", "base64") : Buffer.from(part.data || "", "utf8");
          size += chunk.length;
          if (size > maxBytes) { tooLarge = true; break; }
          chunks.push(chunk);
          if (part.eof) break;
        }
      } finally {
        if (resource.stream) await this.cdp.send("IO.close", { handle: resource.stream }).catch?.(() => {});
      }
      const headers = resource.headers || {};
      const contentTypeKey = Object.keys(headers).find((name) => name.toLowerCase() === "content-type");
      const responseUrl = resource.url || sourceUrl;
      if (!isMabangUrl(responseUrl)) throw publicError("IMAGE_REDIRECT_BLOCKED", "图片跳转目标未通过马帮域名校验。", 403);
      return {
        status: Number(resource.httpStatusCode || 0),
        responseUrl,
        contentType: contentTypeKey ? headers[contentTypeKey] : null,
        tooLarge,
        size,
        buffer: tooLarge ? Buffer.alloc(0) : Buffer.concat(chunks),
      };
    }
    if (!/method.*not found|-32601/i.test(String(loaded.error?.message || loaded.error?.code || ""))) {
      throw publicError("IMAGE_NETWORK_ERROR", "图片下载网络请求失败。", 502);
    }
    ...
    const contextId = this.contextForUrl(sourceUrl, this.capture?.frameId);
