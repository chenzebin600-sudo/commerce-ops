...
const STORED_FILENAME_SKU_PATTERN = /^(.+)_\d{6,}\.(jpg|jpeg|png|webp)$/i;
export const MABANG_NON_PRODUCT_IMAGE_FILENAMES = Object.freeze([
  "icon_sluggish.png",
  "icon_flat.png",
  "icon_schwann.png",
  "icon_explode.png",
...
const MABANG_NON_PRODUCT_IMAGE_FILENAME_SET = new Set(MABANG_NON_PRODUCT_IMAGE_FILENAMES);
...
function allHttpUrls(value, result = []) {
  if (typeof value === "string") {
    for (const part of value.split(/\s*,\s*/)) {
      const candidate = part.trim().split(/\s+/, 1)[0];
      if (IMAGE_URL_PATTERN.test(candidate)) result.push(candidate);
    }
  } else if (Array.isArray(value)) {
    for (const item of value) allHttpUrls(item, result);
  } else if (value && typeof value === "object") {
    for (const item of Object.values(value)) allHttpUrls(item, result);
  }
  return result;
}
...
function detectedImages(row) {
  const fields = flattenImageValues(row);
  const candidates = [];
  for (const field of fields) {
    const named = isImageKey(field.key);
    const extension = typeof field.value === "string"
      && IMAGE_URL_PATTERN.test(field.value) && IMAGE_EXTENSION_PATTERN.test(field.value);
    if (!named && !extension) continue;
    for (const url of allHttpUrls(field.value)) {
      candidates.push({ path: field.path, url, sourceKind: "interface" });
    }
  }
  return candidates.filter((item, index, items) => items.findIndex((candidate) => candidate.url === item.url) === index);
}

function flattenImageValues(value, prefix = "", depth = 0, result = []) {
  if (depth > 8 || value == null) return result;
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      flattenImageValues(value[index], `${prefix}[${index}]`, depth + 1, result);
    }
    return result;
  }
  if (typeof value === "object") {
    for (const [key, item] of Object.entries(value)) {
      flattenImageValues(item, prefix ? `${prefix}.${key}` : key, depth + 1, result);
    }
    return result;
  }
  result.push({ path: prefix, key: prefix.split(".").pop() || prefix, value });
...
function rowProjection(row) {
  ...
  if (!sku) return null;
  const images = detectedImages(row);
...
export function analyzeInventoryHtmlPayload(payload, { request = {}, transport = "xhr" } = {}) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) return null;
  const candidates = Object.entries(payload).filter(([, value]) => typeof value === "string")
    .map(([field, value]) => ({
      field,
      value,
      skuNodes: (value.match(/\bshopStock\b/gi) || []).length,
      imageNodes: (value.match(/<img\b/gi) || []).length,
    }))
    .filter((candidate) => candidate.skuNodes > 0)
    .sort((left, right) => right.skuNodes - left.skuNodes || right.imageNodes - left.imageNodes);
  const best = candidates[0];
  if (!best) return null;
  const requestFields = requestPagination(request);
  return {
    transport,
    request,
    htmlField: best.field,
    rows: [],
    confidence: Math.min(100, 70 + Math.min(20, best.skuNodes) + Math.min(10, best.imageNodes)),
    profile: {
      ...
      rowsPath: best.field,
      skuPath: ".shopStock",
      imagePath: "img[src|data-src|srcset]",
      warehousePath: "header:仓库",
      productNamePath: "header:中文名称 / 英文名称",
      totalPath: null,
      pagePath: null,
      pageSizePath: null,
      totalPagesPath: null,
      ...
      pageSizeParameter: requestFields.pageSizeParameter,
      total: null,
      currentPage: requestFields.pageParameter?.value || null,
      pageSize: requestFields.pageSizeParameter?.value || null,
      totalPages: null,
      hasImages: best.imageNodes > 0,
    },
  };
}
...
export function filenameSkuFromOriginalFilename(value) {
  const filename = String(value || "").replaceAll("\\", "/").split("/").pop() || "";
  try {
    return text(decodeURIComponent(filename).match(STORED_FILENAME_SKU_PATTERN)?.[1] || "") || null;
  } catch {
    return text(filename.match(STORED_FILENAME_SKU_PATTERN)?.[1] || "") || null;
  }
}

export function isKnownMabangNonProductImage({ sourceUrl, originalFilename } = {}) {
  const filename = String(originalFilename || "").replaceAll("\\", "/").split("/").pop()?.toLowerCase() || "";
  if (MABANG_NON_PRODUCT_IMAGE_FILENAME_SET.has(filename)) return true;
  try {
    const parsed = new URL(String(sourceUrl || ""));
    const urlFilename = decodeURIComponent(parsed.pathname.split("/").pop() || "").toLowerCase();
    return parsed.hostname.toLowerCase() === "global.mabangerp.com"
      && parsed.pathname.toLowerCase().startsWith("/image/icon_")
      && /\.(?:jpe?g|png|webp)$/i.test(urlFilename);
  } catch {
    return false;
  }
}
...
export function normalizeDiscoveryRow(input, { pageNumber = 1, rowNumber = 1, sourceKind = "interface" } = {}) {
  ...
  const append = (value, kind) => {
    for (const url of allHttpUrls(value)) candidates.push({ url, sourceKind: kind });
  };
  for (const candidate of input.imageCandidates || input.image_candidates || []) {
    if (typeof candidate === "string") append(candidate, sourceKind);
    else append(candidate?.url, candidate?.sourceKind || candidate?.source_kind || sourceKind);
  }
  append(input.imageUrls ?? input.image_urls, sourceKind);
  append(input.sourceImageUrl ?? input.source_image_url, sourceKind);
  append(input.imageSrc ?? input.image_src, "src");
  append(input.imageDataSrc ?? input.image_data_src, "data_src");
  append(input.imageSrcset ?? input.image_srcset, "srcset");
  const background = backgroundUrl(input.imageBackgroundUrl ?? input.image_background_url);
  if (background) append(background, "background");
  const imageCandidates = candidates
    .filter((candidate) => IMAGE_URL_PATTERN.test(candidate.url))
    .filter((candidate) => !isKnownMabangNonProductImage({ sourceUrl: candidate.url }))
    .filter((candidate, index, items) => items.findIndex((item) => item.url === candidate.url) === index);
  const sourceImageUrl = imageCandidates[0]?.url || null;
...
export function deduplicateDiscoveryRows(rows) {
  ...
  for (const row of rows) {
    ...
    const key = [
      ...
      (row.imageCandidates || []).map((image) => image.url).sort().join("|"),
      row.sourceImageUrl || "",
      row.warehouseName || "",
    ].join("\u0000");
...
export function inventoryPageHash(rows) {
  const payload = rows.map((row) => [
    row.sourceSkuNormalized,
    row.sourceImageUrl || "",
    (row.imageCandidates || []).map((image) => image.url).sort(),
    row.warehouseName || "",
  ]);
