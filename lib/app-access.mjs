...
export function resolveAppConfig(env = process.env) {
  ...
  const allowUnauthenticatedLan = envFlag(env.APP_ALLOW_UNAUTHENTICATED_LAN);

  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error("APP_PORT 必须是 1 到 65535 之间的整数");
  }
  if (!isLoopbackBindHost(host) && !accessToken && !allowUnauthenticatedLan) {
    throw new Error("外部监听必须配置 APP_ACCESS_TOKEN");
  }

  const unauthenticatedCompatibilityMode = !accessToken
    && (isLoopbackBindHost(host) || allowUnauthenticatedLan);
  ...
  return {
    ...
    localCompatibilityMode: unauthenticatedCompatibilityMode,
  };
}
...
export function createAccessPolicy({ host, accessToken, allowUnauthenticatedLan = false }) {
  const expectedToken = String(accessToken || "").trim();
  const authenticationEnabled = Boolean(expectedToken);
  const localCompatibilityMode = !authenticationEnabled
    && (isLoopbackBindHost(host) || Boolean(allowUnauthenticatedLan));
...
export function appStartupMessages({ host, port }, policy) {
  ...
  if (policy.localCompatibilityMode && isLoopbackBindHost(host)) {
    messages.push("当前未启用访问Token，仅允许本机访问");
  } else if (policy.localCompatibilityMode) {
    messages.push("当前未启用访问Token，局域网设备可以直接访问");
  }
  return messages;
}
...
export function authenticationApiResponse({ method, pathname, headers }, policy) {
  ...
  if (pathname === "/api/auth/logout") {
    ...
    if (!policy.isAuthenticated(headers)) return { status: 401, body: { ok: false, error: "Unauthorized" } };
    return { status: 200, body: { ok: true } };
  }
