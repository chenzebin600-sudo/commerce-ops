...
export function describeAuditRequest(method, pathname) {
  ...
  if (pathname === "/api/sales-assortment/analyze" && method === "POST") {
    return descriptor("growth_radar", "deepseek.call", { metadata: { provider: "sales_assortment" } });
  }
  ...
  if (/^\/api\/growth-radar\/shops\/[^/]+\/confirm$/.test(pathname) && method === "POST") return descriptor("growth_radar", "growth_radar.shop.confirmed");
  if (/^\/api\/growth-radar\/shops\/[^/]+\/revoke$/.test(pathname) && method === "POST") return descriptor("growth_radar", "growth_radar.shop.confirmation_revoked");
  ...
  if (pathname === "/api/growth-radar/v2/configuration/country-mappings" && method === "PUT") {
    return descriptor("growth_radar", "growth_radar.country_mapping.updated");
  }
  if (pathname === "/api/growth-radar/v2/configuration/rules" && method === "PUT") {
    return descriptor("growth_radar", "growth_radar.rule_set.updated");
  }
  if (pathname === "/api/inventory-sync/prepare" && method === "POST") return descriptor("mabang", "inventory_sync.source.refreshed");
  if (pathname === "/api/inventory-sync/preview" && method === "POST") return descriptor("mabang", "inventory_sync.plan.previewed");
  if (/^\/api\/inventory-sync\/plans\/[^/]+\/approve$/.test(pathname) && method === "POST") return descriptor("mabang", "inventory_sync.plan.approved");
  if (/^\/api\/inventory-sync\/plans\/[^/]+\/execute$/.test(pathname) && method === "POST") return descriptor("mabang", "inventory_sync.execution.started");
  if (/^\/api\/inventory-sync\/plans\/[^/]+\/continuous-start$/.test(pathname) && method === "POST") return descriptor("mabang", "inventory_sync.continuous.started");
  if (pathname === "/api/mabang-images/batches" && method === "POST") return descriptor("mabang", "mabang_images.collect_started");
  if (pathname === "/api/mabang-images/sync-runs" && method === "POST") return descriptor("mabang", "mabang_images.sync_started");
  if (pathname === "/api/mabang-images/match-products" && method === "POST") return descriptor("mabang", "mabang_images.bulk_linked");
  if (/^\/api\/mabang-images\/sync-runs\/[^/]+\/resume$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.sync_resumed");
  if (/^\/api\/mabang-images\/batches\/[^/]+\/pause$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.pause_requested");
  if (/^\/api\/mabang-images\/batches\/[^/]+\/resume$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.resumed");
  if (/^\/api\/mabang-images\/products\/[^/]+\/assets$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.manual_uploaded");
  if (/^\/api\/mabang-images\/links\/[^/]+\/confirm-primary$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.primary_confirmed");
  if (/^\/api\/mabang-images\/links\/[^/]+\/confirm-gallery$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.gallery_confirmed");
  if (/^\/api\/mabang-images\/links\/[^/]+\/reject$/.test(pathname) && method === "POST") return descriptor("mabang", "mabang_images.link_rejected");
...
export function createHttpAuditContext(req, url, {
  trustedProxies = new Set(),
  localCompatibilityMode = false,
  now = () => new Date(),
} = {}) {
  const startedAt = now();
  const remoteAddress = String(req.socket?.remoteAddress || "").replace(/^::ffff:/, "");
  const trustedIdentitySource = trustedProxies.has(remoteAddress);
  const assertedIdentity = trustedIdentitySource ? String(req.headers?.["x-authenticated-user"] || "").trim() : "";
  const assertedRoles = trustedIdentitySource ? String(req.headers?.["x-authenticated-roles"] || "").split(",").map((role) => role.trim()).filter(Boolean) : [];
  const authenticated = Boolean(req.headers?.authorization);
  const trustedIdentity = authenticated && assertedIdentity && /^[A-Za-z0-9][A-Za-z0-9_.@-]{0,127}$/.test(assertedIdentity)
    ? Object.freeze({ actorId: assertedIdentity, roles: Object.freeze([...new Set(assertedRoles)].sort()), source: "trusted_proxy" })
    : null;
  const localShopeeIdentity = localCompatibilityMode === true
    && ["127.0.0.1", "::1"].includes(remoteAddress)
    && (url.pathname === "/api/shopee-discount" || url.pathname.startsWith("/api/shopee-discount/"))
    ? Object.freeze({ actorId: "local-operator", roles: Object.freeze(["shopee_discount_execute"]), source: "loopback_compatibility" })
    : null;
  const identity = trustedIdentity || localShopeeIdentity;
  const context = {
    ...
    actorType: authenticated ? "authenticated_session" : "local_session",
    hadAuthorization: authenticated,
    identity,
    privilegedIdentity: identity?.roles.includes("shopee_discount_execute") ? "privileged_execute_identity" : null,
