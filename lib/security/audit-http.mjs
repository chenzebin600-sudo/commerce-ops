...
function customerServiceDescriptor(method, pathname) {
  ...
  const actions = {
    start: "customer_service.local_runtime.start_requested",
    stop: "customer_service.local_runtime.stop_requested",
    retry: "customer_service.local_runtime.retry_requested",
  };
  return descriptor("customer_service", actions[match[1]]);
}

function productKnowledgeDescriptor(method, pathname) {
  if (method !== "POST") return null;
  if (/^\/api\/product-knowledge\/candidates\/[^/]+\/reviews$/.test(pathname)) {
    return descriptor("product_knowledge", "product_knowledge.candidate.reviewed");
  }
  if (pathname === "/api/product-knowledge/releases") {
    return descriptor("product_knowledge", "product_knowledge.release.created");
  }
  if (/^\/api\/product-knowledge\/releases\/[^/]+\/publish$/.test(pathname)) {
    return descriptor("product_knowledge", "product_knowledge.release.published");
  }
  return null;
}
...
export function describeAuditRequest(method, pathname) {
  ...
  if (pathname === "/api/price-control/sync" && method === "POST") return descriptor("price_control", "product.price_control.sync.requested");
  if (pathname === "/api/price-control/automation" && method === "PUT") return descriptor("price_control", "product.price_control.automation.updated");
  ...
  return productKnowledgeDescriptor(method, pathname)
    || customerServiceDescriptor(method, pathname)
    || schedulerDescriptor(method, pathname);
}
...
function finalDescriptor(context, httpStatus) {
  ...
  if (current.action === "product.price_control.sync.requested" && httpStatus >= 400) return { ...current, action: "product.price_control.sync.failed" };
...
export function completeHttpAudit(audit, context, { httpStatus = 200, error = null, now = () => new Date() } = {}) {
  ...
  const writes = entries.map((item) => audit.recordSafely({
    ...base,
  ...
  if (writes.some((write) => write && typeof write.then === "function")) {
    return Promise.all(writes).then((results) => results.filter(Boolean));
  }
