...
import { SqliteAuditRepository } from "../data/sqlite/sqlite-audit-repository.mjs";
...
function serializeRow(row) {
  if (!row) return null;
  let metadata = {};
  if (row.metadata_json && typeof row.metadata_json === "object" && !Array.isArray(row.metadata_json)) {
...
export class OperationAuditService {
  constructor({ repository = null, database = null, logger = console, now = () => new Date(), secretValues = [] }) {
    this.repository = repository || (database ? new SqliteAuditRepository({ provider: database }) : null);
  ...
  recordAuditEvent(input = {}) {
    ...
    return created && typeof created.then === "function" ? created.then(serializeRow) : serializeRow(created);
  }

  recordSafely(input = {}) {
    try {
      const recorded = this.recordAuditEvent(input);
  ...
  getEvent(id) {
    const row = this.repository.get(String(id || ""));
    return row && typeof row.then === "function" ? row.then(serializeRow) : serializeRow(row);
  }
  ...
  queryEvents(filters = {}) {
    ...
    const queried = this.repository.query({
      ...normalized,
    ...
    const format = ({ rows, total }) => ({ events: rows.map(serializeRow), page, pageSize, total, totalPages: Math.max(1, Math.ceil(total / pageSize)) });
    return queried && typeof queried.then === "function" ? queried.then(format) : format(queried);
  }
  ...
  async summary({ start, end } = {}) {
    ...
    const [{ total }, { byStatus, byModule }] = await Promise.all([range, this.repository.summary({
...
export function createOperationAuditService({ repository = null, db = null, env = process.env, logger = console, now } = {}) {
  const resolvedRepository = repository || new SqliteAuditRepository({ provider: db });
  const secretValues = [
