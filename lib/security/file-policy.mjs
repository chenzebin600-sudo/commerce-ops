...
const PUBLIC_MESSAGES = Object.freeze({
  ...
  FILE_MISSING: "文件记录存在，但物理文件缺失。",
  FILE_NOT_AVAILABLE: "文件当前不可下载。",
...
function defaultStatus(code) {
  if (code === FILE_ERROR_CODES.FILE_NOT_FOUND || code === FILE_ERROR_CODES.FILE_MISSING) return 404;
  if (code === FILE_ERROR_CODES.FILE_NOT_AVAILABLE || code === FILE_ERROR_CODES.FILE_INTEGRITY_FAILED) return 409;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw new FilePolicyError(FILE_ERROR_CODES.FILE_STORAGE_ERROR, { cause: statError });
        }
      }
    }
...
export async function atomicMoveFile({
  ...
  const destination = await resolveNewFile(destinationRoot, destinationRelativePath, { allowedExtensions });
  try {
...
export function inspectXlsxBuffer(buffer, limits) {
  ...
  for (const worksheet of worksheets) {
    ...
    formulaCells += [...xml.matchAll(/<f(?:\s[^>]*)?>/gi)].length;
  }
  return Object.freeze({ entries: entries.size, sheets: worksheets.length, formulaCells });
}
