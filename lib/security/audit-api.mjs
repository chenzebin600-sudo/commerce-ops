  ...
  return async function handleAuditApi(req, res, url) {
    ...
    try {
      if (pathname === "/api/audit/events" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, ...audit.queryEvents(queryFilters(url.searchParams)), actionLabels: AUDIT_ACTION_LABELS });
      }
      const detail = pathname.match(/^\/api\/audit\/events\/([A-Za-z0-9-]+)$/);
      if (detail && req.method === "GET") {
        const event = audit.getEvent(detail[1]);
      ...
      if (pathname === "/api/audit/summary" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, summary: audit.summary({ start: url.searchParams.get("start"), end: url.searchParams.get("end") }) });
      }
