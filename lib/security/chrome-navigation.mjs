...
export async function createChromeNavigationGuard({
  ...
  function track(task) {
    ...
    task.then(
      () => pendingValidations.delete(task),
      () => pendingValidations.delete(task),
    );
  }
  ...
  async function validateCommittedFrame(params) {
    ...
    if (frame.id) mainFrameId = frame.id;
    try {
  ...
  mainFrameId = frameTree?.result?.frameTree?.frame?.id || mainFrameId;
  ...
  async function navigate(inputUrl) {
    ...
    // CDP can reject the navigation before Page.navigate resolves. Attach a
    // handler immediately so strict unhandled-rejection mode cannot stop the service.
    completion.catch(() => {});
    try {
      const result = await cdp.send("Page.navigate", { url: target.url });
      mainFrameId = result?.result?.frameId || mainFrameId;
