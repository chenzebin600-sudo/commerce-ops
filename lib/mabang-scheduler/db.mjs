...
export class SchedulerDatabase {
  ...
  listTasks() {
    const rows = this.db.prepare(`SELECT t.*, a.name account_name, a.username account_username, d.name dingtalk_name
      ...
      JOIN mabang_account_profiles a ON a.id=t.account_profile_id
  ...
  saveTask(input) {
    ...
    const existing = input.id ? this.db.prepare("SELECT id FROM scheduled_export_tasks WHERE id=?").get(id) : null;
    if (existing) {
  ...
  setTaskEnabled(id, enabled, nextRunAt = null) {
    this.db.prepare("UPDATE scheduled_export_tasks SET enabled=?, next_run_at=?, updated_at=? WHERE id=?")
      .run(Number(Boolean(enabled)), nextRunAt, iso(), id);
    return this.getTask(id);
  }

  updateTaskScheduleState(id, { nextRunAt, lastRunAt, lastRunStatus }) {
    this.db.prepare(`UPDATE scheduled_export_tasks SET next_run_at=COALESCE(?,next_run_at),last_run_at=COALESCE(?,last_run_at),last_run_status=COALESCE(?,last_run_status),updated_at=? WHERE id=?`)
      .run(nextRunAt ?? null, lastRunAt ?? null, lastRunStatus ?? null, iso(), id);
  }

  deleteTask(id) {
    return this.db.prepare("DELETE FROM scheduled_export_tasks WHERE id=?").run(id).changes;
  }
  ...
  listRuns({ taskId = null, status = null, limit = 100 } = {}) {
    ...
    const rows = this.db.prepare(`SELECT r.*,t.name task_name,t.task_type task_type,f.status file_status,f.original_filename
  ...
  getRun(id) {
    const row = this.db.prepare(`SELECT r.*,t.name task_name,t.task_type task_type,f.status file_status,f.original_filename
      FROM scheduled_export_runs r JOIN scheduled_export_tasks t ON t.id=r.task_id
  ...
  createExportFile(input) {
    ...
    this.db.prepare(`INSERT INTO export_files
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(
      id, input.taskId, input.runId, input.originalFilename, input.storageFilename, input.relativePath,
      input.fileSize, input.fileHash || null, input.status || "available", input.expiresAt ? iso(input.expiresAt) : null, iso(),
    );
    return this.getExportFile(id);
  }

  getExportFile(id) {
    const row = this.db.prepare("SELECT * FROM export_files WHERE id=?").get(id);
    if (!row) return null;
    return {
      id: row.id, taskId: row.task_id, runId: row.run_id, originalFilename: row.original_filename,
      storageFilename: row.storage_filename, relativePath: row.relative_path, fileSize: Number(row.file_size || 0),
      fileHash: row.file_hash || null, status: row.status, expiresAt: row.expires_at || null, createdAt: row.created_at,
    };
  }

  expiredFiles(now = new Date()) {
    return this.db.prepare("SELECT * FROM export_files WHERE status='available' AND expires_at IS NOT NULL AND expires_at<=?").all(iso(now));
  }

  markFileExpired(id) {
    this.db.prepare("UPDATE export_files SET status='expired' WHERE id=?").run(id);
  }
  ...
  dueTasks(now = new Date()) {
    return this.db.prepare("SELECT id FROM scheduled_export_tasks WHERE enabled=1 AND next_run_at IS NOT NULL AND next_run_at<=? ORDER BY next_run_at").all(iso(now))
