...
export function createTaskExecutor({
  ...
  runHeartbeatIntervalMs = Number(process.env.SCHEDULER_RUN_HEARTBEAT_INTERVAL_MS || 30000),
  now = () => new Date(),
  audit = null,
  fileService = null,
  ...
  const heartbeatIntervalMs = Math.max(25, Number(runHeartbeatIntervalMs) || 30000);
  async function runStep(runId, stage, callback, { attempt = 1, message = "" } = {}) {
    ...
    const heartbeatTimer = typeof db.touchRun === "function"
      ? setInterval(() => {
          if (heartbeatInFlight) return;
          heartbeatInFlight = Promise.resolve(db.touchRun(runId, now()))
            .catch((error) => process.stderr.write(`[scheduler] ${new Date().toISOString()} run heartbeat failed: ${error.message}\n`))
      ...
      if (reportTask) {
        ...
        run = await db.updateRun(runId, {
          ...
          logSummary: {
            ...
            agentTaskId: report.agentTaskId || null,
          },
        });
        ...
        return persistCollectedData({
          ...
          sourceScope: inventoryTask
            ...
            : {
                ...
                shopCoverageMode: effectiveFilters.length ? "FILTERED_SHOPS" : "ALL_VISIBLE_SHOPS",
              },
