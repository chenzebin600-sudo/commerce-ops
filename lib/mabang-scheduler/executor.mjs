  ...
  async function executeRun(runId) {
    ...
    try {
      if (reportTask) {
        ...
        db.addRunEvent({ runId, stage: "complete", status: "success", startedAt: executionStarted, finishedAt, durationMs: finishedAt - executionStarted, message: "经营日报已生成并推送钉钉。" });
        db.updateTaskScheduleState(task.id, { lastRunAt: finishedAt.toISOString(), lastRunStatus: "success" });
        audit?.recordSafely({
