...
export function createTaskExecutor({ db, runWorker, exportRoot, tempRoot = path.join(exportRoot, ".temp"), notify = sendDingtalkMessage, retryDelays = retryDelaysFromEnv(), now = () => new Date(), audit = null, fileService = null }) {
  ...
  async function executeRun(runId) {
    ...
    if (!claim.claimed) {
      const rejectedRun = claim.run || db.getRun(runId);
      if (claim.reason === "task_deleted" && rejectedRun) {
        const rejectedTask = db.getTask(rejectedRun.taskId);
        audit?.recordSafely({
          module: "mabang",
    ...
    try {
      ...
      const deletedDuringExecution = Boolean(db.getTask(task.id)?.deletedAt);
      ...
      audit?.recordSafely({
        ...
        metadata: { taskType: task.taskType, triggerType: run.triggerType, result: deletedDuringExecution ? `${status}_task_deleted` : status },
      });
