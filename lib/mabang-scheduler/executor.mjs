...
export function createTaskExecutor({ db, runWorker, exportRoot, notify = sendDingtalkMessage, retryDelays = retryDelaysFromEnv(), now = () => new Date() }) {
  ...
  async function executeRun(runId) {
    ...
    try {
      ...
      const targetDir = path.join(exportRoot, task.id, monthFolder);
      const storageFilename = `${runId}-${filename}`;
      const outputPath = path.join(targetDir, storageFilename);
      await fs.mkdir(targetDir, { recursive: true });
      await runStep(runId, stage, () => runWorker({
        action: "write-xlsx", outputPath, kind: inventoryTask ? "inventory" : "orders", columns: result.columns, records: result.records,
      ...
      const fileRecord = await runStep(runId, stage, async () => {
        ...
        const relativePath = path.relative(exportRoot, outputPath).split(path.sep).join("/");
        return db.createExportFile({
          taskId: task.id, runId, originalFilename: filename, storageFilename, relativePath,
          fileSize: content.length, fileHash: createHash("sha256").update(content).digest("hex"),
