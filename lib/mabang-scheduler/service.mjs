...
export class MabangSchedulerService {
  constructor({ db, executor, exportRoot, audit = null, pollIntervalMs = Number(process.env.SCHEDULER_POLL_INTERVAL_MS || 10000), catchUpThresholdMs = Number(process.env.SCHEDULER_CATCH_UP_THRESHOLD_MS || 2 * 60 * 60 * 1000), now = () => new Date() }) {
    this.db = db;
  ...
  async cleanupExpiredFiles(current = this.now()) {
    for (const file of this.db.expiredFiles(current)) {
      try {
        const target = await resolveExistingFile(this.exportRoot, file.relative_path, { allowedExtensions: [".xlsx"] });
