...
async function assertTaskRunIdle(db, taskId) {
  const active = await db.getActiveRunForTask(taskId);
  if (!active) return;
  throw new TaskStateError(
    "TASK_RUN_ACTIVE",
    `该任务已有${active.status === "running" ? "运行中" : "排队中"}的执行记录，请等待完成后再重试。`,
    409,
  );
}
  ...
  return async function handleMabangSchedulerApi(req, res, url) {
    ...
    try {
      ...
      if (match && req.method === "POST") {
        ...
        await assertTaskRunIdle(db, source.taskId);
