  ...
  return async function handleMabangSchedulerApi(req, res, url) {
    ...
    try {
      ...
      if (match && req.method === "GET") {
        ...
        const file = validateDownloadMetadata(db.getExportFile(fileId));
        req.auditContext?.annotate({ fileId, taskId: file.taskId, runId: file.runId });
        const target = await resolveExistingFile(exportRoot, file.relativePath, { allowedExtensions: [".xlsx"] });
        if (target.stat.size <= 0 || target.stat.size !== file.fileSize) {
