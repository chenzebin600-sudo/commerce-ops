...
async function taskInput(body, db, existing = null, now = new Date()) {
  ...
  if (!["order_export", "inventory_export", "daily_report"].includes(taskType)) throw new Error("请选择订单、库存或经营日报任务。");
  const accountProfileId = String(body.accountProfileId || "");
  if (!await db.getAccountProfile(accountProfileId)) throw new Error("请选择有效的马帮账号配置。");
  const dingtalkConfigId = body.dingtalkConfigId ? String(body.dingtalkConfigId) : null;
  if (dingtalkConfigId && !await db.getDingtalkConfig(dingtalkConfigId)) throw new Error("钉钉机器人配置不存在。");
...
function manualOrderExecutionOptions(body, task) {
  if (task.taskType !== "order_export") return null;
  const hasOverrides = ["paymentDateMode", "paymentDateConfig", "filters"].some((key) => Object.hasOwn(body, key));
  if (!hasOverrides) return null;
  const paymentDateMode = String(body.paymentDateMode || task.paymentDateMode || "yesterday");
  const paymentDateConfig = Object.hasOwn(body, "paymentDateConfig")
    ? body.paymentDateConfig
    : paymentDateMode === task.paymentDateMode ? task.paymentDateConfig : {};
  return {
    paymentDateMode,
    paymentDateConfig: validatePaymentDateConfig(paymentDateMode, paymentDateConfig || {}),
    filters: normalizeTaskFilters(Object.hasOwn(body, "filters") ? body.filters : task.filters),
  };
}
...
function accountInput(body, existing = null) {
  ...
  const username = cleanText(body.username, "马帮账号", { required: true, max: 120 });
  if (existing && username !== existing.username && body.allowUsernameChange !== true) {
    const error = new Error("当前页面的账号信息已过期，请刷新后再切换马帮账号。");
    error.code = "MABANG_ACCOUNT_STALE_UPDATE";
    throw error;
  }
  return {
    id: existing?.id,
    ...
    try {
      if (pathname === "/api/mabang/scheduler-meta" && req.method === "GET") {
        return sendJson(res, 200, {
          ok: true,
          scheduler: await db.schedulerStatus(now()),
          ...
          taskTypes: ["order_export", "inventory_export", "daily_report"],
      ...
      if (pathname === "/api/mabang/account-profiles") {
        if (req.method === "GET") return sendJson(res, 200, { ok: true, profiles: await db.listAccountProfiles() });
        if (req.method === "POST") {
          const profile = await db.saveAccountProfile(accountInput(await readJson(req)));
      ...
      if (match) {
        ...
        const existing = await db.getAccountProfile(id, { includeSecret: true });
        if (!existing) return sendJson(res, 404, { ok: false, error: "马帮账号配置不存在。" });
        req.auditContext?.annotate({ actorIdentifier: existing.username });
        if (req.method === "PUT") return sendJson(res, 200, { ok: true, profile: await db.saveAccountProfile(accountInput(await readJson(req), existing)) });
        if (req.method === "DELETE") return sendJson(res, 200, { ok: true, deleted: Boolean(await db.deleteAccountProfile(id)) });
        return sendJson(res, 405, { ok: false, error: "Method not allowed" });
      }
      match = idMatch(pathname, "/api/mabang/account-profiles", "test");
      if (match && req.method === "POST") {
        const id = decodeURIComponent(match[1]);
        const profile = await db.getAccountProfile(id, { includeSecret: true });
        ...
        try {
          ...
          await db.updateAccountVerification(id, "success", result.message || "登录成功");
          ...
          await db.updateAccountVerification(id, "failed", error.message);
          return sendJson(res, 400, { ok: false, error: error.message });
        }
      }

      if (pathname === "/api/notifications/dingtalk/configs") {
        if (req.method === "GET") return sendJson(res, 200, { ok: true, configs: await db.listDingtalkConfigs() });
        if (req.method === "POST") return sendJson(res, 201, { ok: true, config: await db.saveDingtalkConfig(robotInput(await readJson(req))) });
        return sendJson(res, 405, { ok: false, error: "Method not allowed" });
      }
      match = idMatch(pathname, "/api/notifications/dingtalk/configs");
      if (match) {
        const id = decodeURIComponent(match[1]);
        const existing = await db.getDingtalkConfig(id, { includeSecret: true });
        if (!existing) return sendJson(res, 404, { ok: false, error: "钉钉机器人配置不存在。" });
        if (req.method === "PUT") return sendJson(res, 200, { ok: true, config: await db.saveDingtalkConfig(robotInput(await readJson(req), existing)) });
        if (req.method === "DELETE") return sendJson(res, 200, { ok: true, deleted: Boolean(await db.deleteDingtalkConfig(id)) });
        return sendJson(res, 405, { ok: false, error: "Method not allowed" });
      }
      match = idMatch(pathname, "/api/notifications/dingtalk/configs", "test");
      if (match && req.method === "POST") {
        const config = await db.getDingtalkConfig(decodeURIComponent(match[1]), { includeSecret: true });
      ...
      if (pathname === "/api/mabang/scheduled-tasks") {
        if (req.method === "GET") {
          ...
          return sendJson(res, 200, { ok: true, tasks: await db.listTasks({ includeDeleted }) });
        }
        if (req.method === "POST") {
          const task = await db.saveTask(await taskInput(await readJson(req), db, null, now()));
      ...
      if (match) {
        ...
        if (req.method === "PUT") {
          ...
          return sendJson(res, 200, { ok: true, task: await db.saveTask(await taskInput(await readJson(req), db, existing, now())) });
        }
        if (req.method === "DELETE") {
          const body = await readJson(req);
          const deleteReason = sanitizeDeleteReason(body.reason);
          const result = await db.softDeleteTask(id, {
      ...
      if (match && req.method === "POST") {
        ...
        if (!existing) return sendJson(res, 404, { ok: false, error: "定时任务不存在。" });
        req.auditContext?.annotate({ taskId: id, actorIdentifier: existing.accountUsernameMasked, metadata: { taskType: existing.taskType } });
        const result = await db.restoreTask(id, { now: now() });
      ...
      for (const action of ["enable", "disable", "run-now", "duplicate", "preview"]) {
        ...
        if (action === "enable") return sendJson(res, 200, { ok: true, task: await db.setTaskEnabled(id, true, nextRunAt(task, now()).toISOString()) });
        if (action === "disable") return sendJson(res, 200, { ok: true, task: await db.setTaskEnabled(id, false, null) });
        if (action === "preview") {
          const previewAt = now();
          return sendJson(res, 200, {
            ok: true,
        ...
        if (action === "run-now") {
          const scheduled = now();
          const body = await readJson(req);
          const executionOptions = manualOrderExecutionOptions(body, task);
          const run = await db.createRun({ taskId: id, triggerType: "manual", scheduledRunAt: scheduled, executionOptions });
          ...
          return sendJson(res, 202, {
            ...
            paymentDateRange: task.taskType === "order_export" ? paymentDateRange(effectivePaymentDateMode, effectivePaymentDateConfig, scheduled, task.timezone) : null,
            filterCount: task.taskType === "order_export" ? (executionOptions?.filters || task.filters).length : 0,
          });
        }
        const copy = await db.saveTask({ ...task, id: undefined, name: `${task.name} - 副本`, enabled: false, nextRunAt: null });
        return sendJson(res, 201, { ok: true, task: copy });
      }

      if (pathname === "/api/mabang/scheduled-runs" && req.method === "GET") {
        return sendJson(res, 200, { ok: true, runs: await db.listRuns({ taskId: url.searchParams.get("taskId"), status: url.searchParams.get("status"), limit: url.searchParams.get("limit") }) });
      }
      match = idMatch(pathname, "/api/mabang/scheduled-runs");
      if (match && req.method === "GET") {
        const run = await db.getRunDetails(decodeURIComponent(match[1]));
      ...
      if (match && req.method === "POST") {
        const source = await db.getRun(decodeURIComponent(match[1]));
        if (!source) return sendJson(res, 404, { ok: false, error: "执行记录不存在。" });
        requireActiveTask(req, await db.getTask(source.taskId));
        const run = await db.createRun({
          taskId: source.taskId,
          triggerType: "retry",
          scheduledRunAt: now(),
          executionOptions: source.logSummary?.executionOptions || null,
        });
      ...
      if (pathname === "/api/mabang/scheduler-filter-options" && req.method === "GET") {
        ...
        return sendJson(res, 200, { ok: true, options: accountProfileId ? await db.filterOptions(accountProfileId) : {} });
      }
