...
import { ShopeeDiscountRepository } from "../shopee-discount/repository.mjs";
...
function repositoriesFor({ provider, scheduler, postgresql = null }) {
  return Object.freeze({
    ...
    shopeeDiscount: postgresql ? new postgresql.PostgresqlShopeeDiscountRepository({ provider }) : new ShopeeDiscountRepository({ provider }),
  });
}
...
export function openCommerceDataAccess({ rootDir, databasePath, migrationsDir = null, providerName = "sqlite",
  ...
  if (selected === "postgres") {
    return Promise.all([
      ...
      import("../shopee-discount/postgresql-repository.mjs"),
    ]).then(async ([providerModule, schedulerModule, fileModule, auditModule, lifecycleModule, reviewModule, healthModule, advertisingModule, discountModule]) => {
      ...
      const postgresql = { ...fileModule, ...auditModule, ...lifecycleModule, ...reviewModule, ...healthModule, ...advertisingModule, ...discountModule };
