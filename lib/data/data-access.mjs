...
import { openProvider, normalizeDatabaseProvider } from "./open-provider.mjs";
import { PostgresqlSchedulerRepository } from "./postgresql/postgresql-scheduler-repository.mjs";
import { PostgresqlExportFileRepository } from "./postgresql/postgresql-file-repositories.mjs";
import { PostgresqlAuditRepository } from "./postgresql/postgresql-audit-repository.mjs";
import { PostgresqlFileLifecycleRepository } from "./postgresql/postgresql-file-lifecycle-repository.mjs";
import { PostgresqlFileReviewRepository } from "./postgresql/postgresql-file-review-repository.mjs";
import { PostgresqlShopeeHealthRepository } from "../shopee-health/postgresql-repository.mjs";
import { PostgresqlShopeeAdvertisingRepository } from "../advertising/postgresql-shopee-advertising-repository.mjs";
import { loadSharedPostgresqlConfig } from "./postgresql/shared-runtime-config.mjs";

function repositoriesFor({ provider, scheduler, postgres = false }) {
  return Object.freeze({
    ...
    exportFiles: postgres ? new PostgresqlExportFileRepository({ provider }) : new ExportFileRepository({ db: provider }),
    audit: postgres ? new PostgresqlAuditRepository({ provider }) : new SqliteAuditRepository({ provider }),
    fileLifecycle: postgres ? new PostgresqlFileLifecycleRepository({ provider }) : new FileLifecycleRepository({ db: provider }),
    fileReview: postgres ? new PostgresqlFileReviewRepository({ provider }) : new FileReviewRepository({ db: provider }),
    ...
    shopeeHealth: postgres ? new PostgresqlShopeeHealthRepository({ provider }) : new ShopeeHealthRepository({ provider }),
    shopeeAdvertising: postgres ? new PostgresqlShopeeAdvertisingRepository({ provider }) : new ShopeeAdvertisingRepository({ provider }),
  });
}
...
export function openCommerceDataAccess({ rootDir, databasePath, migrationsDir = null, providerName = "sqlite",
  ...
  if (selected === "postgres") {
    return openProvider({ providerName: selected, databasePath, postgresqlConfig, credentials, PoolClass })
      .then((provider) => {
        const scheduler = new PostgresqlSchedulerRepository({ provider });
        return dataAccess(provider, repositoriesFor({ provider, scheduler, postgres: true }));
      });
  }
...
export function openConfiguredCommerceDataAccess({ runtimeConfig, env = process.env, PoolClass } = {}) {
