...
function isReadOnlyPragma(sql) {
  const match = String(sql).match(/^\s*PRAGMA\s+(?:(?:[A-Za-z_][A-Za-z0-9_]*)\.)?([A-Za-z_][A-Za-z0-9_]*)([\s\S]*)$/i);
...
function isReadOnlyStatement(sql) {
  return /^\s*(?:SELECT|EXPLAIN)\b/i.test(String(sql)) || isReadOnlyPragma(sql);
}

function isTransactionControlStatement(sql) {
  return /^\s*(?:BEGIN|COMMIT|END|ROLLBACK|SAVEPOINT|RELEASE)\b/i.test(String(sql));
}
...
function guardedConnection(connection, state, capability) {
  ...
  SQLITE_RAW_TARGETS.set(facade, connection);
  return facade;
}
...
export class SqliteProvider extends DatabaseProvider {
  ...
  constructor({ databasePath, connection = null }) {
    ...
    this.#capability = {};
    this.#connection = rawConnection(connection) || new DatabaseSync(databasePath);
  ...
  async query(text, parameters = []) {
    ...
    const rows = this.#connection.prepare(text).all(...parameters);
