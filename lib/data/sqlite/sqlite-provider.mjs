...
export class SqliteProvider extends DatabaseProvider {
  constructor({ databasePath, connection = null, readOnly = false }) {
    super({ dialect: DATABASE_DIALECTS.SQLITE });
    if (!connection && !databasePath) throw new TypeError("SQLite database path is required");
    if (databasePath && !readOnly) mkdirSync(path.dirname(databasePath), { recursive: true });
    this.databasePath = databasePath || null;
    this.readOnly = Boolean(readOnly);
    this._connection = connection || new DatabaseSync(databasePath, { readOnly: this.readOnly });
    this.ownsConnection = !connection;
    this._connection.exec("PRAGMA busy_timeout = 5000");
    this._connection.exec("PRAGMA foreign_keys = ON");
    if (this.readOnly) this._connection.exec("PRAGMA query_only = ON");
    else if (this.ownsConnection) withSqliteBusyRetry(() => this._connection.exec("PRAGMA journal_mode = WAL"));
