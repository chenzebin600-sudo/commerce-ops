...
export class SchedulerDatabase {
  ...
  migrate() {
    ...
    for (const filename of files) {
      ...
      const foreignKeysOff = sql.includes("-- migration: foreign_keys_off");
      if (foreignKeysOff) this.db.exec("PRAGMA foreign_keys=OFF");
      let appliedNow;
      try { appliedNow = this.transactionManager.run(() => {
        const existing = this.db.prepare("SELECT 1 FROM schema_migrations WHERE version=?").get(filename);
        if (existing) return false;
        this.db.exec(sql);
        if (foreignKeysOff && this.db.prepare("PRAGMA foreign_key_check").all().length) {
          throw new Error(`Migration ${filename} left foreign-key violations`);
        }
