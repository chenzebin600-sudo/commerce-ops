...
import { SqliteProvider } from "./sqlite-provider.mjs";
...
export class SchedulerDatabase {
  constructor({ databasePath, migrationsDir, provider = null }) {
    this.provider = provider || new SqliteProvider({ databasePath });
    this.ownsProvider = !provider;
    this.databasePath = databasePath || this.provider.databasePath;
    this.migrationsDir = migrationsDir;
    this.db = this.provider.connection;
    this.transactionManager = this.provider.transactionManager;
    this.exportFiles = new ExportFileRepository({ db: this.db });
  }

  close() {
    if (this.ownsProvider) this.provider.close();
  }

  migrate() {
    this.provider.withBusyRetry(() => this.db.exec("CREATE TABLE IF NOT EXISTS schema_migrations (version TEXT PRIMARY KEY, applied_at TEXT NOT NULL)"));
    ...
    for (const filename of files) {
      ...
      const appliedNow = this.transactionManager.run(() => {
  ...
  createRun({ taskId, triggerType, scheduledRunAt, status = "pending", errorMessage = null, executionOptions = null }) {
    assertTaskNotDeleted(this.getTask(taskId));
    const id = randomUUID();
    const now = iso();
    const logSummary = executionOptions ? { executionOptions } : {};
    this.db.prepare(`INSERT INTO scheduled_export_runs
      ...
      VALUES (?,?,?,?,?,?,?,?,?)`).run(id, taskId, triggerType, iso(scheduledRunAt), status, errorMessage, JSON.stringify(logSummary), now, now);
  ...
  getActiveRunForTask(taskId) {
    const row = this.db.prepare(`SELECT r.*,t.name task_name,t.task_type task_type,t.deleted_at task_deleted_at,
      f.status file_status,f.original_filename
      FROM scheduled_export_runs r JOIN scheduled_export_tasks t ON t.id=r.task_id
      LEFT JOIN export_files f ON f.id=r.export_file_id
      WHERE r.task_id=? AND r.status IN ('pending','running') ORDER BY r.created_at LIMIT 1`).get(taskId);
    return serializeRun(row);
  }
  ...
  updateRun(id, fields) {
    ...
    const currentLogSummary = entries.some(([key]) => key === "logSummary")
      ? parseJson(this.db.prepare("SELECT log_summary_json FROM scheduled_export_runs WHERE id=?").get(id)?.log_summary_json, {})
      : null;
    const values = entries.map(([key, value]) => key === "logSummary"
      ? JSON.stringify({ ...currentLogSummary, ...(value || {}) })
      : value);
  ...
  touchRun(id, at = new Date()) {
    return this.db.prepare("UPDATE scheduled_export_runs SET updated_at=? WHERE id=? AND status='running'")
      .run(iso(at), id).changes;
  }
  ...
  recoverStaleRuns(now = new Date(), staleMs = 30 * 60 * 1000) {
    ...
    return this.db.prepare(`UPDATE scheduled_export_runs SET status='failed',finished_at=?,error_stage='scheduler',error_code='PROCESS_RESTART',error_message='服务重启时检测到未完成任务。',updated_at=?
      WHERE status='running' AND updated_at<?`).run(iso(now), iso(now), cutoff).changes;
  }
}

export function openSchedulerDatabase({
  rootDir,
  databasePath = process.env.DATABASE_PATH || process.env.SCHEDULER_DB_PATH,
} = {}) {
  const resolvedRoot = rootDir || process.cwd();
  const defaultPath = path.join(resolvedRoot, "storage", "commerce-ops.sqlite");
  const target = databasePath ? path.resolve(resolvedRoot, databasePath) : defaultPath;
  const relativeTarget = path.relative(resolvedRoot, target);
  const targetInsideProject = relativeTarget === "" || (!relativeTarget.startsWith("..") && !path.isAbsolute(relativeTarget));
  if (databasePath && targetInsideProject && target !== defaultPath && !existsSync(target) && existsSync(defaultPath)) {
    throw new Error("DATABASE_PATH points to a missing database while the default formal database exists");
  }
