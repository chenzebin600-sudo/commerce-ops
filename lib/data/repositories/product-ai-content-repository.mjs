...
export class ProductAiContentRepository {
  ...
  async create(input) {
    return this.provider.transaction(async (tx) => {
      ...
      const values = [
        ...
        input.status === "confirmed" ? input.createdBy : null, this.boolean(input.isManuallyModified),
  ...
  async markManual(productId, contentId, manualContent, { operatorLabel } = {}) {
    ...
    if (!content || content.productId !== productId) return null;
    const now = iso();
    await this.provider.execute(`UPDATE ${this.table("product_ai_contents")} SET is_manually_modified=${this.provider.dialect === DATABASE_DIALECTS.POSTGRESQL ? "true" : "1"},
