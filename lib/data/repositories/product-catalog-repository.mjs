...
export class ProductCatalogRepository {
  ...
  baseSelect() {
    return `SELECT
      ...
      (SELECT count(*) FROM ${this.table("product_images")} pi WHERE pi.sku_id=s.id AND pi.status='available') image_count,
  ...
  async get(id) {
    ...
    if (!base) return null;
    const [source, packaging, costs, inventories, lifecycleEvents, images, overrides, overrideEvents] = await Promise.all([
  ...
  async filters() {
    const [categories, lifecycle, warehouses, countries] = await Promise.all([
      ...
      this.provider.query(`SELECT DISTINCT country_raw FROM ${this.table("product_skus")}
        WHERE country_raw IS NOT NULL AND country_raw<>'' ORDER BY country_raw`),
    ]);
    return {
