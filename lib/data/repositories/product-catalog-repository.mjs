...
function serializeProduct(row) {
  ...
  return {
    ...
    sourceDatabaseValues: jsonValue(row.source_database_values_json, {}),
  };
}
...
export class ProductCatalogRepository {
  ...
  baseSelect() {
    return `SELECT
      s.id,s.source_system,s.source_sku,s.normalized_sku,s.sku_code_normalized,s.country_raw,s.source_product_name,s.source_main_sku,
      ...
      ,(SELECT count(*) FROM ${this.table("product_import_field_changes")} change
        WHERE change.import_batch_id=(SELECT sync.import_batch_id FROM ${this.table("product_package_sync_runs")} sync
          WHERE sync.import_batch_id IS NOT NULL AND sync.status='SUCCEEDED'
          ORDER BY sync.finished_at DESC,sync.id DESC LIMIT 1)
          AND change.country_raw=s.country_raw AND UPPER(change.sku_code)=s.sku_code_normalized) latest_change_count
      ,(SELECT package.raw_payload_json FROM ${this.table("product_package_rows")} package
        WHERE package.latest_import_row_id=s.current_source_row_id LIMIT 1) source_database_values_json
  ...
  async listIdentitySet() {
    const result = await this.provider.query(`SELECT id,source_sku,normalized_sku,source_main_sku
    ...
    return result.rows.map((row) => ({
      ...
      sourceMainSku: row.source_main_sku || null,
    }));
  }
  ...
  async get(id) {
    ...
    const [source, packaging, costs, inventories, lifecycleEvents, images, mabangImages, overrides, overrideEvents] = await Promise.all([
      this.provider.query(`SELECT latest_source_row_number AS source_row_number,raw_payload_json,normalized_payload_json
        FROM ${this.table("product_package_rows")}
        WHERE latest_import_row_id=(SELECT current_source_row_id FROM ${this.table("product_skus")} WHERE id=${p})
        LIMIT 1`, [id]),
      ...
      this.provider.query(`SELECT warehouse_raw,warehouse_stock,planned_warehouse_raw,captured_at,batch_id
        FROM ${this.table("product_inventory_snapshots")} WHERE sku_id=${p}
          AND batch_id=(SELECT last_seen_batch_id FROM ${this.table("product_skus")}
            WHERE id=${this.table("product_inventory_snapshots")}.sku_id)
        ORDER BY warehouse_raw`, [id]),
