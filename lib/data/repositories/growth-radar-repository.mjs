...
function batchRow(row) {
  ...
  return {
    ...
    sourceScope: json(row.source_scope_json, {}),
...
export class GrowthRadarRepository {
  ...
  async createBatch(input, client = this.provider) {
    const values = [
      ...
      JSON.stringify(input.sourceHeaders || []), JSON.stringify(input.redactedHeaders || []),
    ...
    await client.execute(`INSERT INTO ${this.table("growth_source_batches")} (
      ...
      query_started_at,query_ended_at,collected_at,imported_at,source_scope_json,source_headers_json,
      redacted_headers_json,row_count,status,error_code,created_by,created_at,updated_at
  ...
  async updateBatch(id, input, client = this.provider) {
    await client.execute(`UPDATE ${this.table("growth_source_batches")} SET status=${client.placeholder(1)},
      imported_at=${client.placeholder(2)},error_code=${client.placeholder(3)},updated_at=${client.placeholder(4)}
      WHERE id=${client.placeholder(5)}`, [input.status, input.importedAt, input.errorCode, input.updatedAt, id]);
  ...
  async insertOrderHeader(input, client = this.provider) {
    const values = [
      ...
      input.orderAmountSourceField, input.effectiveStatus, input.firstSourceBatchId, input.sourceBatchId,
    ...
    await client.execute(`INSERT INTO ${this.table("growth_order_headers")} (
      ...
      order_amount,order_amount_source_field,effective_status,first_source_batch_id,source_batch_id,
  ...
  async updateOrderHeader(input, client = this.provider) {
    const values = [
      ...
      input.orderCurrency, input.orderAmount, input.orderAmountSourceField, input.effectiveStatus,
    ...
    await client.execute(`UPDATE ${this.table("growth_order_headers")} SET
      ...
      order_amount=${client.placeholder(7)},order_amount_source_field=${client.placeholder(8)},effective_status=${client.placeholder(9)},
      source_batch_id=${client.placeholder(10)},source_quality_status=${client.placeholder(11)},last_seen_at=${client.placeholder(12)},
      updated_at=${client.placeholder(13)},revision=revision+1 WHERE id=${client.placeholder(14)}`, values);
    return this.getOrderHeaderByKey(input.businessKeyVersion, input.businessKey, client);
  }
  ...
  async insertOrderLine(input, client = this.provider) {
    const values = [
      ...
      input.sourceSku, input.normalizedSourceSku, input.platformSku, input.mappedProductId, input.mappedCountry,
    ...
    await client.execute(`INSERT INTO ${this.table("growth_order_lines")} (
      ...
      line_occurrence,dedupe_confidence,source_sku,normalized_source_sku,platform_sku,mapped_product_id,mapped_country,
  ...
  async updateOrderLine(input, client = this.provider) {
    const values = [
      input.sourceBatchId, input.sourceRowNumber, input.mappedProductId, input.mappedCountry, input.quantity,
    ...
    await client.execute(`UPDATE ${this.table("growth_order_lines")} SET
      source_batch_id=${client.placeholder(1)},source_row_number=${client.placeholder(2)},mapped_product_id=${client.placeholder(3)},
      mapped_country=${client.placeholder(4)},quantity=${client.placeholder(5)},line_amount=${client.placeholder(6)},
      line_amount_status=${client.placeholder(7)},product_name=${client.placeholder(8)},mapping_status=${client.placeholder(9)},
      effective_status=${client.placeholder(10)},is_current=1,last_seen_at=${client.placeholder(11)},updated_at=${client.placeholder(12)},
      revision=revision+1 WHERE id=${client.placeholder(13)}`, values);
    return this.getOrderLineByKey(input.sourceLineKeyVersion, input.sourceLineKey, client);
  }

  async productCandidates(normalizedSku, countryCode = null, client = this.provider) {
    const parameters = [normalizedSku];
    let where = `UPPER(TRIM(sku_code_normalized))=${client.placeholder(1)} AND archived_at IS NULL`;
    if (countryCode) {
  ...
  async getShopMapping(sourceSystem, platform, normalizedSourceShopName, client = this.provider) {
    const rows = await client.query(`SELECT m.*,s.id AS shop_id,s.internal_shop_code,s.display_name,s.platform AS shop_platform,
      s.country_code AS shop_country_code,s.country_name,s.status AS shop_status
  ...
  async insertInventorySnapshot(input, client = this.provider) {
    ...
    const values = [id, input.batchId, input.sourceRowNumber, input.sourceSku, input.normalizedSourceSku,
      ...
      input.inTransitQuantity, input.pendingShipmentQuantity, null, "unconfirmed", input.sourcePredictedDailySales,
      input.predictedDailySalesSemanticStatus, null, "unavailable", input.snapshotAt, input.mappingStatus,
      input.qualityStatus, input.createdAt];
  ...
  async observationAggregate(platform, normalizedSourceShopName, normalizedSourceSku, client = this.provider) {
    const rows = await client.query(`SELECT
      ...
      WHERE h.platform=${client.placeholder(1)} AND h.normalized_source_shop_name=${client.placeholder(2)}
        ...
        AND h.effective_status<>'invalid_cancelled'`,
  ...
  async batchDetail(id) {
    ...
    for (const [name, table] of Object.entries({
      orderRawRows: "growth_order_raw_rows", inventoryRawRows: "growth_inventory_raw_rows",
      orderHeaders: "growth_order_headers", orderLines: "growth_order_lines",
      inventorySnapshots: "growth_inventory_snapshots", mappingIssues: "growth_mapping_issues",
      qualityIssues: "growth_data_quality_issues",
    })) {
      const column = new Set(["orderHeaders", "orderLines"]).has(name) ? "source_batch_id" : (name === "mappingIssues" ? "source_batch_id" : "batch_id");
  ...
  async listShopMappings(filters = {}) {
    ...
    const rows = await this.provider.query(`SELECT m.*,s.id AS shop_id,s.internal_shop_code,s.display_name,s.platform AS shop_platform,
      s.country_code AS shop_country_code,s.country_name,s.status AS shop_status ${from} ${clause}
  ...
  async listQualityIssues(filters = {}) {
    ...
    const rows = await this.provider.query(`SELECT * FROM ${this.table("growth_data_quality_issues")} ${clause}
      ORDER BY created_at DESC,id LIMIT ${this.provider.placeholder(queryParameters.length - 1)} OFFSET ${this.provider.placeholder(queryParameters.length)}`, queryParameters);
    return {
      issues: rows.rows.map((row) => ({ id: row.id, batchId: row.batch_id, entityType: row.entity_type,
        entityId: row.entity_id || null, code: row.issue_code, severity: row.severity, message: row.message,
        sourceContext: json(row.source_context_json, {}), status: row.status, createdAt: row.created_at, resolvedAt: row.resolved_at || null })),
