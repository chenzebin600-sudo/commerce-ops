...
function serialize(row) {
  ...
  return {
    ...
    countryName: row.country_name || row.country,
    marketplaceCode: row.marketplace_code || row.marketplace || null,
    sku: row.sku,
    ...
    primaryScenarios: row.primary_scenarios || "",
    ...
    aiContextHash: row.ai_context_hash || null,
    aiAdoptions: json(row.ai_adoptions_json, {}),
    status: row.status,
...
export class ProductListingRepository {
  ...
  async upsert(input) {
    return this.provider.transaction(async (tx) => {
      ...
      const now = iso();
      const columns = [
        "country_code", "country_name", "marketplace_code", "shop_id", "shop_name", "marketplace", "platform_category_id", "platform_category_name", "listing_mode",
        "title", "subtitle", "description", "search_keywords_json", "brand", "model", "target_users", "content_language",
        "product_positioning", "content_style", "price_positioning", "primary_scenarios", "special_requirements", "forbidden_content",
        "selling_points_json", "usage_scenarios_json", "platform_attributes_json", "variants_json", "pricing_json", "media_json",
        "logistics_json", "compliance_json", "ai_context_hash", "ai_adoptions_json", "status", "validation_result_json", "updated_by", "updated_at",
      ];
      const values = [
        input.countryCode, input.countryName, input.marketplaceCode, input.shopId, input.shopName, input.marketplaceCode,
        input.platformCategoryId, input.platformCategoryName, input.listingMode,
        ...
        input.contentLanguage, input.productPositioning, input.contentStyle, input.pricePositioning, input.primaryScenarios,
        input.specialRequirements, input.forbiddenContent, JSON.stringify(input.sellingPoints), JSON.stringify(input.usageScenarios), JSON.stringify(input.platformAttributes),
        JSON.stringify(input.variants), JSON.stringify(input.pricing), JSON.stringify(input.media), JSON.stringify(input.logistics),
        JSON.stringify(input.compliance), input.aiContextHash, JSON.stringify(input.aiAdoptions), input.status,
        JSON.stringify(input.validationResult), input.updatedBy, now,
      ];
      if (existing) {
