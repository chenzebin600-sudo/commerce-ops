...
export class ProductImportRepository {
    ...
    await this.provider.transaction(async (tx) => {
      ...
      for (let offset = 0; offset < validation.rows.length; offset += 150) {
        ...
        for (const row of chunk) {
          ...
          const p = this.placeholders(tx, 15, values.length);
          groups.push(`(${p.join(",")})`);
          values.push(
            row.id, batchId, row.sourceRowNumber, row.sourceSku, row.sourceCountryRaw, row.productKey,
            row.rowHash, row.productHash, JSON.stringify(row.rawPayload), JSON.stringify(row.normalizedPayload),
            JSON.stringify(row.validationCodes), row.outcome, null, null, now,
          );
        }
        await tx.execute(`INSERT INTO ${this.table("product_import_rows")} (
          id,batch_id,source_row_number,source_sku,source_country_raw,product_key,row_sha256,product_sha256,
          raw_payload_json,normalized_payload_json,validation_codes_json,outcome,target_sku_id,applied_at,created_at
    ...
    return this.provider.transaction(async (tx) => {
      ...
      for (let offset = 0; offset < total; offset += 1000) {
        ...
        for (const source of rowsResult.rows.map(serializeRow)) {
          if (new Set(["exception", "conflict"]).has(source.outcome)) continue;
  ...
  async loadApplyCaches(tx) {
    const [categoryRows, modelRows, skuRows, lifecycleRows, packagingRows] = await Promise.all([
  ...
  async insertInventorySnapshots(tx, { skuId, batchId, inventories }) {
    ...
    for (let offset = 0; offset < rows.length; offset += 300) {
      ...
      const groups = [];
      for (const item of chunk) {
        const p = this.placeholders(tx, 7, values.length);
        groups.push(`(${p.join(",")})`);
        values.push(
          randomUUID(), skuId, batchId, String(item.warehouse_raw || "未指定").trim(),
          item.warehouse_stock ?? null, item.planned_warehouse_raw || null, now,
        );
      }
