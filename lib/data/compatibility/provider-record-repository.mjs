...
function encodeValue(value, column, dialect) {
  if (dialect === DATABASE_DIALECTS.POSTGRESQL
    ...
    return normalizeMigrationValue(value, column) ? 1 : 0;
  }
...
export class RepositoryCompatibilityError extends Error {
  constructor(code, cause, context = null) {
    super(code);
...
function normalizeError(error, context) {
  ...
  return new RepositoryCompatibilityError(errorCode(error), error, context);
}
...
export class ProviderRecordRepository {
  ...
  async insert(record) {
    ...
    try {
      ...
      throw normalizeError(error, `${this.table.name}.insert`);
    }
  }
  ...
  async get(key) {
    ...
    try {
      ...
      throw normalizeError(error, `${this.table.name}.get`);
    }
  }
  ...
  async list(criteria = null) {
    ...
    try {
      ...
      throw normalizeError(error, `${this.table.name}.list`);
    }
  }
  ...
  async update(key, changes) {
    ...
    try {
      ...
      throw normalizeError(error, `${this.table.name}.update`);
    }
  }
  ...
  async delete(key) {
    ...
    try {
      ...
      throw normalizeError(error, `${this.table.name}.delete`);
    }
  }
}
