...
export class PostgresqlProvider extends DatabaseProvider {
  constructor({ config, database, user, password, pool = null, PoolClass = null, onPoolError = null, readOnly = false }) {
    ...
    this.ownsPool = !pool;
    const ResolvedPoolClass = PoolClass || loadPoolClass();
    this._pool = pool || new ResolvedPoolClass({
      host: config.host,
      port: config.port,
      database,
      user,
  ...
  async acquire() {
    ...
    try {
      if (!this._initializedClients.has(client)) {
        await client.query("SELECT set_config('search_path', $1, false), set_config('TimeZone', 'UTC', false), set_config('statement_timeout', $2, false), set_config('default_transaction_read_only', $3, false)", [
          `${this.config.schema},public`,
          String(this.config.statementTimeoutMs),
          this.readOnly ? "on" : "off",
        ]);
...
export function createPostgresqlProvider(config, { database = "production", role = "app", pool = null, PoolClass = null } = {}) {
