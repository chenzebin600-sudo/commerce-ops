...
function validateRequestId(value) {
  if (typeof value !== "string" || value.length < 1 || value.length > 200 || /[\r\n]/.test(value)) throw invalidInput("requestId is invalid");
  return value;
}
...
function normalizeSuccess(result, fallbackRequestId, attempts) {
  ...
  const requestId = data?.request_id || payload?.request_id || fallbackRequestId || null;
  return {
...
function safeRequestId(value, fallback = null) {
...
function parseTransportResult(result, fallbackRequestId, relayPath) {
  ...
  if (status === 429 || /rate.?limit|too many requests/i.test(errorText)) {
    throw adapterError("SHOPEE_RATE_LIMITED", "Shopee rate limit reached", { requestId, status });
  }
  if (status >= 500) throw adapterError("SHOPEE_UNAVAILABLE", "Shopee relay is unavailable", { requestId, status });
