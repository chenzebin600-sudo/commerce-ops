import { AUTH_CODES, platformErrorCode, READ_RATE_CODES, safeRequestId, TECHNICAL_CODES, validHttpStatus } from "./response-boundary.mjs";
...
function parseTransportResult(result, fallbackRequestId, relayPath) {
  ...
  if (validHttpStatus(status) && (status === 401 || status === 403)) {
    let authPayload = result?.body;
    if (typeof authPayload === "string") {
      try { authPayload = JSON.parse(authPayload); } catch { authPayload = null; }
    }
    const authData = authPayload?.data ?? authPayload;
    const authRequestId = safeRequestId(authData?.request_id, safeRequestId(authPayload?.request_id, fallbackRequestId));
    throw adapterError("SHOPEE_AUTH_ERROR", "Shopee authentication failed", { requestId: authRequestId, status });
  }
  if (validHttpStatus(status) && status === 429) throw adapterError("SHOPEE_RATE_LIMITED", "Shopee rate limit reached", { requestId: fallbackRequestId, status });
  if (validHttpStatus(status) && status >= 500) throw adapterError("SHOPEE_UNAVAILABLE", "Shopee relay is unavailable", { requestId: fallbackRequestId, status });
  ...
  if (READ_RATE_CODES.has(platformCode)) {
    throw adapterError("SHOPEE_RATE_LIMITED", "Shopee rate limit reached", { requestId, status });
  }
  if (TECHNICAL_CODES.has(platformCode)) throw adapterError("SHOPEE_UNAVAILABLE", "Shopee relay is unavailable", { requestId, status });
  if (AUTH_CODES.has(platformCode)) {
    throw adapterError("SHOPEE_AUTH_ERROR", "Shopee authentication failed", { requestId, status });
  }
  if (platformCode) throw adapterError("SHOPEE_BUSINESS_ERROR", "Shopee rejected the read request", { requestId, status });
  if (/rate.?limit|too many requests/i.test(errorText)) throw adapterError("SHOPEE_RATE_LIMITED", "Shopee rate limit reached", { requestId, status });
  if (/auth|token|permission|forbidden|unauthori[sz]ed/i.test(errorText)) throw adapterError("SHOPEE_AUTH_ERROR", "Shopee authentication failed", { requestId, status });
