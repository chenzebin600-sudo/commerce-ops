...
export function createProductionShardAccumulator({ shardSize, flushShard }) {
  ...
  return {
    add(item) { buffer.push(item); itemCount += 1; maxBuffered = Math.max(maxBuffered, buffer.length); return buffer.length === shardSize ? flush() : null; },
    flush,
