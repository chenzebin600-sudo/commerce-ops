...
export class ShopeeDiscountScheduler {
  constructor({
    ...
    if (!repository?.createDueJob || !repository?.claimDueJobs || !repository?.renewDueJobLease || !repository?.completeDueJob) {
  ...
  async enqueueDailyScan({ country, shopIds, timeZone = null }) {
    const settings = await this.repository.getSettings?.();
    const zone = timeZone || settings?.timezone;
    if (!zone) throw schedulerError("SHOPEE_DISCOUNT_TIMEZONE_INVALID", "Daily scheduling requires a configured IANA timezone");
    const day = localDay(this.now(), zone);
    const normalizedShops = [...new Set((shopIds || []).map(String))].sort();
    return this.#createOnce({
      jobType: "DAILY_SCAN", dedupeKey: `daily:${country}:${normalizedShops.join(",")}:${zone}:${day}`,
      dueAt: this.now(), payload: { country, shopIds: normalizedShops, timeZone: zone, logicalDay: day },
    });
  }
  ...
  async #foundationTask(job, draft = null) {
    ...
    return this.foundation.tasks.create({
      domain: "product",
  ...
  async tick({ enqueueDaily = true } = {}) {
    ...
    if (enqueueDaily) {
      ...
      if (settings?.enabled && scope?.country && Array.isArray(scope.shopIds) && scope.shopIds.length) {
        await this.enqueueDailyScan({ ...scope, timeZone: settings.timezone });
      }
    }
