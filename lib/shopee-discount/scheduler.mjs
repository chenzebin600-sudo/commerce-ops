...
import { isAllowedWarehouseBaseUrl } from "./warehouse-client.mjs";
...
export function resolveShopeeDiscountSchedulerStartup(env = {}) {
  ...
  try {
    ...
    if (!isAllowedWarehouseBaseUrl(parsed.href)) throw new Error("protocol");
...
export class ShopeeDiscountScheduler {
  ...
  async #handle(job) {
    ...
    if (["DAILY_SCAN", "MANUAL_SCAN"].includes(job.jobType)) {
      ...
      const scan = await this.scan({ ...job.payload, triggerType: job.jobType === "DAILY_SCAN" ? "daily" : "manual", dueJobId: job.id });
      if (!this.foundation?.tasks?.create) return scan;
      const planId = scan?.correctionPlanId || scan?.planId || null;
      const task = await this.#foundationTask(job, { planId });
      let notification = { status: "SKIPPED", reason: "not_configured" };
      if (planId && this.notifications?.sendReminder) notification = await this.notifications.sendReminder({
        dueJobId: job.id, foundationTaskId: task.id, planId,
        shopId: job.payload.shopId || job.payload.shopIds?.[0] || null, severity: "INFO",
      });
      return { scan, foundationTaskId: task.id, notification };
    }
