...
function baselineKeys(baseline) {
  ...
  return new Set(baseline.rows.filter((row) => row && typeof row === "object").map(rowKey));
}
...
export function validateWarehouseSnapshot(snapshot, baseline, policy = {}, { now = new Date() } = {}) {
  ...
  for (const row of snapshot.rows) {
    ...
    if (row.status !== "ACTIVE") return block("WAREHOUSE_SCHEMA_INVALID", snapshot.rows, [], evidence);
    const key = rowKey(row);
    if (seen.has(key)) return block("WAREHOUSE_DUPLICATE_SKU", snapshot.rows, [], evidence);
    seen.add(key);
  }

  const prior = baselineKeys(baseline);
