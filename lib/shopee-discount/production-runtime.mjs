...
export function createManualExecutionRuntime({ repository, foundation, shopeeRead, shopeeWrite, shopeeWrites = null, warehouse, writeSecurity,
  currentPolicyHash, siteCapability, siteCapabilities = null, runPlan = runApprovedPlan, now = () => new Date() }) {
  if (!repository?.getPlan || !foundation || (!shopeeWrite && !shopeeWrites) || typeof writeSecurity !== "function"
    || !currentPolicyHash || (!siteCapability && !siteCapabilities) || typeof runPlan !== "function") {
    throw new TypeError("Complete manual execution runtime is required");
  }
  ...
  return async (planId, { context } = {}) => {
    ...
    const country = String(plan?.country || "").toUpperCase();
    const selectedWrite = shopeeWrites?.[country] || shopeeWrite;
    const selectedCapability = siteCapabilities?.[country] || siteCapability;
    if (!country || !selectedWrite || !selectedCapability) {
      throw Object.assign(new Error("No execution capability is configured for the plan country"), { code: "SHOPEE_SITE_CAPABILITY_MISSING" });
    }
    return runPlan(planId, { repository, foundation, readers, shopeeWrite: selectedWrite, writeSecurity, currentPolicyHash, siteCapability: selectedCapability,
