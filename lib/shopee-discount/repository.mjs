...
export class ShopeeDiscountRepository {
  ...
  async getLatestWarehouseBaseline({ country, category, tier, shopId = null }) {
    const rows = shopId
      ? this.db.prepare("SELECT evidence_json FROM shopee_discount_events WHERE event_type='WAREHOUSE_BASELINE' AND json_extract(evidence_json,'$.scope.shopId')=? ORDER BY occurred_at DESC,id DESC LIMIT 20").all(shopId)
      : this.db.prepare("SELECT evidence_json FROM shopee_discount_events WHERE event_type='WAREHOUSE_BASELINE' ORDER BY occurred_at DESC,id DESC LIMIT 500").all();
    for (const row of rows) {
      const value = json(row.evidence_json, null);
      if (value?.scope?.country === country && value.scope.category === category && value.scope.tier === tier
        && (!shopId || value.scope.shopId === shopId)) return value;
    }
    return null;
  }
