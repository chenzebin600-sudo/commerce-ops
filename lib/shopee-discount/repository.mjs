...
export class ShopeeDiscountRepository {
  ...
  async updatePreviewActivity({ planId, shopId, activityType, platformActivityId = null, metadata = {} }) {
    ...
    const result = this.db.prepare(`UPDATE shopee_discount_activities SET activity_type=?,platform_activity_id=?,metadata_json=?,updated_at=?
      ...
      .run(text(activityType, "activityType"), platformActivityId == null ? null : text(platformActivityId, "platformActivityId"),
        JSON.stringify(metadata), timestamp, text(planId, "planId"), text(shopId, "shopId"), planId);
  ...
  async finalizePreviewMetadata({ planId, sourceSnapshotHash, summary }) {
    const result = this.db.prepare(`UPDATE shopee_discount_plans SET source_snapshot_hash=?,summary_json=?,updated_at=?
      WHERE id=? AND state='PREVIEWING'`).run(text(sourceSnapshotHash, "sourceSnapshotHash"), JSON.stringify(summary || {}),
        iso(this.now()), text(planId, "planId"));
  ...
  async claimPreviewOwnership({ planId, expectedOwnerToken, ownerToken, leaseUntil }) {
    return this.provider.transactionManager.run(() => {
      ...
      const result = this.db.prepare(`UPDATE shopee_discount_plans SET summary_json=?,updated_at=?
        WHERE id=? AND state='PREVIEWING' AND summary_json=?`).run(JSON.stringify(summary), iso(this.now()), planId, row.summary_json);
  ...
  async appendPlanShard({ planId, shardIndex, shardHash, items = [] }) {
    ...
    return this.provider.transactionManager.run(() => {
      ...
      if (existingShard) {
        const stored = this.db.prepare(`SELECT sequence_no,shop_id,item_id,model_id,payload_hash FROM shopee_discount_plan_items
