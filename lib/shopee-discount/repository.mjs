...
function requirePreviewOwner(row, ownerToken, ownerEpoch) {
  if (row.preview_owner_token == null) return;
  if (row.preview_owner_token !== ownerToken || Number(row.preview_owner_epoch) !== Number(ownerEpoch)) {
...
function settingsRow(row) {
  ...
  return {
    ...
    warehouseKeyUpdatedAt: row.warehouse_key_updated_at, credentialGeneration: Number(row.credential_generation || 0),
...
function planRow(row) {
  ...
  if (Object.hasOwn(row, "preview_owner_token")) {
    ...
    summary.previewOwnerLeaseUntil = row.preview_owner_lease_until;
  }
  return {
    id: row.id,
...
function notificationRow(row) {
  ...
  return {
    ...
    lastErrorCode: row.last_error_code, metadata: json(row.metadata_json), readAt: row.read_at,
    deliveryLeaseUntil: row.delivery_lease_until, coordinationState: row.coordination_state,
    coordinationEvidence: json(row.coordination_evidence_json),
    deliveredAt: row.delivered_at, retentionUntil: row.retention_until, createdAt: row.created_at, updatedAt: row.updated_at,
  };
}
...
function eventRow(row) {
  if (!row) return null;
  return { id: row.id, planId: row.plan_id, jobId: row.job_id, intentId: row.intent_id || null, eventType: row.event_type,
...
export class ShopeeDiscountRepository {
  ...
  async getStorageMode() {
    return { dialect: "sqlite", productionScale: false, pilotLimits: null };
  }
  ...
  async saveSettings(input = {}, audit = {}) {
    ...
    return this.provider.transactionManager.run(() => {
      ...
      const metadata = keyChanged ? {} : Object.hasOwn(input, "metadata")
      ...
      this.db.prepare(`UPDATE shopee_discount_settings SET
        encrypted_warehouse_key_ciphertext=?,warehouse_key_reference=?,warehouse_key_hint=?,warehouse_key_updated_at=?,credential_generation=?,
        ...
        Number(current.credential_generation || 0) + (keyChanged ? 1 : 0),
        pick("timezone", "timezone"), Object.hasOwn(input, "enabled") ? Number(Boolean(input.enabled)) : current.enabled,
        JSON.stringify(metadata), audit.actorId || audit.actorName || current.updated_by || null, timestamp,
      );
      if (keyChanged) this.db.prepare(`UPDATE shopee_discount_plans SET state='BLOCKED',reason_code='SHOPEE_DISCOUNT_SECURITY_SETTINGS_CHANGED',state_version=state_version+1,updated_at=?
        WHERE state IN ('PREVIEWING','PREVIEWED','APPROVED')`).run(timestamp);
      return settingsRow(this.db.prepare("SELECT * FROM shopee_discount_settings WHERE id='default'").get());
    });
  }
  ...
  async markSettingsVerified({ expected, metadata }, audit = {}) {
    ...
    const timestamp = iso(audit.occurredAt || this.now());
    const result = this.db.prepare(`UPDATE shopee_discount_settings SET metadata_json=?,updated_by=?,updated_at=? WHERE id='default'
      AND credential_generation=? AND encrypted_warehouse_key_ciphertext IS ? AND warehouse_key_reference IS ? AND warehouse_key_updated_at IS ?`).run(
    ...
    return result.changes ? this.getSettings() : null;
  }

  async invalidateActivePlans({ reasonCode }) {
    const timestamp = iso(this.now());
    const result = this.db.prepare(`UPDATE shopee_discount_plans SET state='BLOCKED',reason_code=?,state_version=state_version+1,updated_at=?
      WHERE state IN ('PREVIEWING','PREVIEWED','APPROVED')`).run(text(reasonCode, "reasonCode"), timestamp);
    return { invalidatedCount: Number(result.changes) };
  }
  ...
  async createPlan(input) {
    ...
    return this.provider.transactionManager.run(() => {
      const existing = this.db.prepare("SELECT * FROM shopee_discount_plans WHERE id=?").get(id);
      if (existing) return planRow(existing);
      const expireDraft = this.db.prepare(`UPDATE shopee_discount_plans
        SET state='EXPIRED',reason_code='SHOPEE_DISCOUNT_PREVIEW_TTL_EXPIRED',state_version=state_version+1,updated_at=?
        WHERE id=? AND state='PREVIEWED' AND expires_at IS NOT NULL AND expires_at<=?`);
      const endDraftActivities = this.db.prepare(`UPDATE shopee_discount_activities SET status='ENDED',updated_at=?
        WHERE plan_id=? AND status IN ('PLANNED','ACTIVE')`);
      const expiredDrafts = this.db.prepare(`SELECT DISTINCT p.id FROM shopee_discount_plans p
        JOIN shopee_discount_activities a ON a.plan_id=p.id
        WHERE a.shop_id=? AND a.target_starts_at<? AND a.target_ends_at>?
          AND p.state='PREVIEWED' AND p.expires_at IS NOT NULL AND p.expires_at<=?`);
      ...
      for (const activity of activities) {
        for (const draft of expiredDrafts.all(activity.shopId, activity.targetEndsAt, activity.targetStartsAt, timestamp)) {
          if (expireDraft.run(timestamp, draft.id, timestamp).changes) endDraftActivities.run(timestamp, draft.id);
        }
      ...
      this.db.prepare(`INSERT INTO shopee_discount_plans (
        ...
        JSON.stringify(input.summary || {}), timestamp, timestamp, owner.previewOwnerToken || null,
        Number(owner.previewOwnerEpoch || (owner.previewOwnerToken ? 1 : 0)), owner.previewOwnerLeaseUntil || null,
      );
  ...
  async bindFoundationPlan(planId, foundationPlanId, { ownerToken = null, ownerEpoch = 0 } = {}) {
    const result = this.db.prepare(`UPDATE shopee_discount_plans SET foundation_plan_id=?,updated_at=? WHERE id=? AND state='PREVIEWING'
      AND (foundation_plan_id IS NULL OR foundation_plan_id=?) AND (preview_owner_token IS NULL OR
      (preview_owner_token=? AND preview_owner_epoch=?))`)
      .run(foundationPlanId, iso(this.now()), planId, foundationPlanId, ownerToken, ownerEpoch);
  ...
  async updatePreviewActivity({ planId, shopId, activityType, platformActivityId = null, metadata = {}, ownerToken = null, ownerEpoch = 0 }) {
    const timestamp = iso(this.now());
    const result = this.db.prepare(`UPDATE shopee_discount_activities SET activity_type=?,platform_activity_id=?,metadata_json=?,updated_at=?
      WHERE plan_id=? AND shop_id=? AND EXISTS (SELECT 1 FROM shopee_discount_plans p WHERE p.id=? AND p.state='PREVIEWING'
        AND (p.preview_owner_token IS NULL OR (p.preview_owner_token=? AND p.preview_owner_epoch=?)))`)
      .run(text(activityType, "activityType"), platformActivityId == null ? null : text(platformActivityId, "platformActivityId"),
        JSON.stringify(metadata), timestamp, text(planId, "planId"), text(shopId, "shopId"), planId, ownerToken, ownerEpoch);
    if (!result.changes) throw error("SHOPEE_DISCOUNT_PLAN_IMMUTABLE", "Preview activity can only be configured while PREVIEWING");
    return this.getPlanActivity(planId, shopId);
  }

  async finalizePreviewMetadata({ planId, sourceSnapshotHash, summary, ownerToken = null, ownerEpoch = 0 }) {
    return this.provider.transactionManager.run(() => {
      const id = text(planId, "planId");
      const current = this.db.prepare("SELECT summary_json FROM shopee_discount_plans WHERE id=?").get(id);
      const merged = { ...json(current?.summary_json, {}), ...(summary || {}) };
      const result = this.db.prepare(`UPDATE shopee_discount_plans SET source_snapshot_hash=?,summary_json=?,updated_at=?
        WHERE id=? AND state='PREVIEWING' AND (preview_owner_token IS NULL OR
          (preview_owner_token=? AND preview_owner_epoch=?))`).run(text(sourceSnapshotHash, "sourceSnapshotHash"), JSON.stringify(merged),
          iso(this.now()), id, ownerToken, ownerEpoch);
      if (!result.changes) throw error("SHOPEE_DISCOUNT_PLAN_IMMUTABLE", "Preview metadata can only be finalized while PREVIEWING");
      return planRow(this.db.prepare("SELECT * FROM shopee_discount_plans WHERE id=?").get(id));
    });
  }
  ...
  async claimPreviewOwnership({ planId, expectedOwnerToken, expectedOwnerEpoch = 0, expectedLeaseUntil = null, ownerToken, leaseUntil, now = this.now() }) {
    return this.provider.transactionManager.run(() => {
      const row = this.db.prepare("SELECT * FROM shopee_discount_plans WHERE id=?").get(planId);
      ...
      if ((row.preview_owner_token || null) !== (expectedOwnerToken || null)
        ...
        || (row.preview_owner_lease_until || null) !== (expectedLeaseUntil || null)) return null;
      const takeover = row.preview_owner_token !== ownerToken;
      if (takeover && row.preview_owner_lease_until && row.preview_owner_lease_until > iso(now)) return null;
      ...
      const result = this.db.prepare(`UPDATE shopee_discount_plans SET summary_json=?,preview_owner_token=?,preview_owner_epoch=?,
        preview_owner_lease_until=?,updated_at=? WHERE id=? AND state='PREVIEWING' AND preview_owner_token IS ?
        AND preview_owner_epoch=? AND preview_owner_lease_until IS ?`).run(JSON.stringify(summary), ownerToken, epoch, iso(leaseUntil),
        iso(this.now()), planId, expectedOwnerToken || null, Number(expectedOwnerEpoch || 0), expectedLeaseUntil || null);
      return result.changes ? planRow(this.db.prepare("SELECT * FROM shopee_discount_plans WHERE id=?").get(planId)) : null;
    });
  }
  ...
  async getLatestWarehouseBaseline({ country, category, tier, shopId = null }) {
    const row = this.db.prepare(`SELECT evidence_json FROM shopee_discount_events
      ...
      .get(country, category, shopId, tier);
  ...
  async appendPlanShard({ planId, shardIndex, shardHash, items = [], ownerToken = null, ownerEpoch = 0 }) {
    ...
    return this.provider.transactionManager.run(() => {
      ...
      requirePreviewOwner(plan, ownerToken, ownerEpoch);
      ...
      const existingShard = this.db.prepare("SELECT * FROM shopee_discount_plan_shards WHERE plan_id=? AND shard_index=?").get(planId, shardIndex);
      ...
      if (existingShard) {
        if (existingShard.content_hash != null) {
          ...
          return { planId, shardIndex, shardHash, itemCount: items.length, createdAt: existingShard.created_at };
        }
        const stored = this.db.prepare(`SELECT sequence_no,shop_id,item_id,model_id,item_key,sku,currency,scale,current_price_minor,
          control_price_minor,target_price_minor,payload_hash,payload_json FROM shopee_discount_plan_items
          WHERE plan_id=? AND shard_index=? ORDER BY sequence_no`).all(planId, shardIndex);
      ...
      this.db.prepare(`INSERT INTO shopee_discount_plan_shards
        (id,plan_id,shard_index,shard_hash,item_count,created_at,content_hash) VALUES (?,?,?,?,?,?,?)`).run(
  ...
  async markPlanState({ planId, fromState, toState, expectedVersion, reasonCode = null, ownerToken = null, ownerEpoch = 0 }) {
    ...
    const timestamp = iso(this.now());
    return this.provider.transactionManager.run(() => {
      const ownerClause = fromState === "PREVIEWING" ? " AND (preview_owner_token IS NULL OR (preview_owner_token=? AND preview_owner_epoch=?))" : "";
      ...
      const result = this.db.prepare(`UPDATE shopee_discount_plans SET state=?,reason_code=?,state_version=state_version+1,updated_at=?
        WHERE id=? AND state=? AND state_version=?${ownerClause}`).run(...values);
  ...
  async listUnknownDispatchIntentsPage({ cursor = null, pageSize = 100 } = {}, authorizedShopIds = null) {
    ...
    if (cursor) { clauses.push("(i.dispatched_at>? OR (i.dispatched_at=? AND i.id>?))"); values.push(cursor.dispatchedAt, cursor.dispatchedAt, cursor.id); }
    if (authorizedShopIds) {
      if (!authorizedShopIds.length) return { items: [], nextCursor: null };
      clauses.push(`NOT EXISTS (SELECT 1 FROM shopee_discount_activities a WHERE a.plan_id=i.plan_id AND a.shop_id NOT IN (${authorizedShopIds.map(() => "?").join(",")}))`);
      values.push(...authorizedShopIds);
    }
    const limit = Math.max(1, Math.min(100, Number(pageSize) || 50));
    const rows = this.db.prepare(`SELECT i.* FROM shopee_discount_dispatch_intents i WHERE ${clauses.join(" AND ")}
      ORDER BY i.dispatched_at,i.id LIMIT ?`).all(...values, limit + 1);
    const page = rows.slice(0, limit).map(intentRow), last = page.at(-1);
    return { items: page, nextCursor: rows.length > limit ? { dispatchedAt: last.dispatchedAt, id: last.id } : null };
  }
  ...
  async recordIntentOutcome({ intentId, jobId, ownerId, epoch, intentStatus, itemStatus, reasonCode = null,
    ...
    return this.provider.transactionManager.run(() => {
      ...
      this.db.prepare(`UPDATE shopee_discount_dispatch_intents SET status=?,platform_object_id=?,readback_json=?,evidence_json=?,
        completed_at=CASE WHEN ? IN ('SUCCEEDED','REJECTED') THEN ? ELSE completed_at END,updated_at=? WHERE id=?`).run(
  ...
  async appendEvent(input) {
    ...
    this.db.prepare(`INSERT INTO shopee_discount_events (
      ...
      input.retentionUntil ? iso(input.retentionUntil) : null, timestamp, baselineScope.country || null,
  ...
  async deferDueJob({ dueJobId, ownerId, epoch, dueAt, result = {}, lastErrorCode = null }) {
    ...
    const changed = this.db.prepare(`UPDATE shopee_discount_due_jobs SET status='PENDING',due_at=?,owner_id=NULL,lease_until=NULL,
      result_json=?,last_error_code=?,updated_at=? WHERE id=? AND status='CLAIMED' AND owner_id=? AND fencing_epoch=?`).run(
  ...
  async renewDueJobLease({ dueJobId, ownerId, epoch, leaseMs = 60_000 }) {
    ...
    const result = this.db.prepare(`UPDATE shopee_discount_due_jobs SET lease_until=?,updated_at=?
      WHERE id=? AND status='CLAIMED' AND owner_id=? AND fencing_epoch=? AND lease_until>?`).run(
      leaseUntil, timestamp, text(dueJobId, "dueJobId"), text(ownerId, "ownerId"), Number(epoch), timestamp,
    );
    return Boolean(result.changes);
  }
  ...
  async createNotification(input) {
    const id = input.id || randomUUID();
    const timestamp = iso(input.createdAt || this.now());
    this.db.prepare(`INSERT INTO shopee_discount_notifications (
      ...
      JSON.stringify(input.metadata || {}), input.retentionUntil ? iso(input.retentionUntil) : null, timestamp, timestamp,
    );
    return notificationRow(this.db.prepare("SELECT * FROM shopee_discount_notifications WHERE id=?").get(id));
  }

  async getNotificationByDedupeKey(dedupeKey) {
    return notificationRow(this.db.prepare("SELECT * FROM shopee_discount_notifications WHERE dedupe_key=?").get(text(dedupeKey, "dedupeKey")));
  }
  ...
  async claimNotificationDelivery({ notificationId, expectedAttemptCount }) {
    ...
    const result = this.db.prepare(`UPDATE shopee_discount_notifications
      SET delivery_status='SENDING',attempt_count=attempt_count+1,delivery_lease_until=?,updated_at=?
      WHERE id=? AND attempt_count=? AND delivery_status IN ('PENDING','RETRY_WAIT')`).run(leaseUntil, timestamp, text(notificationId, "notificationId"), attempt);
    if (!result.changes) return null;
    return notificationRow(this.db.prepare("SELECT * FROM shopee_discount_notifications WHERE id=?").get(notificationId));
  }
  ...
  async markNotificationDelivery({ notificationId, patch = {} }) {
    ...
    const result = this.db.prepare(`UPDATE shopee_discount_notifications SET delivery_status=?,attempt_count=?,last_error_code=?,
      delivered_at=?,delivery_lease_until=NULL,updated_at=? WHERE id=? AND delivery_status='SENDING'`).run(
  ...
  async markNotificationUnknown({ notificationId, reasonCode = "DINGTALK_DELIVERY_UNKNOWN", evidence = {} }) {
    const timestamp = iso(this.now());
    const result = this.db.prepare(`UPDATE shopee_discount_notifications SET delivery_status='FAILED',coordination_state='UNKNOWN',
      coordination_evidence_json=?,last_error_code=?,delivery_lease_until=NULL,updated_at=?
      WHERE id=? AND delivery_status='SENDING' AND delivery_lease_until<=?`).run(
      JSON.stringify(evidence), text(reasonCode, "reasonCode"), timestamp, text(notificationId, "notificationId"), timestamp,
    );
    if (!result.changes) return null;
    return notificationRow(this.db.prepare("SELECT * FROM shopee_discount_notifications WHERE id=?").get(notificationId));
  }
