...
function trustedListener(listener) {
  if (listener?.trustedTopology === true && validHostname(listener.host)) return true;
...
function validHostname(host) {
  if (typeof host !== "string" || host.length < 1 || host.length > 253 || host.startsWith(".") || host.endsWith(".")) return false;
...
function explicitlyTrustedPrivateHttpRelay(env, relay, mode) {
  if (mode !== "trusted_single_role" || !isTrue(env.SHOPEE_WRITE_ALLOW_PRIVATE_HTTP)) return false;
  const rawUrl = String(relay?.url || "");
  const match = /^http:\/\/([^:/?#]+)(?::\d+)?(?:[/?#]|$)/.exec(rawUrl);
  if (!match || !isPrivateIpv4(match[1])) return false;
  try {
    const parsed = new URL(rawUrl);
    return parsed.protocol === "http:" && !parsed.username && !parsed.password && parsed.hostname === match[1];
  } catch {
    return false;
  }
}
...
export function resolveShopeeWriteSecurity({ env = {}, listener = {}, relay = {} } = {}) {
  ...
  const privateHttpRelay = explicitlyTrustedPrivateHttpRelay(env, relay, mode);
  const batchCap = Number(env.SHOPEE_WRITE_MAX_BATCH_ITEMS);
  const countryShopOnly = env.SHOPEE_WRITE_AUTHORIZATION_DIMENSIONS === "country,shop";
  ...
  const checks = {
    ...
    batchCapConfigured: countryShopOnly || (Number.isSafeInteger(batchCap) && batchCap > 0 && batchCap <= 50),
    transportSecure: transport.https || transport.mtls || transport.signedRequests || privateHttpRelay,
  ...
  return Object.freeze({ ...result(mode, privilegedApprovalRequired, transport, checks, reasonCode), constraints: Object.freeze({
    countries, shops, maxBatchItems: countryShopOnly ? null : (checks.batchCapConfigured ? batchCap : null),
  }) });
}
...
export function assertShopeeWriteAuthorized(security, context = {}) {
  ...
  if (!new Set(["preview", "edit", "approve", "execute_plan", "execute"]).has(context.action)) {
  ...
  if (context.action === "execute") {
    ...
    if (!constraints?.countries?.includes(context.country)
      ...
      || (constraints.maxBatchItems != null && (!Number.isSafeInteger(context.batchSize)
        || context.batchSize < 1
  ...
  if (new Set(["execute_plan", "execute"]).has(context.action) && context.approvalIdentity !== PRIVILEGED_IDENTITY) {
