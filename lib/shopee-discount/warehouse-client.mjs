...
export class WarehouseControlPriceClient {
  ...
  async scanPrices({ country, category, skus, watermark, requestId } = {}) {
    ...
    const cacheKey = requestedSkus.length > 1 ? `${requestId}\u001f${country}\u001f${category ?? ""}` : null;
    ...
    for (let pageIndex = 0; pageIndex < this.maxPages; pageIndex += 1) {
      ...
      let normalized;
      try {
        normalized = parsed.rows.map((row) => this.#normalizeRow(row, parsed.watermark));
      } catch {
        return blocked("WAREHOUSE_SCHEMA_INVALID", { requestId, scope, pageCount: pageIndex + 1 });
      }
  ...
  #normalizeRow(source, sourceWatermark) {
    ...
    let rowWatermark = null;
    for (const alias of this.aliases.watermark) if (Object.hasOwn(source, alias)) { rowWatermark = source[alias]; break; }
    const watermark = rowWatermark ?? sourceWatermark;
    ...
    if (new Date(watermark).toISOString() !== new Date(sourceWatermark).toISOString()) throw new TypeError("row watermark differs from source watermark");
    return {
      sku,
      country,
      category,
