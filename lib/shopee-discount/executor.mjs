...
async function assertApprovalRoot(plan, repository) {
  ...
  let shardCursor = -1;
  for (;;) {
    const page = await repository.listPlanShardsPage(plan.id, { cursor: shardCursor, pageSize: 100 });
    shards.push(...page.items);
    if (page.nextCursor == null) break;
    shardCursor = page.nextCursor;
  }
  const rebuiltHashes = [];
  let cursor = -1;
  let itemCount = 0;
    ...
    if (foundationPlan.scope?.workflow === "NEXT_RENEWAL") {
      ...
      for (;;) {
        ...
        for (;;) {
          ...
          if (batchPreflight.status === "READY") {
            readyItems += shopPage.items.length;
            validationSample = shopPage.items;
          } else {
            const affected = batchPreflight.evidence?.planItemId
              ? shopPage.items.filter((item) => item.id === batchPreflight.evidence.planItemId) : shopPage.items;
            for (const affectedItem of affected) await markItem(context.repository, lease, {
              jobId: job.id, planItemId: affectedItem.id, ownerId: context.workerId, epoch: claim.epoch,
              status: batchPreflight.status, reasonCode: batchPreflight.reasonCode,
              evidence: { ...batchPreflight.evidence, requestId: context.requestId, renewalBatchPreflight: true },
            });
            readyItems += shopPage.items.length - affected.length;
            const unaffected = shopPage.items.filter((item) => !affected.some(({ id }) => id === item.id));
            if (unaffected.length) validationSample = unaffected;
          }
