...
async function readAllPlanItems(repository, planId) {
  const items = [];
  let cursor = -1;
  for (;;) {
    const page = await repository.listPlanItems(planId, { cursor, pageSize: 100 });
...
async function assertApprovalRoot(plan, repository) {
  const [items, shards] = await Promise.all([
    readAllPlanItems(repository, plan.id),
    repository.listPlanShards(plan.id),
  ]);
  const shardSize = shards[0]?.itemCount || Math.max(1, items.length);
  const rebuilt = buildApprovalRoot(items.map((item) => approvalItem(plan, item)), { shardSize });
...
function exactReadback(readback, item, activityId) {
  return Boolean(readback
    ...
    && String(readback.platformObjectId || activityId) === String(activityId));
}
...
async function processItem({ plan, job, item, activity, approval, context, lease, epoch }) {
  ...
  if (!warehouse || warehouse.targetPriceMinor !== item.targetPriceMinor
    || warehouse.watermark !== item.payload.warehouseWatermark
    || (item.payload.warehouseApprovedAt && warehouse.approvedAt !== item.payload.warehouseApprovedAt)) {
  ...
  const activityId = activity?.platformActivityId || item.payload.activity?.discountId || discount?.activityId;
  if (!activityId) {
  ...
  try {
    ...
    const readback = await context.readers.readbackIntent({
    ...
    const itemStatus = cause?.code === "SHOPEE_AUTH_ERROR" ? "AUTH_BLOCKED"
      : cause?.code === "SHOPEE_BUSINESS_ERROR" ? "REJECTED" : "UNKNOWN";
    const recorded = await context.repository.recordIntentOutcome({
      intentId: intent.id,
      jobId: job.id,
      ownerId: context.workerId,
      ...
      evidence: { operationUuid, requestId: context.requestId, code: cause?.code || "SHOPEE_WRITE_UNKNOWN" },
    });
    if (!recorded) throw executionError("SHOPEE_DISCOUNT_LEASE_LOST", "Ambiguous write outcome lost fencing ownership");
    if (itemStatus === "AUTH_BLOCKED") await appendAuthIssue(context.repository, {
      planId: plan.id, jobId: job.id, shopId: item.shopId, requestId: context.requestId,
      actorId: context.workerId, intentId: intent.id,
    });
    return itemStatus === "AUTH_BLOCKED"
      ? { stopShop: true, status: "AUTH_BLOCKED", reasonCode: "SHOPEE_AUTH_ERROR" }
      : { stopShop: false };
  }
}
...
async function ensureRenewalActivity({ plan, job, activity, context, lease, epoch }) {
  ...
  const existing = await context.readers.findActivityByMarker({ plan, activity, requestId: context.requestId });
  if (existing) {
  ...
  try {
    ...
    if (cause?.code === "SHOPEE_DISCOUNT_LEASE_LOST") throw cause;
    const status = cause?.code === "SHOPEE_AUTH_ERROR" ? "AUTH_BLOCKED"
      : cause?.code === "SHOPEE_BUSINESS_ERROR" ? "REJECTED" : "UNKNOWN";
    const recorded = await context.repository.recordIntentOutcome({
      intentId: intent.id, jobId: job.id, ownerId: context.workerId, epoch,
      intentStatus: "UNKNOWN", itemStatus: status, reasonCode: cause?.code || "SHOPEE_WRITE_UNKNOWN",
      evidence: { operationUuid, requestId: context.requestId, code: cause?.code || "SHOPEE_WRITE_UNKNOWN" },
    });
    ...
    return { activity, status, reasonCode: cause?.code || "SHOPEE_WRITE_UNKNOWN" };
  }
}
...
async function repairTerminalCompletion({ plan, job, context }) {
  ...
  if (foundationPlan.state === "IN_FLIGHT") {
    await context.foundation.operationPlans.finish(foundationPlan.id, foundationOutcome, {
      result: job.result,
      actorId: context.workerId,
      evidence: { domainState: domainPlan.state, completionRepair: true },
    });
  } else if (foundationPlan.state !== foundationOutcome) {
...
export async function runApprovedPlan(planId, workerContext) {
  ...
  const shopIds = await context.repository.getPlanShopIds(plan.id);
  if (!storage.productionScale && (plan.itemCount > storage.pilotLimits.variants || shopIds.length > storage.pilotLimits.shops)) {
  ...
  try {
    const activities = new Map((await context.repository.listPlanActivities(plan.id)).map((activity) => [activity.shopId, activity]));
    const allItems = await context.repository.listExecutionItems(job.id);
    const itemsById = new Map(allItems.map((item) => [item.id, item]));
    const recoveryIntents = await context.repository.listDispatchIntents({ jobId: job.id, statuses: ["DISPATCHED", "UNKNOWN"] });
    const blockedActivities = new Map();
    for (const intent of recoveryIntents) {
      const item = intent.planItemId ? itemsById.get(intent.planItemId) : null;
      const recoveryShopId = item?.shopId || intent.targetKey.split("\u001f", 1)[0];
      const recovered = await recoverIntent({
        intent, plan, job, item, activity: activities.get(recoveryShopId), context, lease, epoch: claim.epoch,
      });
      if (!item && intent.targetType === "createDiscount" && recovered.status !== "SUCCEEDED") {
        ...
        const current = activities.get(recoveryShopId);
        activities.set(recoveryShopId, { ...current, platformActivityId: String(recovered.readback.platformObjectId) });
      }
    }
    if (foundationPlan.scope?.workflow === "NEXT_RENEWAL") {
      for (const [shopId, activity] of activities) {
        if (blockedActivities.has(shopId)) continue;
        const prepared = await ensureRenewalActivity({ plan, job, activity, context, lease, epoch: claim.epoch });
        activities.set(shopId, prepared.activity);
        if (prepared.status !== "READY") blockedActivities.set(shopId, prepared);
      }
    }
    const pending = await context.repository.listExecutionItems(job.id, { statuses: ["PENDING"] });
    ...
    for (const item of pending) {
      ...
      const result = await processItem({
        plan, job, item, activity: activities.get(item.shopId), approval, context, lease, epoch: claim.epoch,
      });
      if (result?.stopShop) blockedShops.set(item.shopId, result);
    }
    const completedItems = await context.repository.listExecutionItems(job.id);
    const outcome = jobOutcome(completedItems);
    const summary = resultSummary(plan.id, job.id, outcome, completedItems);
