    ...
    if (foundationPlan.scope?.workflow === "NEXT_RENEWAL") {
      ...
      for (;;) {
        ...
        for (;;) {
          ...
          const batchPreflight = await prepareRenewalShop({ plan, job, activity, items: shopPage.items, approval, context, lease, epoch: claim.epoch });
          if (batchPreflight.readyItems) {
            readyItems += batchPreflight.readyItems.length;
            validationSample = batchPreflight.readyItems;
        ...
        const prepared = await ensureRenewalActivity({
          ...
          validateBeforeCreate: () => prepareRenewalShop({
            plan, job, activity, items: validationSample, approval, context, lease, epoch: claim.epoch,
          }),
        });
