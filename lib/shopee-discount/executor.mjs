...
async function prepareRenewalShop({ plan, job, activity, items, approval, context, lease, epoch, authIssueShops = new Set() }) {
  ...
  try {
    ...
    if (authorization?.authorized !== true) {
      if (!authIssueShops.has(activity.shopId)) {
        authIssueShops.add(activity.shopId);
        await appendAuthIssue(context.repository, { planId: plan.id, jobId: job.id, shopId: activity.shopId,
          requestId: context.requestId, actorId: context.workerId });
      }
    ...
    if (new Set(["SHOPEE_AUTH_ERROR", "SHOPEE_WRITE_TARGET_NOT_AUTHORIZED"]).has(cause?.code)) {
      if (!authIssueShops.has(activity.shopId)) await appendAuthIssue(context.repository, {
    ...
    if (foundationPlan.scope?.workflow === "NEXT_RENEWAL") {
      ...
      for (;;) {
        ...
        for (;;) {
          ...
          const batchPreflight = await prepareRenewalShop({ plan, job, activity, items: shopPage.items, approval, context, lease, epoch: claim.epoch, authIssueShops });
