...
async function assertDualApproval(planId, workerContext) {
  ...
  if (!resuming && (!plan.expiresAt || new Date(plan.expiresAt).getTime() <= new Date(now).getTime())) {
  ...
  if (Number(plan.summary?.settingsGeneration || 0) !== Number(settings?.credentialGeneration || 0)) {
    throw executionError("SHOPEE_DISCOUNT_SETTINGS_GENERATION_CHANGED", "Settings generation changed after approval");
  }
...
async function assertApprovalRoot(plan, repository) {
  ...
  for (;;) {
    const shardPage = await repository.listPlanShardsPage(plan.id, { cursor: shardCursor, pageSize: 100 });
    for (const shard of shardPage.items) {
    if (shard.shardIndex !== expectedShardIndex++) throw executionError("SHOPEE_DISCOUNT_APPROVAL_REHASH_MISMATCH", "Approval shards are not contiguous");
    ...
    if (rebuiltShard.shardHashes[0] !== shard.shardHash) throw executionError("SHOPEE_DISCOUNT_APPROVAL_REHASH_MISMATCH", "Persisted approval shard does not match its items");
    accumulator.add(rebuiltShard.shardHashes[0]);
    itemCount += items.length;
    }
    if (shardPage.nextCursor == null) break;
    shardCursor = shardPage.nextCursor;
  }
  const rebuilt = accumulator.finish(itemCount);
  if (rebuilt.root !== plan.merkleRoot || rebuilt.itemCount !== plan.itemCount) {
...
async function reportShopAuthIssueOnce(reported, repository, { planId, jobId, shopId, requestId, actorId, intentId = null }) {
  const key = `${jobId}\u001f${shopId}\u001f${requestId}`;
...
function definiteWriteOutcome(cause) {
  if (cause?.code === "SHOPEE_AUTH_ERROR") return { intentStatus: "REJECTED", itemStatus: "AUTH_BLOCKED" };
  if (new Set(["SHOPEE_BUSINESS_ERROR", "SHOPEE_INPUT_INVALID"]).has(cause?.code)) {
    return { intentStatus: "REJECTED", itemStatus: "REJECTED" };
  }
...
async function prepareRenewalShop({ plan, job, activity, items, approval, context, lease, epoch, reportAuthIssue }) {
  ...
  try {
    ...
    if (authorization?.authorized !== true) {
      await reportAuthIssue({ shopId: activity.shopId });
      return blocked("AUTH_BLOCKED", "SHOPEE_AUTH_ERROR");
    }
    const readyItems = [], blockedItems = [];
    for (const item of items) {
      try {
        const warehouse = await context.readers.getWarehouseState({ plan, item, requestId: context.requestId });
        if (!warehouseMatches(item, warehouse)) { blockedItems.push({ item, status: "REQUIRES_REAPPROVAL", reasonCode: "WAREHOUSE_PRICE_DRIFT" }); continue; }
        ...
        if (!listing || listing.status !== "ACTIVE" || listing.sku !== item.sku || listing.originalPriceMinor !== item.payload.originalMinor) {
          blockedItems.push({ item, status: "REQUIRES_REAPPROVAL", reasonCode: "SHOPEE_LISTING_DRIFT" }); continue;
        }
        const discount = await context.readers.getDiscountState({ plan, item, activity, requestId: context.requestId });
        if (discount?.conflict) { blockedItems.push({ item, status: "REQUIRES_REAPPROVAL", reasonCode: "SHOPEE_DISCOUNT_EXTERNAL_CONFLICT" }); continue; }
        readyItems.push(item);
      } catch (cause) {
        blockedItems.push({ item, status: cause?.code === "SHOPEE_AUTH_ERROR" ? "AUTH_BLOCKED" : "UNKNOWN",
          reasonCode: cause?.code || "SHOPEE_PREFLIGHT_UNKNOWN" });
      }
    }
    if (blockedItems.some(({ status }) => status === "AUTH_BLOCKED")) {
      await reportAuthIssue({ shopId: activity.shopId });
      return blocked("AUTH_BLOCKED", "SHOPEE_AUTH_ERROR");
    }
    if (!await lease.renew()) throw executionError("SHOPEE_DISCOUNT_LEASE_LOST", "Execution lease was lost during shop preparation");
    return { status: readyItems.length ? "READY" : blockedItems[0]?.status || "SKIPPED", readyItems, blockedItems };
  } catch (cause) {
...
async function processItem({ plan, job, item, activity, approval, context, lease, epoch, reportAuthIssue }) {
  ...
  try {
    ...
    await reportAuthIssue({ shopId: item.shopId });
  ...
  try {
    ...
    if (outcome.itemStatus === "AUTH_BLOCKED") await reportAuthIssue({ shopId: item.shopId, intentId: intent.id });
  ...
  try {
    ...
    if (cause?.code === "SHOPEE_AUTH_ERROR") await reportAuthIssue({ shopId: item.shopId, intentId: intent.id });
    return cause?.code === "SHOPEE_AUTH_ERROR"
      ? { stopShop: true, status: "UNKNOWN", reasonCode: "SHOPEE_AUTH_ERROR" }
      : { stopShop: false };
  }
  try {
    if (!await lease.renew()) {
...
async function recoverIntent({ intent, plan, job, item, activity, context, lease, epoch, reportAuthIssue }) {
  ...
  try {
    ...
    if (!item && intent.targetType === "createDiscount" && readback?.verified === true && readback?.markerVerified === true
      && readback?.platformObjectId && exactRenewalProof(intent, activity, readback)) {
    ...
    } else if (item && (activity?.platformActivityId || item.payload.approvalTarget?.targetDiscountId)
      && exactReadback(readback, item, activity?.platformActivityId || item.payload.approvalTarget.targetDiscountId)) {
      itemStatus = "SUCCEEDED";
      reasonCode = null;
    } else if (readback?.membership === false
      || ((activity?.platformActivityId || item?.payload.approvalTarget?.targetDiscountId) && readback?.activityId
        && String(readback.activityId) !== String(activity?.platformActivityId || item.payload.approvalTarget.targetDiscountId))) {
      itemStatus = "CONFLICT";
      reasonCode = "SHOPEE_RECOVERY_READBACK_CONFLICT";
    }
  } catch (cause) {
    if (cause?.code === "SHOPEE_AUTH_ERROR") {
      await reportAuthIssue({ shopId: item?.shopId || activity.shopId, intentId: intent.id });
      itemStatus = "UNKNOWN";
      reasonCode = "SHOPEE_AUTH_ERROR";
    }
  }
...
async function ensureRenewalActivity({ plan, job, activity, context, lease, epoch, validateBeforeCreate, reportAuthIssue }) {
  ...
  try {
    ...
    if (cause?.code === "SHOPEE_AUTH_ERROR") await reportAuthIssue({ shopId: activity.shopId });
    return {
      activity,
  ...
  if (existing) {
    if (existing.markerVerified !== true || !existing.platformObjectId || !exactRenewalProof(null, activity, existing, { requireIntentBinding: false })) {
  ...
  try {
    ...
    if (outcome.itemStatus === "AUTH_BLOCKED") await reportAuthIssue({ shopId: activity.shopId, intentId: intent.id });
  ...
  try {
    ...
    if (cause?.code === "SHOPEE_AUTH_ERROR") await reportAuthIssue({ shopId: activity.shopId, intentId: intent.id });
  ...
  try {
    ...
    if (readback?.verified !== true || readback?.markerVerified !== true || !readback?.platformObjectId || !exactRenewalProof(intent, activity, readback)) {
...
export async function runApprovedPlan(planId, workerContext) {
  ...
  const reportedAuthIssues = new Set();
  const reportAuthIssue = ({ shopId, intentId = null }) => reportShopAuthIssueOnce(reportedAuthIssues, context.repository, {
    planId: plan.id,
    jobId: job.id,
    shopId,
    requestId: context.requestId,
  ...
  try {
    ...
    let reopenedBeforeCursor = false;
    ...
    for (;;) {
      ...
      const recovered = await recoverIntent({
        intent, plan, job, item, activity, context, lease, epoch: claim.epoch, reportAuthIssue,
      });
      if (item && recovered.status === "UNKNOWN" && recovered.reasonCode === "SHOPEE_AUTH_ERROR") {
        blockedShops.set(recoveryShopId, { status: "UNKNOWN", reasonCode: "SHOPEE_AUTH_ERROR" });
      }
      if (!item && intent.targetType === "createDiscount" && recovered.status !== "SUCCEEDED") {
        ...
        for (;;) {
          ...
          if (resetPage.items.length) reopenedBeforeCursor = true;
    ...
    for (;;) {
      ...
      try {
        const authorization = await context.readers.getShopAuthorization({ shopId: item.shopId, requestId: context.requestId });
        if (authorization?.authorized === true) {
          reopenedBeforeCursor = true;
          await context.repository.setExecutionItemStatus({
          ...
          await reportAuthIssue({ shopId: item.shopId });
          blockedShops.set(item.shopId, { status: "AUTH_BLOCKED", reasonCode: "SHOPEE_AUTH_ERROR" });
        }
      } catch (cause) {
        if (cause?.code === "SHOPEE_AUTH_ERROR") {
          await reportAuthIssue({ shopId: item.shopId });
    ...
    if (foundationPlan.scope?.workflow === "NEXT_RENEWAL") {
      ...
      for (;;) {
        ...
        if (blockedShops.has(shopId) || blockedActivities.has(shopId)) continue;
        let shopCursor = -1;
        let readyItems = 0;
        let shopAuthBlocked = false;
        for (;;) {
          ...
          if (!shopPage.items.length) break;
          const batchPreflight = await prepareRenewalShop({ plan, job, activity, items: shopPage.items, approval, context, lease, epoch: claim.epoch, reportAuthIssue });
          if (batchPreflight.readyItems?.length) {
            readyItems += batchPreflight.readyItems.length;
            for (const blockedItem of batchPreflight.blockedItems) await markItem(context.repository, lease, {
              jobId: job.id, planItemId: blockedItem.item.id, ownerId: context.workerId, epoch: claim.epoch,
              status: blockedItem.status, reasonCode: blockedItem.reasonCode,
              evidence: { requestId: context.requestId, renewalBatchPreflight: true },
            });
          } else {
            if (batchPreflight.status === "AUTH_BLOCKED") shopAuthBlocked = true;
            for (const item of shopPage.items) await markItem(context.repository, lease, {
              jobId: job.id, planItemId: item.id, ownerId: context.workerId, epoch: claim.epoch,
              status: batchPreflight.status, reasonCode: batchPreflight.reasonCode,
              evidence: { requestId: context.requestId, renewalBatchPreflight: true },
            });
          }
        ...
        if (shopAuthBlocked) {
          blockedActivities.set(shopId, { status: "AUTH_BLOCKED", reasonCode: "SHOPEE_AUTH_ERROR" });
          continue;
        }
        if (!readyItems) continue;
        const validationPage = await context.repository.listExecutionItemsPage(job.id, {
          cursor: -1,
          shopId,
          statuses: ["PENDING"],
          pageSize: Math.min(100, context.siteCapability.maxAddItems),
        });
        if (!validationPage.items.length) continue;
        const prepared = await ensureRenewalActivity({
          plan, job, activity, context, lease, epoch: claim.epoch, reportAuthIssue,
          validateBeforeCreate: () => prepareRenewalShop({
            plan, job, activity, items: validationPage.items, approval, context, lease, epoch: claim.epoch,
            reportAuthIssue,
          }),
        });
    ...
    let pendingCursor = !reopenedBeforeCursor && job.cursor?.phase === "ITEMS" ? Number(job.cursor.sequence ?? -1) : -1;
    for (;;) {
      ...
      try {
        ...
        if (status === "AUTH_BLOCKED") await reportAuthIssue({ shopId: item.shopId });
        await markItem(context.repository, lease, {
          jobId: job.id, planItemId: item.id, ownerId: context.workerId, epoch: claim.epoch,
      ...
      await context.repository.checkpointJob({ jobId: job.id, ownerId: context.workerId, epoch: claim.epoch,
        cursor: { phase: "ITEMS", sequence: pendingPage.items.at(-1)?.sequence ?? pendingCursor }, counters: await context.repository.countExecutionItemsByStatus(job.id) });
    ...
    if (Number(aggregateCounts.UNKNOWN || 0) + Number(aggregateCounts.AUTH_BLOCKED || 0) + Number(aggregateCounts.DISPATCHED || 0) > 0) {
      await context.repository.checkpointJob({
        ...
        cursor: { phase: "RESUMABLE", sequence: -1 }, counters: summary.counts,
      });
