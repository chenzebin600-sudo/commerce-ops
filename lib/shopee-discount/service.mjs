...
function renewalActivities({ planId, country, shops, priceTier, targetStart, targetEnd }) {
...
export class ShopeeDiscountService {
  ...
  async createPreview(rawInput, context = {}) {
    ...
    for (const candidate of candidates) {
      ...
      ready.push({ candidate, decision, approval, warehouseApprovedAt: warehouseApprovedAt(row, candidate.tier) });
    }
    ...
    try {
      ...
      await this.repository.appendPlanShard({
        ...
        items: shard.map(({ candidate, decision, approval, warehouseApprovedAt: approvedAt }, index) => ({
