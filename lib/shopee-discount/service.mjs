...
export class ShopeeDiscountService {
  constructor({ repository, foundation, shopee, warehouse, writeSecurity, now = () => new Date(), approvalTtlMs = 10 * 60_000,
    ...
    verifyWarehouseKey = null, reconciliationEvidence = null, executeApprovedPlan = null } = {}) {
  ...
  async updateSettings(input, context = {}) {
    ...
    if (Object.hasOwn(patch, "encryptedWarehouseKeyCiphertext") || Object.hasOwn(patch, "warehouseKeyReference")) {
      for (const plan of await this.repository.listPlans({ limit: 1000 })) {
        if (["PREVIEWED", "APPROVED"].includes(plan.state)) await this.repository.markPlanState({
          planId: plan.id, fromState: plan.state, toState: "BLOCKED", expectedVersion: plan.stateVersion,
          reasonCode: "SHOPEE_DISCOUNT_SECURITY_SETTINGS_CHANGED",
        });
      }
    }
  ...
  async verifySettings(context = {}) {
    ...
    await this.repository.saveSettings({ metadata: { ...(settings?.metadata || {}), warehouseKeyVerifiedAt: this.now().toISOString() } }, { actorId: identity.actorId });
    return this.getSettings(context);
  }
  ...
  async lookupOverrides(input, context = {}) {
    exactFields(input, new Set(["country", "shopIds", "query", "limit"]), "override lookup");
    const country = requiredText(input.country, "country", 3).toUpperCase();
    const queryText = requiredText(input.query, "query", 500);
    const itemLink = queryText.match(/-i\.([1-9]\d*)\.([1-9]\d*)(?:[/?#.]|$)/i);
    const productLink = queryText.match(/\/product\/([1-9]\d*)\/([1-9]\d*)(?:[/?#.]|$)/i);
    const canonicalItemId = itemLink?.[2] || productLink?.[2] || (/^[1-9]\d*$/.test(queryText) ? queryText : null);
  ...
  async requestExecution(input, context = {}) {
    ...
    if (current[0]) {
      if (current[0].status === "PENDING" && typeof this.executeApprovedPlan === "function") {
        await this.executeApprovedPlan(plan.id, { context, job: current[0] });
