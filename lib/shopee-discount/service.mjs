...
export class ShopeeDiscountService {
  ...
  async #createProductionPreview(rawInput, context, storage) {
    ...
    const activitySelectionsByShop = new Map();
    for (const selection of input.activitySelection.values()) {
      if (!activitySelectionsByShop.has(selection.shopId)) activitySelectionsByShop.set(selection.shopId, []);
      activitySelectionsByShop.get(selection.shopId).push(selection);
    }
    const now = this.now();
    ...
    const previewOwnerLeaseUntil = new Date(now.getTime() + 5 * 60_000).toISOString();
    ...
    if (!domainPlan) domainPlan = await this.repository.createPlan({
      ...
      summary: { previewSagaId: sagaKey, previewInputHash, previewOwnerToken, previewOwnerLeaseUntil, streaming: true },
    });
    ...
    if (!previewOwner && timestampMs(domainPlan.summary?.previewOwnerLeaseUntil) <= now.getTime()) {
      const claimed = await this.repository.claimPreviewOwnership({ planId: domainPlan.id,
        expectedOwnerToken: domainPlan.summary?.previewOwnerToken || null, ownerToken: previewOwnerToken,
        leaseUntil: previewOwnerLeaseUntil });
    ...
    let shardBuffer = [], shardIndex = 0, sequence = 0, discovered = 0, readyCount = 0;
    const flush = async () => {
      if (!shardBuffer.length) return;
      ...
      } else await this.repository.appendPlanShard({
        planId: domainPlan.id, shardIndex, shardHash: hash,
        items: shardBuffer.map(({ candidate, decision, approval, warehouseApprovedAt: approvedAt, approvalTarget }) => ({
      ...
      if (existing) sequence += shardBuffer.length;
    ...
    try {
      for (const shop of shops) {
        const renewed = await this.repository.claimPreviewOwnership({ planId: domainPlan.id,
          expectedOwnerToken: previewOwnerToken, ownerToken: previewOwnerToken,
          leaseUntil: new Date(this.now().getTime() + 5 * 60_000).toISOString() });
        if (!renewed) {
          previewOwner = false;
          throw domainError("SHOPEE_DISCOUNT_PREVIEW_OWNER_LOST", "Preview ownership was lost before planning the next shop");
        }
        domainPlan = renewed;
        ...
        const planned = await this.#planProductionShop({ input, shop, site, context, renewalIdentity, pinned, skip, activitySelectionsByShop });
        ...
        for (const item of planned.ready) {
          shardBuffer.push(item);
          if (shardBuffer.length === this.shardSize) {
            this.previewObserver?.({ shopId: shop.shopId, shopVariants: planned.shopVariants, shardBuffer: shardBuffer.length,
              persistedItems: readyCount - shardBuffer.length, storageDialect: storage.dialect,
      ...
      domainPlan = await this.repository.finalizePreviewMetadata({ planId: domainPlan.id,
        ...
        summary: { ...summary, previewSagaId: sagaKey, previewInputHash, previewOwnerToken: domainPlan.summary?.previewOwnerToken,
          previewOwnerLeaseUntil: domainPlan.summary?.previewOwnerLeaseUntil } });
      ...
      if (ownsCurrent && !concurrencyConflict && current?.state === "PREVIEWING") await this.repository.markPlanState({
        planId: current.id, fromState: current.state, toState: "BLOCKED", expectedVersion: current.stateVersion, reasonCode: failureCode,
      }).catch(() => {});
      if (ownsCurrent && foundationPlan) await this.foundation.operationPlans.block(foundationPlan.id,
        { reasonCode: failureCode, evidence: { previewSagaId: sagaKey } }).catch(() => {});
  ...
  async createPreview(rawInput, context = {}) {
    ...
    if (storage.productionScale) return this.#createProductionPreview(rawInput, context, storage);
    return this.#createPreviewLegacy(rawInput, context);
  }
