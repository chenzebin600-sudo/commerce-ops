...
function noActiveVariantsError(workflow) {
  const message = workflow === "CURRENT_CORRECTION"
    ? "现有 Discount 活动中没有可处理的在售变体；新店请选择“新建或续期折扣活动”。"
    : "所选店铺没有可处理的虾皮在售变体，请先确认商品已上架。";
  return domainError("SHOPEE_DISCOUNT_NO_ACTIVE_VARIANTS", message);
}
...
function isActive(value) {
  ...
  return status === "NORMAL" || status === "MODEL_NORMAL" || status === "ACTIVE" || status === "ENABLED";
}
...
export class ShopeeDiscountService {
  ...
  async verifySettings(context = {}) {
    ...
    const settings = await this.repository.getSettings();
    if (!settings?.encryptedWarehouseKeyCiphertext && !settings?.warehouseKeyReference) {
      throw domainError("SHOPEE_DISCOUNT_WAREHOUSE_KEY_REQUIRED", "请先填写并保存数仓 Key");
    }
  ...
  async #createProductionPreview(rawInput, context, storage) {
    ...
    if (domainPlan?.state === "CANCELLED" && domainPlan.reasonCode === "SHOPEE_DISCOUNT_NO_ACTIVE_VARIANTS") {
      throw noActiveVariantsError(input.workflow);
    }
    ...
    if (!previewOwner) {
      ...
      if (shared?.state === "CANCELLED" && shared.reasonCode === "SHOPEE_DISCOUNT_NO_ACTIVE_VARIANTS") {
        throw noActiveVariantsError(input.workflow);
      }
    ...
    try {
      ...
      if (discovered === 0) {
        domainPlan = await this.repository.markPlanState({
          planId: domainPlan.id,
          fromState: "PREVIEWING",
          toState: "CANCELLED",
          expectedVersion: domainPlan.stateVersion,
          reasonCode: "SHOPEE_DISCOUNT_NO_ACTIVE_VARIANTS",
          ownerToken: previewOwnerToken,
          ownerEpoch: previewOwnerEpoch,
        });
        throw noActiveVariantsError(input.workflow);
      }
      ...
      if (cause?.code === "SHOPEE_DISCOUNT_NO_ACTIVE_VARIANTS" && current?.state === "CANCELLED") throw cause;
  ...
  async #createPreviewLegacy(rawInput, context = {}) {
    ...
    if (variants.length + ingestionIssues.length === 0) throw noActiveVariantsError(input.workflow);
