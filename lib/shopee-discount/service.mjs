...
export class ShopeeDiscountService {
  ...
  async #assertModuleEnabled() {
    if (!this.enforceSettings) return;
  ...
  async updateSettings(input, context = {}) {
    ...
    if (Object.hasOwn(patch, "encryptedWarehouseKeyCiphertext") || Object.hasOwn(patch, "warehouseKeyReference")) {
      if (!this.repository.invalidateActivePlans) throw domainError("SHOPEE_DISCOUNT_SETTINGS_DISABLED", "Active-plan invalidation is unavailable");
      await this.repository.invalidateActivePlans({ reasonCode: "SHOPEE_DISCOUNT_SECURITY_SETTINGS_CHANGED", actorId: identity.actorId });
    }
  ...
  async lookupOverrideBatch(input, context = {}) {
    ...
    return { country, rowCount: echoes.length, rows: echoes };
  }
  ...
  async listUnknownIntents(filters = {}, context = {}) {
    exactFields(filters, new Set(["limit"]), "UNKNOWN intent filters");
    this.#privileged(context, "Listing UNKNOWN intents");
    const limit = Math.max(1, Math.min(100, Number(filters.limit) || 50));
    const runs = await this.repository.listRunsScoped({ limit: 100 }, context.authorizedShopIds);
    const output = [];
    for (const run of runs) {
      if (output.length >= limit) break;
      const page = await this.repository.listDispatchIntentsPage({ jobId: run.id, statuses: ["UNKNOWN"], pageSize: limit - output.length });
      output.push(...page.items.map((intent) => ({ ...intent, intentId: intent.id, planId: run.planId })));
    }
