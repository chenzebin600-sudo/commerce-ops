...
export class ShopeeDiscountService {
  ...
  async createPreview(rawInput, context = {}) {
    ...
    for (const variant of variants) {
      ...
      const verifiedSelected = selectedTargets.map((selection) => activities.get(variant.shopId)
        .find((entry) => entry.discountId === selection.discountId
          && String(entry.status).toLowerCase() === "ongoing"
      ...
      if (current.length === 1) {
        ...
        if ((stored || selected) && (!verifiedSelected.some((entry) => entry.discountId === activity.discountId) || !storedWindowMatches)) {
        ...
        if (selectedTargets.length !== 1 || verifiedSelected.length !== 1) {
          skip(selectedTargets.length ? "CURRENT_ACTIVITY_AMBIGUOUS" : "CURRENT_ACTIVITY_TARGET_REQUIRED",
            { shopId: variant.shopId, itemId: variant.itemId, modelId: variant.modelId });
          continue;
        }
        activity = verifiedSelected[0];
        activityTier = selectedTargets[0].priceTier;
      }
