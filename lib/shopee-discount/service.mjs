...
export class ShopeeDiscountService {
  ...
  async #planProductionShop({ input, shop, site, context, renewalIdentity, pinned, skip }) {
    ...
    for (const variant of ingestion.variants) {
      ...
      const verifiedSelected = selectedTargets.map((selection) => ongoing.find((entry) => entry.discountId === selection.discountId)).filter(Boolean);
      if (current.length === 1) {
  ...
  async #createProductionPreview(rawInput, context, storage) {
    ...
    if (!domainPlan) domainPlan = await this.repository.createPlan({
      ...
      createdBy: context.actorId || "shopee-discount-preview", summary: { previewSagaId: sagaKey, streaming: true },
    });
    ...
    try {
      for (const shop of shops) {
        ...
        const planned = await this.#planProductionShop({ input, shop, site, context, renewalIdentity, pinned, skip });
      ...
      domainPlan = await this.repository.finalizePreviewMetadata({ planId: domainPlan.id,
        sourceSnapshotHash: foundationContentHash(sourceSnapshot), summary: { ...summary, previewSagaId: sagaKey } });
      ...
      if (current && ["PREVIEWING", "PREVIEWED", "APPROVED"].includes(current.state)) await this.repository.markPlanState({
        planId: current.id, fromState: current.state, toState: "BLOCKED", expectedVersion: current.stateVersion, reasonCode: failureCode,
      }).catch(() => {});
      if (foundationPlan) await this.foundation.operationPlans.block(foundationPlan.id, { reasonCode: failureCode, evidence: { previewSagaId: sagaKey } }).catch(() => {});
