...
function epochSeconds(value) {
  ...
  return Number.isFinite(millis) && millis % 1_000 === 0 ? String(millis / 1_000) : null;
}
...
function exactCreateReadback(intent, activity, readback) {
  ...
  return Boolean(readback?.verified === true
    ...
    && readback.discountName === metadata.discountName
    && readback.marker === metadata.marker
    && readback.fingerprint === metadata.fingerprint
    && readback.startTime === epochSeconds(activity?.startsAt)
    && readback.endTime === epochSeconds(activity?.endsAt));
}
...
export async function reconcileIntent(intentId, resolution, auditContext = {}) {
  ...
  if (!item && intent.targetType === "createDiscount") {
    ...
    activity = (await auditContext.repository.listPlanActivities(intent.planId)).find((entry) => entry.shopId === shopId) || null;
  }
