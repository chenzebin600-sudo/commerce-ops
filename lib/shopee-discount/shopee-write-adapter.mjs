...
function validateRequestId(value) {
  if (typeof value !== "string" || value.length < 1 || value.length > 200 || /[\r\n]/.test(value)) throw inputError("requestId is invalid");
  return value;
}
...
function normalizeResult(result, fallbackRequestId) {
  ...
  return { data: data?.response ?? data, requestId: data?.request_id || fallbackRequestId };
}
...
function classifyWriteResult(result, operationId, fallbackRequestId) {
  ...
  if (status === 401 || status === 403 || /auth|token|permission|forbidden|unauthori[sz]ed/i.test(errorText)) {
    throw definiteError("SHOPEE_AUTH_ERROR", "Shopee write authentication was rejected", requestId);
  }
  if (status === 429 || status >= 500) throw unknownWrite(operationId, requestId);
