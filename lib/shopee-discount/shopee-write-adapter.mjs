...
function positiveIdNumber(value, field) {
  const normalized = positiveId(value, field);
  const numeric = Number(normalized);
  if (!Number.isSafeInteger(numeric)) throw inputError(`${field} exceeds the safe relay range`);
  return numeric;
}
...
function fixedPrice(value, siteCapability, field) {
  ...
  if (minor > BigInt(Number.MAX_SAFE_INTEGER)) throw inputError(`${field} exceeds the safe relay range`);
  if (siteCapability.priceScale === 0) return Number(minor);
  const digits = minor.toString().padStart(siteCapability.priceScale + 1, "0");
  return Number(`${digits.slice(0, -siteCapability.priceScale)}.${digits.slice(-siteCapability.priceScale)}`);
}
...
function classifyWriteResult(result, operationId, fallbackRequestId, apiPath) {
  ...
  if (isDefiniteBusinessCode(apiPath, platformCode)) {
...
export class ShopeeWriteAdapter {
  ...
  async #dispatch({ operationUuid: operationId, shopId, requestId, apiPath, params }) {
    ...
    try {
      result = await this.transport({
        ...
        body: { shop_id: validatedShopId, api_path: apiPath, method: "POST", params: {}, body: params },
      });
    } catch (error) {
    ...
    result = classifyWriteResult(result, uuid, validatedRequestId, apiPath);
  ...
  async addDiscountItems({ operationUuid: operationId, shopId, discountId, items, requestId, ...unknown } = {}) {
    if (Object.keys(unknown).length) throw inputError(`unknown field: ${Object.keys(unknown)[0]}`);
    return this.#dispatch({ operationUuid: operationId, shopId, requestId, apiPath: WRITE_PATHS.addDiscountItems, params: { discount_id: positiveIdNumber(discountId, "discountId"), item_list: this.#items(items, true) } });
  }

  async updateDiscountItems({ operationUuid: operationId, shopId, discountId, items, requestId, ...unknown } = {}) {
    if (Object.keys(unknown).length) throw inputError(`unknown field: ${Object.keys(unknown)[0]}`);
    return this.#dispatch({ operationUuid: operationId, shopId, requestId, apiPath: WRITE_PATHS.updateDiscountItems, params: { discount_id: positiveIdNumber(discountId, "discountId"), item_list: this.#items(items, false) } });
  }
  ...
  #items(items, add) {
    ...
    const normalizedItems = items.map((item) => {
      ...
      const output = { item_id: positiveIdNumber(item.itemId, "itemId") };
      ...
      if (item.models !== undefined) {
        ...
        output.model_list = item.models.map((model) => {
          ...
          const normalized = { model_id: positiveIdNumber(model.modelId, "modelId"), model_promotion_price: fixedPrice(model.modelPromotionPriceMinor, this.siteCapability, "modelPromotionPriceMinor") };
