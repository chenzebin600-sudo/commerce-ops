...
export class PostgresqlShopeeDiscountRepository {
  ...
  async saveSettings(input = {}, audit = {}) {
    ...
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_settings")} SET
      ...
      timezone=COALESCE($8,timezone),enabled=COALESCE($9,enabled),metadata_json=COALESCE($10::jsonb,metadata_json),
  ...
  async markSettingsVerified({ expected, metadata }, audit = {}) {
    ...
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_settings")}
      ...
      AND encrypted_warehouse_key_ciphertext IS NOT DISTINCT FROM $4
      AND warehouse_key_reference IS NOT DISTINCT FROM $5
      AND warehouse_key_updated_at IS NOT DISTINCT FROM $6 RETURNING *`, [
