...
export class PostgresqlShopeeDiscountRepository {
  ...
  async claimNotificationDelivery({ notificationId, expectedAttemptCount }) {
    ...
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_notifications")}
      SET delivery_status='SENDING',attempt_count=attempt_count+1,updated_at=$1
      WHERE id=$2 AND attempt_count=$3 AND delivery_status=ANY($4::text[]) RETURNING *`, [
      timestamp, text(notificationId, "notificationId"), attempt, ["PENDING", "RETRY_WAIT"],
    ]);
    return notificationRow(result.rows[0]);
  }
  ...
  async markNotificationDelivery({ notificationId, patch = {} }) {
    ...
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_notifications")}
      SET delivery_status=$1,attempt_count=$2,last_error_code=$3,delivered_at=$4,updated_at=$5
