...
export class PostgresqlShopeeDiscountRepository {
  ...
  async updatePreviewActivity({ planId, shopId, activityType, platformActivityId = null, metadata = {} }) {
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_activities")} a
      ...
      WHERE a.plan_id=$5 AND a.shop_id=$6 AND p.id=a.plan_id AND p.state='PREVIEWING' RETURNING a.*`, [
      text(activityType, "activityType"), platformActivityId == null ? null : text(platformActivityId, "platformActivityId"),
      JSON.stringify(metadata), iso(this.now()), text(planId, "planId"), text(shopId, "shopId"),
    ]);
  ...
  async finalizePreviewMetadata({ planId, sourceSnapshotHash, summary }) {
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_plans")}
      ...
      WHERE id=$4 AND state='PREVIEWING' RETURNING *`, [text(sourceSnapshotHash, "sourceSnapshotHash"),
      JSON.stringify(summary || {}), iso(this.now()), text(planId, "planId")]);
    if (!result.rows[0]) throw error("SHOPEE_DISCOUNT_PLAN_IMMUTABLE", "Preview metadata can only be finalized while PREVIEWING");
    return planRow(result.rows[0]);
  }

  async claimPreviewOwnership({ planId, expectedOwnerToken, ownerToken, leaseUntil }) {
    return this.provider.transaction(async (transaction) => {
      ...
      if ((summary.previewOwnerToken || null) !== (expectedOwnerToken || null)) return null;
      ...
      const updated = await transaction.query(`UPDATE ${this.#table("shopee_discount_plans")}
        SET summary_json=$1::jsonb,updated_at=$2 WHERE id=$3 AND state='PREVIEWING' RETURNING *`,
      [JSON.stringify(summary), iso(this.now()), planId]);
  ...
  async getLatestWarehouseBaseline({ country, category, tier, shopId = null }) {
    const rows = (await this.provider.query(`SELECT evidence_json FROM ${this.#table("shopee_discount_events")}
      WHERE event_type='WAREHOUSE_BASELINE' AND ($1::text IS NULL OR evidence_json->'scope'->>'shopId'=$1)
      ORDER BY occurred_at DESC,id DESC LIMIT $2`, [shopId, shopId ? 20 : 500])).rows;
    for (const row of rows) { const value = json(row.evidence_json, null); if (value?.scope?.country === country && value.scope.category === category && value.scope.tier === tier && (!shopId || value.scope.shopId === shopId)) return value; }
    return null;
  }
  ...
  async appendPlanShard({ planId, shardIndex, shardHash, items = [] }) {
    ...
    return this.provider.transaction(async (transaction) => {
      ...
      if (existingShard) {
        const stored = (await transaction.query(`SELECT sequence_no,shop_id,item_id,model_id,payload_hash
        ...
        const matches = existingShard.shard_hash === shardHash && Number(existingShard.item_count) === items.length
          && stored.length === items.length && stored.every((row, index) => {
            const item = items[index];
            const sequence = Number.isInteger(item.sequence) ? item.sequence : shardIndex * 10_000 + index;
            return Number(row.sequence_no) === sequence && row.shop_id === String(item.shopId) && row.item_id === String(item.itemId)
              && row.model_id === String(item.modelId) && row.payload_hash === String(item.payloadHash);
          });
