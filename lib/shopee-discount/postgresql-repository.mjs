...
function canonicalShardContent(items, shardIndex) {
  return items.map((value, index) => ({
    sequence: Number.isInteger(value.sequence) ? value.sequence : shardIndex * 10_000 + index,
    shopId: String(value.shopId), itemId: String(value.itemId), modelId: String(value.modelId),
    itemKey: value.itemKey ?? `${value.shopId}\u001f${value.itemId}\u001f${value.modelId}`,
    sku: String(value.sku), currency: String(value.currency), scale: Number(value.scale),
    currentPriceMinor: String(value.currentPriceMinor ?? value.sourcePriceMinor),
    controlPriceMinor: value.controlPriceMinor == null ? null : String(value.controlPriceMinor),
    targetPriceMinor: String(value.targetPriceMinor), payloadHash: String(value.payloadHash), payload: value.payload || {},
  }));
}
function shardContentHash(items, shardIndex) {
  return createHash("sha256").update(canonicalJson(canonicalShardContent(items, shardIndex))).digest("hex");
}
...
function settingsRow(row) {
  ...
  return {
    ...
    warehouseKeyReference: row.warehouse_key_reference, warehouseKeyHint: row.warehouse_key_hint, credentialGeneration: Number(row.credential_generation || 0),
...
function planRow(row) {
  ...
  if (Object.hasOwn(row, "preview_owner_token")) {
    summary.previewOwnerToken = row.preview_owner_token;
    summary.previewOwnerEpoch = Number(row.preview_owner_epoch || 0);
    summary.previewOwnerLeaseUntil = isoNullable(row.preview_owner_lease_until);
  }
  return {
    ...
    retentionUntil: isoNullable(row.retention_until), summary,
    createdAt: isoNullable(row.created_at), updatedAt: isoNullable(row.updated_at),
  };
}
...
function notificationRow(row) {
  if (!row) return null;
  return {
    id: row.id, planId: row.plan_id, dedupeKey: row.dedupe_key,
    notificationType: row.notification_type, severity: row.severity, title: row.title, message: row.message,
    channel: row.channel, status: row.delivery_status, attemptCount: Number(row.attempt_count || 0),
    lastErrorCode: row.last_error_code, metadata: json(row.metadata_json), readAt: isoNullable(row.read_at),
    deliveryLeaseUntil: isoNullable(row.delivery_lease_until), coordinationState: row.coordination_state,
    coordinationEvidence: json(row.coordination_evidence_json),
    deliveredAt: isoNullable(row.delivered_at), retentionUntil: isoNullable(row.retention_until),
    createdAt: isoNullable(row.created_at), updatedAt: isoNullable(row.updated_at),
  };
}
...
function eventRow(row) { return row ? { id: row.id, planId: row.plan_id, jobId: row.job_id, intentId: row.intent_id || null, eventType: row.event_type,
...
export class PostgresqlShopeeDiscountRepository {
  ...
  async saveSettings(input = {}, audit = {}) {
    ...
    const hasEncrypted = Object.hasOwn(input, "encryptedWarehouseKeyCiphertext");
    const hasReference = Object.hasOwn(input, "warehouseKeyReference");
    const hasHint = Object.hasOwn(input, "warehouseKeyHint");
    const keyChanged = hasEncrypted || hasReference;
    const result = await this.provider.query(`WITH updated AS (UPDATE ${this.#table("shopee_discount_settings")} SET
      encrypted_warehouse_key_ciphertext=CASE WHEN $1 THEN $2 ELSE encrypted_warehouse_key_ciphertext END,
      warehouse_key_reference=CASE WHEN $3 THEN $4 ELSE warehouse_key_reference END,
      warehouse_key_hint=CASE WHEN $5 THEN $6 ELSE warehouse_key_hint END,
      warehouse_key_updated_at=CASE WHEN $1 OR $3 THEN $7 ELSE warehouse_key_updated_at END,
      credential_generation=credential_generation+CASE WHEN $12 THEN 1 ELSE 0 END,
      timezone=COALESCE($8,timezone),enabled=COALESCE($9,enabled),metadata_json=CASE WHEN $12 THEN '{}'::jsonb ELSE COALESCE($10::jsonb,metadata_json) END,
      updated_by=COALESCE($11,updated_by),updated_at=$7 WHERE id='default' RETURNING *),
      blocked AS (UPDATE ${this.#table("shopee_discount_plans")} SET state='BLOCKED',reason_code='SHOPEE_DISCOUNT_SECURITY_SETTINGS_CHANGED',state_version=state_version+1,updated_at=$7
        WHERE $12 AND state=ANY($13::text[]) RETURNING id)
      SELECT updated.* FROM updated`, [
      hasEncrypted, input.encryptedWarehouseKeyCiphertext ?? null, hasReference, input.warehouseKeyReference ?? null,
      hasHint, input.warehouseKeyHint ?? null, timestamp, input.timezone ?? null,
      Object.hasOwn(input, "enabled") ? Boolean(input.enabled) : null,
      metadata === null ? null : JSON.stringify(metadata), audit.actorId || audit.actorName || null, keyChanged, ["PREVIEWING", "PREVIEWED", "APPROVED"],
    ]);
    return settingsRow(result.rows[0]);
  }

  async markSettingsVerified({ expected, metadata }, audit = {}) {
    const normalized = normalizeShopeeDiscountSettingsMetadata(metadata);
    const timestamp = iso(audit.occurredAt || this.now());
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_settings")}
      SET metadata_json=$1::jsonb,updated_by=$2,updated_at=$3 WHERE id='default'
      AND credential_generation=$4
      AND encrypted_warehouse_key_ciphertext IS NOT DISTINCT FROM $5
      AND warehouse_key_reference IS NOT DISTINCT FROM $6
      AND warehouse_key_updated_at IS NOT DISTINCT FROM $7 RETURNING *`, [
      JSON.stringify(normalized), audit.actorId || null, timestamp,
      Number(expected.credentialGeneration),
      expected.encryptedWarehouseKeyCiphertext ?? null, expected.warehouseKeyReference ?? null, expected.warehouseKeyUpdatedAt ?? null,
    ]);
    return settingsRow(result.rows[0]);
  }

  async invalidateActivePlans({ reasonCode }) {
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_plans")}
      SET state='BLOCKED',reason_code=$1,state_version=state_version+1,updated_at=$2
      WHERE state=ANY($3::text[]) RETURNING id`, [text(reasonCode, "reasonCode"), iso(this.now()), ["PREVIEWING", "PREVIEWED", "APPROVED"]]);
    return { invalidatedCount: result.rows.length };
  }
  ...
  async createPlan(input) {
    ...
    return this.provider.transaction(async (transaction) => {
      for (const activity of [...activities].sort((left, right) => left.shopId.localeCompare(right.shopId))) {
        ...
        const existing = (await transaction.query(`SELECT * FROM ${this.#table("shopee_discount_plans")} WHERE id=$1`, [id])).rows[0];
        if (existing) return planRow(existing);
        const expired = await transaction.query(`UPDATE ${this.#table("shopee_discount_plans")} p
          SET state='EXPIRED',reason_code='SHOPEE_DISCOUNT_PREVIEW_TTL_EXPIRED',state_version=p.state_version+1,updated_at=$4
          FROM ${this.#table("shopee_discount_activities")} a
          WHERE a.plan_id=p.id AND a.shop_id=$1 AND a.target_starts_at<$2 AND a.target_ends_at>$3
            AND p.state='PREVIEWED' AND p.expires_at IS NOT NULL AND p.expires_at<=$4 RETURNING p.id`, [
          activity.shopId, activity.targetEndsAt, activity.targetStartsAt, timestamp,
        ]);
        if (expired.rows.length) await transaction.execute(`UPDATE ${this.#table("shopee_discount_activities")}
          SET status='ENDED',updated_at=$2 WHERE plan_id=ANY($1::text[]) AND status=ANY($3::text[])`, [
          expired.rows.map(({ id: planId }) => planId), timestamp, ["PLANNED", "ACTIVE"],
        ]);
      ...
      const created = await transaction.query(`INSERT INTO ${this.#table("shopee_discount_plans")} (
        ...
        preview_owner_token,preview_owner_epoch,preview_owner_lease_until
      ) VALUES ($1,$2,$3,'PREVIEWING',$4,$5,$6,$7,$8,$9,$10,$11::jsonb,$12,$12,$13,$14,$15) RETURNING *`, [
        ...
        owner.previewOwnerToken || null, Number(owner.previewOwnerEpoch || (owner.previewOwnerToken ? 1 : 0)), owner.previewOwnerLeaseUntil || null,
      ]);
  ...
  async bindFoundationPlan(planId, foundationPlanId, { ownerToken = null, ownerEpoch = 0 } = {}) {
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_plans")} SET foundation_plan_id=$1,updated_at=$2
      WHERE id=$3 AND state='PREVIEWING' AND (foundation_plan_id IS NULL OR foundation_plan_id=$1)
        AND (preview_owner_token IS NULL OR (preview_owner_token=$4 AND preview_owner_epoch=$5))
      RETURNING *`, [foundationPlanId, iso(this.now()), planId, ownerToken, ownerEpoch]);
  ...
  async updatePreviewActivity({ planId, shopId, activityType, platformActivityId = null, metadata = {}, ownerToken = null, ownerEpoch = 0 }) {
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_activities")} a
      SET activity_type=$1,platform_activity_id=$2,metadata_json=$3::jsonb,updated_at=$4
      FROM ${this.#table("shopee_discount_plans")} p
      WHERE a.plan_id=$5 AND a.shop_id=$6 AND p.id=a.plan_id AND p.state='PREVIEWING'
        AND (p.preview_owner_token IS NULL OR (p.preview_owner_token=$7 AND p.preview_owner_epoch=$8))
      RETURNING a.*`, [
      text(activityType, "activityType"), platformActivityId == null ? null : text(platformActivityId, "platformActivityId"),
      JSON.stringify(metadata), iso(this.now()), text(planId, "planId"), text(shopId, "shopId"), ownerToken, ownerEpoch,
    ]);
    if (!result.rows[0]) throw error("SHOPEE_DISCOUNT_PLAN_IMMUTABLE", "Preview activity can only be configured while PREVIEWING");
    return activityRow(result.rows[0]);
  }

  async finalizePreviewMetadata({ planId, sourceSnapshotHash, summary, ownerToken = null, ownerEpoch = 0 }) {
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_plans")}
      SET source_snapshot_hash=$1,summary_json=summary_json || $2::jsonb,updated_at=$3
      WHERE id=$4 AND state='PREVIEWING' AND (preview_owner_token IS NULL OR
        (preview_owner_token=$5 AND preview_owner_epoch=$6)) RETURNING *`, [text(sourceSnapshotHash, "sourceSnapshotHash"),
      JSON.stringify(summary || {}), iso(this.now()), text(planId, "planId"), ownerToken, ownerEpoch]);
    if (!result.rows[0]) throw error("SHOPEE_DISCOUNT_PLAN_IMMUTABLE", "Preview metadata can only be finalized while PREVIEWING");
    return planRow(result.rows[0]);
  }

  async claimPreviewOwnership({ planId, expectedOwnerToken, expectedOwnerEpoch = 0, expectedLeaseUntil = null, ownerToken, leaseUntil, now = this.now() }) {
    return this.provider.transaction(async (transaction) => {
      const row = (await transaction.query(`SELECT * FROM ${this.#table("shopee_discount_plans")} WHERE id=$1 FOR UPDATE`, [planId])).rows[0];
      if (!row || row.state !== "PREVIEWING") return null;
      if ((row.preview_owner_token || null) !== (expectedOwnerToken || null)
        || Number(row.preview_owner_epoch || 0) !== Number(expectedOwnerEpoch || 0)
        || isoNullable(row.preview_owner_lease_until) !== (expectedLeaseUntil || null)) return null;
      const takeover = row.preview_owner_token !== ownerToken;
      if (takeover && row.preview_owner_lease_until && new Date(row.preview_owner_lease_until).getTime() > new Date(now).getTime()) return null;
      const epoch = Number(row.preview_owner_epoch || 0) + Number(takeover);
      const summary = json(row.summary_json, {});
      summary.previewOwnerToken = text(ownerToken, "ownerToken");
      summary.previewOwnerEpoch = epoch;
      summary.previewOwnerLeaseUntil = iso(leaseUntil);
      const updated = await transaction.query(`UPDATE ${this.#table("shopee_discount_plans")}
        SET summary_json=$1::jsonb,preview_owner_token=$2,preview_owner_epoch=$3,preview_owner_lease_until=$4,updated_at=$5
        WHERE id=$6 AND state='PREVIEWING' AND preview_owner_token IS NOT DISTINCT FROM $7
          AND preview_owner_epoch=$8 AND preview_owner_lease_until IS NOT DISTINCT FROM $9 RETURNING *`,
      [JSON.stringify(summary), ownerToken, epoch, iso(leaseUntil), iso(this.now()), planId,
        expectedOwnerToken || null, Number(expectedOwnerEpoch || 0), expectedLeaseUntil || null]);
      return updated.rows[0] ? planRow(updated.rows[0]) : null;
    });
  }
  ...
  async getLatestWarehouseBaseline({ country, category, tier, shopId = null }) {
    const row = (await this.provider.query(`SELECT evidence_json FROM ${this.#table("shopee_discount_events")}
      WHERE event_type='WAREHOUSE_BASELINE' AND baseline_country=$1 AND baseline_category=$2
        AND baseline_shop_id IS NOT DISTINCT FROM $3 AND baseline_tier=$4
      ORDER BY occurred_at DESC,id DESC LIMIT 1`, [country, category, shopId, tier])).rows[0];
    return row ? json(row.evidence_json, null) : null;
  }
  ...
  async appendPlanShard({ planId, shardIndex, shardHash, items = [], ownerToken = null, ownerEpoch = 0 }) {
    if (!Number.isInteger(shardIndex) || shardIndex < 0) throw new TypeError("shardIndex must be a non-negative integer");
    const timestamp = iso(this.now());
    return this.provider.transaction(async (transaction) => {
      const plan = (await transaction.query(`SELECT id,state,preview_owner_token,preview_owner_epoch,preview_owner_lease_until
        FROM ${this.#table("shopee_discount_plans")} WHERE id=$1 FOR UPDATE`, [planId])).rows[0];
      ...
      if (plan.preview_owner_token != null && (plan.preview_owner_token !== ownerToken
        || Number(plan.preview_owner_epoch) !== Number(ownerEpoch))) {
        throw error("SHOPEE_DISCOUNT_PREVIEW_LEASE_LOST", "Preview ownership lease was lost");
      }
      const shardId = `${planId}:${shardIndex}`;
      const existingShard = (await transaction.query(`SELECT * FROM ${this.#table("shopee_discount_plan_shards")}
        WHERE plan_id=$1 AND shard_index=$2`, [planId, shardIndex])).rows[0];
      const contentHash = shardContentHash(items, shardIndex);
      if (existingShard) {
        if (existingShard.content_hash != null) {
          if (existingShard.shard_hash !== shardHash || existingShard.content_hash !== contentHash || Number(existingShard.item_count) !== items.length) {
            throw error("SHOPEE_DISCOUNT_PREVIEW_SAGA_CONFLICT", "Persisted shard differs from the requested shard");
          }
          return { planId, shardIndex, shardHash, itemCount: items.length, createdAt: isoNullable(existingShard.created_at) || timestamp };
        }
        const stored = (await transaction.query(`SELECT sequence_no,shop_id,item_id,model_id,item_key,sku,currency,scale,current_price_minor,
          control_price_minor,target_price_minor,payload_hash,payload_json
          FROM ${this.#table("shopee_discount_plan_items")} WHERE plan_id=$1 AND shard_index=$2 ORDER BY sequence_no`, [planId, shardIndex])).rows;
        const matches = existingShard.shard_hash === shardHash && Number(existingShard.item_count) === items.length
          && stored.length === items.length && shardContentHash(stored.map((row) => ({ sequence: Number(row.sequence_no),
            shopId: row.shop_id, itemId: row.item_id, modelId: row.model_id, itemKey: row.item_key, sku: row.sku,
            currency: row.currency, scale: Number(row.scale), currentPriceMinor: row.current_price_minor,
            controlPriceMinor: row.control_price_minor, targetPriceMinor: row.target_price_minor,
            payloadHash: row.payload_hash, payload: json(row.payload_json, {}) })), shardIndex) === contentHash;
        if (!matches) throw error("SHOPEE_DISCOUNT_PREVIEW_SAGA_CONFLICT", "Persisted shard differs from the requested shard");
        return { planId, shardIndex, shardHash, itemCount: items.length, createdAt: isoNullable(existingShard.created_at) || timestamp };
      }
      await transaction.execute(`INSERT INTO ${this.#table("shopee_discount_plan_shards")}
        (id,plan_id,shard_index,shard_hash,item_count,created_at,content_hash) VALUES ($1,$2,$3,$4,$5,$6,$7)`, [
        shardId, planId, shardIndex, text(shardHash, "shardHash"), items.length, timestamp, contentHash,
      ]);
      if (items.length) {
        const values = [], groups = [];
  ...
  async sealPlan({ planId, merkleRoot, itemCount, shardCount, expectedVersion, ownerToken = null, ownerEpoch = 0 }) {
  ...
  async markPlanState({ planId, fromState, toState, expectedVersion, reasonCode = null, ownerToken = null, ownerEpoch = 0 }) {
    ...
    return this.provider.transaction(async (transaction) => {
      const timestamp = iso(this.now());
      const ownerClause = fromState === "PREVIEWING"
        ? " AND (preview_owner_token IS NULL OR (preview_owner_token=$7 AND preview_owner_epoch=$8))" : "";
      const values = [toState, reasonCode, timestamp, planId, fromState, expectedVersion];
      if (fromState === "PREVIEWING") values.push(ownerToken, ownerEpoch);
      const result = await transaction.query(`UPDATE ${this.#table("shopee_discount_plans")} SET state=$1,reason_code=$2,
        state_version=state_version+1,updated_at=$3 WHERE id=$4 AND state=$5 AND state_version=$6${ownerClause} RETURNING *`, values);
  ...
  async listUnknownDispatchIntentsPage({ cursor = null, pageSize = 100 } = {}, authorizedShopIds = null) {
    const clauses = ["i.status='UNKNOWN'"], values = [];
    if (cursor) { values.push(cursor.dispatchedAt, cursor.id); clauses.push(`(i.dispatched_at>$${values.length - 1} OR (i.dispatched_at=$${values.length - 1} AND i.id>$${values.length}))`); }
    if (authorizedShopIds) {
      if (!authorizedShopIds.length) return { items: [], nextCursor: null };
      values.push(authorizedShopIds); clauses.push(`NOT EXISTS (SELECT 1 FROM ${this.#table("shopee_discount_activities")} a WHERE a.plan_id=i.plan_id AND NOT (a.shop_id=ANY($${values.length}::text[])))`);
    }
    const limit = Math.max(1, Math.min(100, Number(pageSize) || 50)); values.push(limit + 1);
    const rows = (await this.provider.query(`SELECT i.* FROM ${this.#table("shopee_discount_dispatch_intents")} i WHERE ${clauses.join(" AND ")}
      ORDER BY i.dispatched_at,i.id LIMIT $${values.length}`, values)).rows;
    const page = rows.slice(0, limit).map(intentRow), last = page.at(-1);
    return { items: page, nextCursor: rows.length > limit ? { dispatchedAt: last.dispatchedAt, id: last.id } : null };
  }
  ...
  async appendEvent(input) {
    ...
    const baselineScope = input.eventType === "WAREHOUSE_BASELINE" ? input.evidence?.scope || {} : {};
    const result = await this.provider.query(`INSERT INTO ${this.#table("shopee_discount_events")} (
      ...
      baseline_country,baseline_category,baseline_shop_id,baseline_tier
    ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10,$9,$11,$12,$13,$14) RETURNING *`, [
      ...
      input.retentionUntil ? iso(input.retentionUntil) : null, baselineScope.country || null,
      baselineScope.category || null, baselineScope.shopId || null, baselineScope.tier || null,
    ]);
  ...
  async deferDueJob({ dueJobId, ownerId, epoch, dueAt, result = {}, lastErrorCode = null }) {
    const timestamp = iso(this.now()), nextDue = iso(dueAt);
    if (new Date(nextDue) <= new Date(timestamp)) throw new TypeError("Deferred due time must be in the future");
    const changed = await this.provider.execute(`UPDATE ${this.#table("shopee_discount_due_jobs")}
      SET status='PENDING',due_at=$1,owner_id=NULL,lease_until=NULL,result_json=$2::jsonb,last_error_code=$3,updated_at=$4
      WHERE id=$5 AND status='CLAIMED' AND owner_id=$6 AND fencing_epoch=$7`, [
      nextDue, JSON.stringify(result), lastErrorCode, timestamp, text(dueJobId, "dueJobId"), text(ownerId, "ownerId"), Number(epoch),
    ]);
    return Number(changed.rowCount || 0) > 0;
  }

  async renewDueJobLease({ dueJobId, ownerId, epoch, leaseMs = 60_000 }) {
    const timestamp = iso(this.now());
    const ttl = Math.max(5_000, Number(leaseMs) || 60_000);
    const leaseUntil = new Date(new Date(timestamp).getTime() + ttl).toISOString();
    const result = await this.provider.execute(`UPDATE ${this.#table("shopee_discount_due_jobs")} SET lease_until=$1,updated_at=$2
      WHERE id=$3 AND status='CLAIMED' AND owner_id=$4 AND fencing_epoch=$5 AND lease_until>$2`, [
      leaseUntil, timestamp, text(dueJobId, "dueJobId"), text(ownerId, "ownerId"), Number(epoch),
    ]);
    return Number(result.rowCount || 0) > 0;
  }
  ...
  async createNotification(input) {
    const id = input.id || randomUUID(), timestamp = iso(input.createdAt || this.now());
    const result = await this.provider.query(`INSERT INTO ${this.#table("shopee_discount_notifications")} (
      id,plan_id,dedupe_key,notification_type,severity,title,message,channel,delivery_status,attempt_count,
      metadata_json,retention_until,created_at,updated_at
    ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'PENDING',0,$9::jsonb,$10,$11,$11) RETURNING *`, [
      id, input.planId || null, text(input.dedupeKey, "dedupeKey"), text(input.notificationType, "notificationType"),
      text(input.severity, "severity"), text(input.title, "title"), text(input.message, "message"), text(input.channel, "channel"),
      JSON.stringify(input.metadata || {}), input.retentionUntil ? iso(input.retentionUntil) : null, timestamp,
    ]);
    return notificationRow(result.rows[0]);
  }

  async getNotificationByDedupeKey(dedupeKey) {
    return notificationRow((await this.provider.query(`SELECT * FROM ${this.#table("shopee_discount_notifications")} WHERE dedupe_key=$1`, [text(dedupeKey, "dedupeKey")])).rows[0]);
  }

  async claimNotificationDelivery({ notificationId, expectedAttemptCount }) {
    const attempt = Number(expectedAttemptCount);
    if (!Number.isSafeInteger(attempt) || attempt < 0) throw new TypeError("expectedAttemptCount is invalid");
    const timestamp = iso(this.now());
    const leaseUntil = new Date(new Date(timestamp).getTime() + 60_000).toISOString();
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_notifications")}
      SET delivery_status='SENDING',attempt_count=attempt_count+1,delivery_lease_until=$1,updated_at=$2
      WHERE id=$3 AND attempt_count=$4 AND delivery_status=ANY($5::text[]) RETURNING *`, [
      leaseUntil, timestamp, text(notificationId, "notificationId"), attempt, ["PENDING", "RETRY_WAIT"],
    ]);
    return notificationRow(result.rows[0]);
  }

  async markNotificationDelivery({ notificationId, patch = {} }) {
    if (!new Set(["RETRY_WAIT", "DELIVERED", "FAILED"]).has(patch.status)) throw new TypeError("Notification delivery status is invalid");
    const timestamp = iso(patch.updatedAt || this.now());
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_notifications")}
      SET delivery_status=$1,attempt_count=$2,last_error_code=$3,delivered_at=$4,delivery_lease_until=NULL,updated_at=$5
      WHERE id=$6 AND delivery_status='SENDING' RETURNING *`, [
      patch.status, Number(patch.attemptCount), patch.lastErrorCode || null,
      patch.deliveredAt ? iso(patch.deliveredAt) : null, timestamp, text(notificationId, "notificationId"),
    ]);
    return notificationRow(result.rows[0]);
  }

  async markNotificationUnknown({ notificationId, reasonCode = "DINGTALK_DELIVERY_UNKNOWN", evidence = {} }) {
    const timestamp = iso(this.now());
    const result = await this.provider.query(`UPDATE ${this.#table("shopee_discount_notifications")}
      SET delivery_status='FAILED',coordination_state='UNKNOWN',coordination_evidence_json=$1::jsonb,
        last_error_code=$2,delivery_lease_until=NULL,updated_at=$3
      WHERE id=$4 AND delivery_status='SENDING' AND delivery_lease_until<=$3 RETURNING *`, [
      JSON.stringify(evidence), text(reasonCode, "reasonCode"), timestamp, text(notificationId, "notificationId"),
    ]);
    return notificationRow(result.rows[0]);
  }
}
