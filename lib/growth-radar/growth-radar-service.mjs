...
import { projectMabangOrderGmvSource } from "../data-foundation/mabang-gmv-contract.mjs";
import {
...
const ORDER_AMOUNT_CNY_SOURCE_FIELD = "订单核算金额（人民币）";
...
function safeScope(value) {
  ...
  const allowed = ["platform", "countryCode", "queryType", "dateFrom", "dateTo", "snapshotAt", "shopCoverageMode"];
...
function resolvedOrderAmount(rows = []) {
  const candidates = rows
    .map((row) => row?.normalized || row)
    .filter((row) => row?.orderAmount !== null && row?.orderAmount !== undefined);
  const directCandidates = candidates.filter(
    (row) => row.orderAmountSourceField === ORDER_AMOUNT_CNY_SOURCE_FIELD,
  );
  const selected = directCandidates.length ? directCandidates : candidates;
  const amounts = [...new Set(selected.map((row) => String(row.orderAmount)))];
  const representative = selected[0] || null;
  return Object.freeze({
    amounts,
    conflict: amounts.length > 1,
    amount: amounts.length === 1 ? Number(amounts[0]) : null,
    currency: amounts.length === 1 ? representative?.orderCurrency || null : null,
    sourceField: amounts.length === 1 ? representative?.orderAmountSourceField || null : null,
  });
}
...
export class GrowthRadarService {
  ...
  async productCandidateSummaryMap(skus) {
    if (typeof this.repository.productCandidateSummaries === "function") {
      const summaries = await this.repository.productCandidateSummaries([...skus]);
      return new Map(summaries.map((item) => [normalizeSku(item.normalizedSku), item]));
    }
    const summaries = new Map();
    for (const sku of skus) {
      const candidates = await this.repository.productCandidates(sku);
      summaries.set(normalizeSku(sku), {
        normalizedSku: normalizeSku(sku),
        candidateCount: candidates.length,
        countryCount: new Set(candidates.map((item) => normalizeText(item.countryCode).toUpperCase())).size,
      });
    }
    return summaries;
  }
  ...
  async orderPreviewSummary(parsed) {
    ...
    for (const rows of groups.values()) {
      ...
      if (resolvedOrderAmount(rows).conflict) {
        orderAmountConflictCount += 1;
      }
    }
    const candidateSummaries = await this.productCandidateSummaryMap(skus);
    let crossCountryAmbiguousSkus = 0;
    let unmatchedSkus = 0;
    let countryUnresolvedSingleCandidateSkus = 0;
    for (const sku of skus) {
      const candidate = candidateSummaries.get(normalizeSku(sku));
      if (!candidate?.candidateCount) unmatchedSkus += 1;
      else if (candidate.countryCount > 1 || candidate.candidateCount > 1) crossCountryAmbiguousSkus += 1;
    ...
    return {
      ...
      lineAmountStatus: validRows.length && validRows.every((row) => row.normalized.lineAmountStatus === "confirmed")
        ? "confirmed"
        : validRows.some((row) => row.normalized.lineAmountStatus === "confirmed")
          ? "partial"
          : validRows.some((row) => row.normalized.lineAmountStatus === "unconfirmed") ? "unconfirmed" : "unavailable",
  ...
  async inventoryPreviewSummary(parsed) {
    ...
    for (const sku of skus) if (!candidateSummaries.get(normalizeSku(sku))?.candidateCount) unmatchedSkuCount += 1;
    const canApply = validRows.length > 0;
    return {
      rawRowCount: parsed.rowCount,
      ...
      daysOfSupplyStatus: validRows.length && validRows.every((row) => row.normalized.daysOfSupplyStatus === "confirmed")
        ? "confirmed"
        : validRows.some((row) => row.normalized.daysOfSupplyStatus === "confirmed") ? "partial" : "unavailable",
    };
  }
  ...
  async applyOrderRows(preview, batchId, at, tx) {
    ...
    for (const [businessKey, rows] of groups) {
      ...
      const orderAmountResolution = resolvedOrderAmount(rows);
      const statuses = [...new Set(rows.map((row) => row.normalized.orderStatus))];
      const gmvSource = projectMabangOrderGmvSource(rows);
      const headerQuality = orderAmountResolution.conflict || statuses.length > 1 || gmvSource.gmvSourceStatus === "CONFLICT"
        ? "review_required" : "confirmed";
      const orderAmount = orderAmountResolution.amount;
      ...
      const headerInput = {
        ...
        orderCurrency: orderAmountResolution.currency,
        orderAmount,
        orderAmountSourceField: orderAmountResolution.sourceField,
        ...gmvSource,
      ...
      if (orderAmountResolution.conflict) {
        await this.repository.upsertQualityIssue({
      ...
      if (gmvSource.gmvSourceStatus !== "CONFIRMED") {
        const issueCode = gmvSource.gmvSourceStatus === "CONFLICT" ? "GMV_COMPONENT_CONFLICT" : "GMV_COMPONENT_MISSING";
        await this.repository.upsertQualityIssue({
          issueKey: sha({ batchId, businessKey, issue: issueCode }),
          batchId, entityType: "order_header", entityId: header.id, issueCode,
          severity: gmvSource.gmvSourceStatus === "CONFLICT" ? "blocker" : "warning",
          message: gmvSource.gmvSourceStatus === "CONFLICT"
            ? "同一订单的 GMV 原始金额或原币优惠金额在来源行中不一致，GMV 保持为空。"
            : "订单缺少原始商品总金额或优惠金额（原始货币），GMV 保持为空。",
          sourceContext: { sourceRowNumbers: rows.map((row) => row.sourceRowNumber) }, status: "open", createdAt: at,
        }, tx);
      }
      ...
      for (const row of rows) {
        ...
        const lineInput = {
          ...
          lineAmount: normalized.lineAmount ?? null,
          lineAmountStatus: normalized.lineAmountStatus || "unavailable",
  ...
  async applyInventoryRows(preview, batchId, at, tx) {
    ...
    for (const row of preview.parsed.rows) {
      ...
      await this.repository.insertInventorySnapshot({
        ...
        transferPendingShipmentQuantity: normalized.transferPendingShipmentQuantity,
        sellableQuantity: normalized.sellableQuantity ?? null,
        sellableQuantityStatus: normalized.sellableQuantityStatus || "unconfirmed",
        ...
        daysOfSupply: normalized.daysOfSupply ?? null,
        daysOfSupplyStatus: normalized.daysOfSupplyStatus || "unavailable",
