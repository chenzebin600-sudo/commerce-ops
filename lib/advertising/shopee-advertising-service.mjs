...
export class ShopeeAdvertisingService {
  ...
  async importCsv(input = {}) {
    ...
    const existing = await this.repository.findBatchByHash(parsed.batch.rawSha256);
    if (existing) return { batch: existing, duplicate: true };
    return { batch: await this.repository.createBatch(parsed.batch, parsed.facts), duplicate: false };
  }
  ...
  async deleteBatch(batchId) {
    ...
    const deleted = await this.repository.deleteBatch(normalizedBatchId);
    if (!deleted) throw Object.assign(new Error("该报表批次不存在或已删除。"), { status: 404, code: "ADS_BATCH_NOT_FOUND" });
    return deleted;
  }

  async saveTargets({ shopId, effectiveFrom, sourceType = "manual", targets = [] } = {}) {
    ...
    return { saved: await this.repository.upsertTargets(records), targets: await this.repository.listTargets(normalizedShopId, effectiveDay) };
  }

  async dashboard({ shopId = null } = {}) {
    const allBatches = await this.repository.listBatches({ shopId, limit: 80 });
    ...
    const [dayFacts, sevenFacts, fourteenFacts, previousFourteenFacts, longFacts] = await Promise.all([
      dayBatch ? this.repository.listFacts(dayBatch.id) : [], sevenBatch ? this.repository.listFacts(sevenBatch.id) : [],
      fourteenBatch ? this.repository.listFacts(fourteenBatch.id) : [],
      previousFourteenBatch ? this.repository.listFacts(previousFourteenBatch.id) : [],
      longBatch ? this.repository.listFacts(longBatch.id) : [],
    ]);
    ...
    const targets = await this.repository.listTargets(selectedShopId, latestTo);
