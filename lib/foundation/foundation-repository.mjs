...
export class FoundationRepository {
  constructor({ provider }) {
    ...
    this.operationPlanApprovalTail = Promise.resolve();
  }
  ...
  async approveOperationPlanWithEvent({ planId, expectedVersion, approvedBy, approvedAt, event }, now = new Date()) {
    const timestamp = iso(now);
    const nextVersion = number(expectedVersion) + 1;
    const perform = async () => this.provider.transaction(async (transaction) => {
      const p = Array.from({ length: 6 }, (_, index) => transaction.placeholder(index + 1));
      const updated = await transaction.execute(
        `UPDATE foundation_operation_plans SET state='APPROVED',approved_by=${p[0]},approved_at=${p[1]},
         state_version=${p[2]},updated_at=${p[3]} WHERE id=${p[4]} AND state='PREVIEWED' AND state_version=${p[5]}`,
        [approvedBy, approvedAt, nextVersion, timestamp, planId, expectedVersion],
      );
      if (updated.rowCount !== 1) throw Object.assign(new Error("Foundation operation plan was changed concurrently."), {
        code: "FOUNDATION_OPERATION_PLAN_VERSION_CONFLICT",
      });
      const ep = Array.from({ length: 14 }, (_, index) => transaction.placeholder(index + 1));
      const inserted = await transaction.execute(
        ...
        ) VALUES (${ep.join(",")}) ON CONFLICT(plan_id,idempotency_key) DO NOTHING`,
        [event.id || randomUUID(), planId, "APPROVED", "PREVIEWED", "APPROVED", event.actorType,
          event.actorId, null, null, toJson(event.evidence), `${planId}:state:${nextVersion}`, nextVersion,
          event.occurredAt || timestamp, timestamp],
      );
      if (inserted.rowCount !== 1) throw Object.assign(new Error("Foundation approval event binding conflicts."), {
        code: "FOUNDATION_OPERATION_APPROVAL_EVENT_CONFLICT",
      });
    });
    if (this.provider.dialect === DATABASE_DIALECTS.SQLITE) {
      const pending = this.operationPlanApprovalTail.then(perform, perform);
      this.operationPlanApprovalTail = pending.catch(() => {});
      await pending;
    } else await perform();
    return this.getOperationPlan(planId);
  }
