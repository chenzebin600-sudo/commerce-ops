...
export class FoundationOperationPlanService {
  ...
  async create({
    ...
    if (id) {
      ...
      if (byId) {
        const expectedExpiresAt = new Date(new Date(byId.createdAt).getTime() + boundedTtl).toISOString();
        const expectedPlanHash = foundationContentHash({
          contractVersion: 1, taskId, operationType: type, approvalMode: mode,
          createdAt: byId.createdAt, expiresAt: expectedExpiresAt, ...hashes, summary,
        });
        const matches = byId.taskId === taskId && byId.operationType === type && byId.approvalMode === mode
          && byId.createdBy === createdBy && byId.expiresAt === expectedExpiresAt && byId.planHash === expectedPlanHash
          && Object.entries(hashes).every(([key, value]) => byId[key] === value)
          && foundationContentHash(byId.summary) === foundationContentHash(summary);
        if (!matches) {
  ...
  async approve(planId, {
    ...
    if (plan.state === "APPROVED") {
      const events = await this.repository.listOperationPlanEvents(planId);
      const approved = [...events].reverse().find((event) => event.eventType === "APPROVED");
      const textHash = approvalText ? foundationContentHash({ text: String(approvalText) }) : null;
      if (plan.planHash === planHash && plan.approvedBy === actorId && approved?.actorType === actorType
        && approved.actorId === actorId && approved.evidence?.planHash === planHash
        && approved.evidence?.approvalMode === plan.approvalMode
        && approved.evidence?.approvalTextHash === textHash && textHash === plan.approvalTextHash) return plan;
    ...
    try { return await this.repository.approveOperationPlanWithEvent({
      planId: plan.id,
      expectedVersion: plan.stateVersion,
      approvedBy: actorId,
      approvedAt: clockDate(this.now).toISOString(),
      event: { actorType, actorId, evidence: {
        planHash: plan.planHash, approvalMode: plan.approvalMode, approvalTextHash: plan.approvalTextHash,
      } },
    }, clockDate(this.now)); } catch (cause) {
      if (cause?.code !== "FOUNDATION_OPERATION_PLAN_VERSION_CONFLICT") throw cause;
      return this.approve(planId, { planHash, approvalText, actorType, actorId });
    }
  }
  ...
  async #assertFresh(plan) {
    const expiresAtMs = Date.parse(plan.expiresAt instanceof Date ? plan.expiresAt.toISOString() : String(plan.expiresAt || ""));
    const nowMs = clockDate(this.now).getTime();
    if (Number.isFinite(expiresAtMs) && expiresAtMs > nowMs) return plan;
    if (["PREVIEWED", "APPROVED"].includes(plan.state)) {
