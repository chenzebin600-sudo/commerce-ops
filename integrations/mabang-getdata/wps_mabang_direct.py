...
# 并发数不宜过高，避免触发马帮限流或让 WPS 运行环境线程过多。
...
class MabangClient:
    ...
    def login(self, username, password):
        ...
        if not data.get("success"):
            ...
            if "可登录IP" in message or "登录IP" in message or "未被设置为可登录IP" in message:
                raise Exception(
                    "马帮账号启用了登录IP限制，WPS云端IP无法在代码中改成本地IP。"
                    "请让管理员在马帮【员工列表-员工详情】中关闭该脚本账号的登录IP限制，"
                    "或者允许该账号从任意IP登录。设置完成后直接重新运行本脚本。"
                )
