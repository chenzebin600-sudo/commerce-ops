...
test("fulfillment dashboard proxy validates and forwards batch SKU replacement jobs", async () => {
  ...
  assert.equal(await proxy(planRequest,planResponse,
    new URL("http://localhost/api/fulfillment-dashboard/sku-replacements/batch-plan")),true);
  ...
  const executeRequest = Readable.from([JSON.stringify({
    batchHash:"a".repeat(64),approvalText:"确认批量更换SKU 2项",unexpected:"drop",
  })]);
  executeRequest.method = "POST";
  assert.equal(await proxy(executeRequest,proxyResponse(),
    new URL("http://localhost/api/fulfillment-dashboard/sku-replacements/batch-execute")),true);
  assert.equal(calls[1].url,"http://127.0.0.1:3112/api/fulfillment/sku-replacements/batch-execute");
  assert.deepEqual(JSON.parse(calls[1].options.body), {
    batchHash:"a".repeat(64),approvalText:"确认批量更换SKU 2项",
  });

  const statusRequest = Readable.from([]);
  statusRequest.method = "GET";
  assert.equal(await proxy(statusRequest,proxyResponse(),
    new URL("http://localhost/api/fulfillment-dashboard/sku-replacements/batch-executions/task-123")),true);
  ...
  assert.equal(await proxy(duplicateRequest,duplicateResponse,
    new URL("http://localhost/api/fulfillment-dashboard/sku-replacements/batch-plan")),true);
...
test("fulfillment dashboard proxy validates SKU preview and single replacement requests", async () => {
  ...
  const cases = [
    ["batch-preview", { orderReferences:["260810ABC123","260810XYZ789"],unexpected:"drop" },
      { orderReferences:["260810ABC123","260810XYZ789"] }],
    ["batch-recover", { orderReferences:["260810ABC123"] }, { orderReferences:["260810ABC123"] }],
    ["plan", { orderReference:"260810ABC123",itemId:"10001",replacementSku:"SKU-NEW",unexpected:"drop" },
      { orderReference:"260810ABC123",itemId:"10001",replacementSku:"SKU-NEW" }],
    ["execute", { planHash:"b".repeat(64),approvalText:"确认更换 SKU",unexpected:"drop" },
      { planHash:"b".repeat(64),approvalText:"确认更换 SKU" }],
  ];
  for (const [route,input,expected] of cases) {
    const req = Readable.from([JSON.stringify(input)]);
    req.method = "POST";
    const response = proxyResponse();
    assert.equal(await proxy(req,response,
      new URL(`http://localhost/api/fulfillment-dashboard/sku-replacements/${route}`)),true);
    assert.equal(response.status,200);
    assert.deepEqual(JSON.parse(calls.at(-1).options.body),expected);
  }
  assert.deepEqual(calls.map((call)=>call.url), cases.map(([route])=>
    `http://127.0.0.1:3112/api/fulfillment/sku-replacements/${route}`));
});
