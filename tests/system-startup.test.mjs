...
test("supervisor locks use a heartbeat before trusting a live PID", () => {
  for (const file of ["system-supervisor.mjs", "fulfillment-supervisor.mjs"]) {
    const source = fs.readFileSync(path.join(rootDir, "scripts", file), "utf8");
    assert.match(source, /LOCK_FRESH_MS/);
    assert.match(source, /setInterval\(refreshLock, 30_000\)/);
    assert.match(source, /lockAge <= LOCK_FRESH_MS/);
  }
});
...
test("Windows development entry elevates a truthful safe stop before automatic reload", () => {
  ...
  const restartScript = fs.readFileSync(path.join(rootDir, "scripts", "restart-system.cmd"), "utf8");
  ...
  assert.match(restartScript, /stop-system\.ps1/);
  assert.match(restartScript, /Restart was cancelled/);
  assert.match(restartScript, /--no-pause/);
