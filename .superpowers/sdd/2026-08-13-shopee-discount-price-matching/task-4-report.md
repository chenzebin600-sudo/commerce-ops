...
- Replaced the broad invented write-business names with endpoint-specific exact sets curated from the bundled `add_discount`, `add_discount_item`, and `update_discount_item` error tables and nested documented `discount.error_time`. Representative create/add/update codes are regression-tested. Technical, auth, and overloaded codes are excluded; notably `error_param` remains UNKNOWN for all three endpoints because each snapshot table assigns it multiple meanings, including gateway/protocol ambiguity.
- Trusted-topology hostnames now follow DNS label rules: 1–253 total characters, 1–63 per label, no empty labels, alphanumeric label ends, and hyphens only internally. Dot/hyphen-only, leading/trailing dot or hyphen, repeated-dot, and oversized-label forms fail closed.
- Read classification now gives valid HTTP 401/403 precedence even for malformed bodies, then evaluates documented rate, technical, auth, and business codes before any message heuristic. Auth/rate wording is consulted only when the platform code is empty; `error_server`/`error_network` containing auth/token text remains bounded-retry eligible.

Round 2 RED evidence: endpoint-specific create/add/update business codes initially became UNKNOWN; malformed 401/403 reads initially became malformed-contract errors; technical codes with auth/token messages initially became auth failures; known business codes with rate/auth text initially became rate-limit failures; and invalid DNS-label topologies were initially accepted. The targeted implementation followed each observed failure.
