...
状态：G0 DESIGN COMPLETE / G1A PLANNED / NOT IMPLEMENTED
...
7. **未知状态必须保持未知。** 当前产品包没有图片、AI 内容、平台属性和刊登结果，G1A 只能显示“未接入”或“未评估”，不得把空值伪装成 0、失败或处理中。
...
`uploaded → validating → preview_ready → applying → applied`

异常状态：`validation_failed`、`apply_failed`、`cancelled`。回滚预留状态为 `rollback_pending → rolled_back / rollback_failed`。

解析、校验和暂存可以按行分块执行；正式应用批次必须是原子事务，不使用 `partially_applied` 作为业务终态。应用失败时保留来源文件、预览、决策和错误证据，但当前产品事实不得停留在只应用了一部分的状态。
...
资料完整度不能只有一个总百分比。系统按五个维度分别输出得分、已完成项、缺失项、阻塞项和建议动作：

| 维度 | G1A 状态 | 首批检查内容 | 后续启用条件 |
|---|---|---|---|
| 基础资料 | 启用 | SKU、主 SKU、款号、款名、品类、产品状态 | G1A 导入应用完成 |
| 规格包装 | 启用 | 销售规格、尺寸、净重、毛重、箱规、装箱数 | G1A 导入应用完成 |
| 成本价格 | 启用 | 人民币成本、国家币成本、汇率口径、四档价一致性 | G1A 导入应用完成 |
| 素材内容 | 未评估 | 图片、视频、说明书、卖点、FAQ、本地化文案 | G1B 素材与内容富化完成 |
| 刊登合规 | 未评估 | 平台类目、必填属性、禁限售、国家合规、利润和库存门槛 | 国家适配与刊登模块完成 |

G1A 页面只展示“基础治理得分”和“已评估 3/5 个维度”。未启用维度的分数必须为 `NULL`、状态为 `not_evaluated`，不能按 0 分计入总分。规则需要版本、适用类目、严重程度、启停状态和生效时间；规则结果需要记录证据值和规则版本，避免规则升级后历史结果无法解释。

### 15.5 数据问题中心

导入校验、身份解析和完整度计算统一产出问题，不在各页面维护互不相认的错误字符串。首期问题包括：

- 缺少主 SKU、缺少款号、重复 SKU、款号关联冲突。
- 成本异常、汇率异常、尺寸异常、重量异常。
- 资料缺失、图片缺失、平台属性缺失和导入冲突。

状态统一为：`pending → in_progress → pending_review → resolved`，也允许从待处理或处理中进入 `ignored`。同一实体、问题类型、字段和规则形成稳定问题指纹；后续批次重复发现时更新 `last_seen_batch_id`，不重复制造待办。已解决问题再次出现时重新打开并保留历史事件。

问题必须记录严重程度、实体、字段、当前值、建议值、首次/最近来源批次、负责人标签、处理状态和处理事件。当前认证只有单一访问 Token，尚不能证明自然人身份；G1A 首期负责人只能是经过校验的操作员标签，不能伪造用户外键。未来接入用户与角色体系后再迁移为真实人员身份。

### 15.6 字段来源与修改边界

| 字段来源 | 前端标识 | 修改入口 | 规则 |
|---|---|---|---|
| 中台事实 | 中台来源 | 无直接编辑 | 只能由已确认产品包批次或未来中台同步更新 |
| 运营扩展 | 人工维护 | 运营编辑流程 | 必须记录旧值、新值、原因和操作审计 |
| AI 候选 | AI 生成 | 人工审核后采纳 | 未审核值不得成为正式刊登事实 |
| 平台回读 | 平台回读 | 无人工覆盖 | 只能由平台同步写入 |
| 系统计算 | 系统计算 | 修改规则或参数 | 保存规则版本与输入证据 |
| 纠错建议 | 待纠错 | 数据问题中心 | 不改写中台原值；通过纠错事件、别名或临时例外处理 |

详情抽屉和差异预览必须在字段旁显示来源标识。写接口按字段所有权白名单校验，不能只依靠前端禁用输入框。
...
目标：先形成运营人员真实可用的产品事实闭环，不等待素材、AI 或刊登能力完成。

交付：

- 产品包安全上传、34 字段校验、批次登记和文件哈希幂等。
- 新增、更新、无变化、冲突、异常的字段级差异预览和人工决策。
- 原子应用、变更历史和回滚证据设计，不覆盖运营、AI 或平台字段。
- 类目、款系、主 SKU 与 SKU 的可追溯身份模型。
- 服务端搜索、筛选、排序、分页、列配置和工作区保存视图。
- 快速详情抽屉、可解释完整度和统一数据问题中心。

验收重点：262 行真实样本可重复导入；来源事实可回放；问题可追踪；列表不伪造图片、AI 或刊登数据；原四个业务模块不受影响。

### G1B：产品素材与内容富化中心

目标：在可信产品身份上补齐图片、视频、说明书、结构化属性与本地化内容。

交付：

- 素材文件元数据、版本、证据来源、国家/平台/SKU 适用范围。
- 核心强类型属性与类目动态属性定义。
- 销售规格和尺寸原文的 AI 候选提取与人工审核。
- 卖点、FAQ、本地化文案和内容审核状态。
- 启用“素材内容”完整度维度，但不直接覆盖中台事实。

验收重点：文件继续进入统一持久化与安全边界；AI 候选必须经人工审核；素材与内容能够追溯到产品和来源。

### G2：国家适配与成本利润基线

目标：把同一商品事实安全映射到不同国家的成本、价格和运营约束。

交付：国家商品适配、汇率口径、平台费率、物流与售后风险参数、利润场景、最低控价和阻断规则。

验收重点：金额由高精度规则计算；来源四档价与规则结果并列展示；缺少关键参数时保持未知并阻断高风险动作。

### G3：刊登模板与批量草稿

目标：基于产品、素材和国家适配生成 Shopee、TikTok Shop、Lazada 多店草稿。

交付：平台类目与属性映射、模板版本、变体映射、多语言内容、批量草稿、人工审核和刊登合规完整度。

验收重点：一套物理事实、多套平台表达；首期只生成可审核草稿，不直接全自动发布。

### G4：订单、库存、广告经营关联

目标：形成“商品 → 链接 → 流量 → 订单 → 库存 → 利润”的可查询关系。

交付：马帮 SKU 与平台 Item/Variation 桥接、订单和库存快照归因、广告记录归因、商品经营摘要、无法映射队列。

验收重点：历史事实不随产品当前值变化；映射保留来源、证据与置信度；广告和利润可追到 SKU。

### G5：客服、售后、活动与任务中心

目标：把客服、退款差评、活动报名和异常处理接入统一商品身份与问题队列。

交付：产品知识查询、售后归因、活动资格与控价校验、统一待办、负责人、处理时限和效果复盘。

验收重点：高风险回复、退款、赔偿和活动价格保持人工确认；处理结果反向沉淀为产品问题证据。

### G6：中台与平台增量同步

目标：在 Excel 全量导入稳定后，将数据获取方式升级为可恢复的定时同步。

交付：中台/平台适配器、游标、分页、重试、限速、登录失效、增量同步、定期全量对账和变化影响清单。

验收重点：采集方式变化不改变下游合同；断点续传不重复；同步失败不污染当前主数据；中台变化不直接修改平台链接。

### G7：受控自动化与运营闭环

目标：在模板、利润、库存、权限和审核规则稳定后提升一人多店产能。

交付：平台发布与回读、失败重试和补偿、7/14/30 天表现跟踪、AI 建议与人工批准、活动/联盟/达人动作、效果复盘。

验收重点：先建议、再审批、后受控自动执行；任何动作可追溯、可停止、可补偿、可复查。

## 19. G1A-0 设计冻结前的业务确认项

以下问题不影响 G0 设计完成，但必须在 G1A Schema 和导入规则冻结前确认：
