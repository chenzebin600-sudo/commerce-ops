...
- 达到 `MABANG_IMAGE_MAX_SKUS_PER_BATCH`（默认 100）个唯一标准化 SKU；同一 SKU 的多仓库行不重复占用名额。

批次创建时会把 SKU 上限固化到批次接口档案中。暂停、进程重启或失败恢复时，采集器先从已持久化的 discovery 记录重建唯一 SKU 集合，再继续填充剩余名额；恢复后不会因环境变量变化突破原批次上限。达到上限的当前页处理完成后，批次正常结束且不再请求下一页。
